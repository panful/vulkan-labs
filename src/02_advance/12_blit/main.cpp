...
private:
  ...
  void PrepareImGui() {
    ...
    ImGui::Begin("Blit control");
    {
      ...
      if (ImGui::Button("PPM")) {
        SaveScreenshot("screenshot_" + std::to_string(s_index++), FileType::PPM);
        std::cout << "Successfully saved\n";
      }

      if (ImGui::SameLine(), ImGui::Button("JPEG")) {
        SaveScreenshot("screenshot_" + std::to_string(s_index++), FileType::JPEG);
        std::cout << "Successfully saved\n";
      }

      ImGui::SliderFloat("RotateAngle", &m_rotate_angle, 0.F, 360.F);
      ImGui::ColorEdit3("BackGround", m_back_graound.data());
      ImGui::InputFloat3("RotateAxisXYZ", m_rotate_axis.data());
    }
    ImGui::End();
  ...
  void CreateSwapChain() {
    ...
    create_info.imageUsage =
      ...
      VK_IMAGE_USAGE_TRANSFER_SRC_BIT;  // 指定在图像上进行怎样的操作，比如显示（颜色附件）、后处理等
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_12_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_12_base_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    clear_values.at(0).color = {{m_back_graound[0], m_back_graound[1], m_back_graound[2], 1.F}};
  ...
  void DrawFrame() {
    ...
    VkResult result =
      vkAcquireNextImageKHR(m_device, m_swap_chain, std::numeric_limits<uint64_t>::max(),
                            m_image_available_semaphores.at(m_current_frame), VK_NULL_HANDLE, &m_image_index);
    ...
    vkResetCommandBuffer(m_command_buffers.at(m_image_index), 0);
    RecordCommandBuffer(m_command_buffers.at(m_image_index), m_swap_chain_framebuffers.at(m_image_index));
    ...
    submit_info.pCommandBuffers = &m_command_buffers[m_image_index];  // 指定实际被提交执行的指令缓冲对象
    submit_info.signalSemaphoreCount = 1;
    submit_info.pSignalSemaphores =
      &m_render_finished_semaphores.at(m_image_index);  // 指定在指令缓冲执行结束后发出信号的信号量对象
    ...
    present_info.pWaitSemaphores = &m_render_finished_semaphores.at(m_image_index);  // 指定开始呈现操作需要等待的信号量
    present_info.swapchainCount = 1;
    present_info.pSwapchains = &m_swap_chain;     // 指定用于呈现图像的交换链
    present_info.pImageIndices = &m_image_index;  // 指定需要呈现的图像在交换链中的索引
  ...
  /// @param commandBuffer
  void EndSingleTimeCommandsWithFences(VkCommandBuffer command_buffer) const noexcept {
    vkEndCommandBuffer(command_buffer);
  ...
  void UpdateUniformBuffer(uint32_t current_image) {
    ...
    ubo.model = glm::rotate(glm::mat4(1.F), glm::radians(m_rotate_angle),
                            glm::vec3(m_rotate_axis[0], m_rotate_axis[1], m_rotate_axis[2]));
  ...
  void SaveScreenshot(const std::string& file_name, FileType type) {
    VkFormatProperties format_props{};
    bool supports_blit{true};

    // 检查GPU是否支持对交换链图像进行blit操作
    vkGetPhysicalDeviceFormatProperties(m_physical_device, m_swap_chain_image_format, &format_props);
    ...
    vkGetPhysicalDeviceFormatProperties(m_physical_device, VK_FORMAT_R8G8B8A8_UNORM, &format_props);
    ...
    // 屏幕呈现的图像
    VkImage src_image = m_swap_chain_images[m_image_index];

    // 用来将屏幕图像缓存起来，然后将这个图像数据保存为图片文件
    VkImage dst_image = {};
    VkDeviceMemory dst_memory = {};
    CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_LINEAR,
                VK_IMAGE_USAGE_TRANSFER_DST_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, dst_image, dst_memory);

    auto cmd_buffer = BeginSingleTimeCommands();

    // 将目标图像转换为传输目标布局
    InsertImageMemoryBarrier(cmd_buffer, dst_image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0,
                             VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT);

    // 将交换链图像从展示布局转换为传输源布局
    InsertImageMemoryBarrier(cmd_buffer, src_image, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                             VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_MEMORY_READ_BIT,
                             VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT);

    if (supports_blit) {
      BlitImage(cmd_buffer, m_swap_chain_extent.width, m_swap_chain_extent.height, src_image, dst_image);
    } else {
      CopyImage(cmd_buffer, m_swap_chain_extent.width, m_swap_chain_extent.height, src_image, dst_image);
    }

    // 将目标图像转换为通用布局
    InsertImageMemoryBarrier(cmd_buffer, dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL,
                             VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT);

    // blit操作完成后，将交换链图像布局恢复
    InsertImageMemoryBarrier(cmd_buffer, src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                             VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_MEMORY_READ_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

    EndSingleTimeCommandsWithFences(cmd_buffer);
    ...
    WriteImage(file_name, type, supports_blit, m_swap_chain_extent.width, m_swap_chain_extent.height, data);
  ...
  void InsertImageMemoryBarrier(VkCommandBuffer cmdbuffer, VkImage image, VkImageLayout old_layout,
                                VkImageLayout new_layout, VkAccessFlags src_access_mask, VkAccessFlags dst_access_mask,
                                VkPipelineStageFlags src_stage_flags, VkPipelineStageFlags dst_stage_flags) {
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.oldLayout = old_layout;
    barrier.newLayout = new_layout;
  ...
  void BlitImage(VkCommandBuffer cmdbuffer, uint32_t width, uint32_t height, VkImage src_image, VkImage dst_image) {
    ...
    vkCmdBlitImage(cmdbuffer, src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dst_image,
                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &image_blit_region, VK_FILTER_NEAREST);
  }
  ...
  void CopyImage(VkCommandBuffer cmdbuffer, uint32_t width, uint32_t height, VkImage src_image, VkImage dst_image) {
    ...
    vkCmdCopyImage(cmdbuffer, src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dst_image,
...
private:
  ...
  static void WriteImage(const std::string& file_name, FileType type, bool rgba, uint32_t w, uint32_t h,
                         const uint8_t* data) {
    switch (type) {
      case FileType::PPM: {
        std::ofstream ppm_file(file_name + ".ppm");
        ppm_file << "P3\n" << w << " " << h << "\n255\n";
      ...
      case FileType::JPEG: {
        if (rgba) {
          ...
          for (size_t i = 0; i < w * h * 4 - 3; i += 4) {
            rgba_data[i] = data[i + 2];
            rgba_data[i + 1] = data[i + 1];
            rgba_data[i + 2] = data[i];
            rgba_data[i + 3] = data[i + 3];
          }

          stbi_write_jpg((file_name + ".jpg").c_str(), static_cast<int>(w), static_cast<int>(h), 4, rgba_data.data(),
                         100);
        }
      } break;
      default:
...
private:
  ...
  std::array<float, 3> m_back_graound{.1f, .2f, .3f};
  std::array<float, 3> m_rotate_axis{1.F, 1.F, 1.F};
