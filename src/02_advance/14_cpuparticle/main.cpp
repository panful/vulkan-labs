...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_14_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_14_base_frag.spv");
  ...
  void UpdateParticles() {
    ...
    std::memcpy(m_vertex_buffers_mapped, g_vertices.data(), static_cast<size_t>(m_buffer_size));
  }
  ...
  void CreateVertexBuffer() {
    m_buffer_size = sizeof(Vertex) * g_vertices.size();

    CreateBuffer(m_buffer_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, m_vertex_buffer,
                 m_vertex_buffer_memory);

    vkMapMemory(m_device, m_vertex_buffer_memory, 0, m_buffer_size, 0, &m_vertex_buffers_mapped);
    std::memcpy(m_vertex_buffers_mapped, g_vertices.data(), static_cast<size_t>(m_buffer_size));
...
private:
  ...
  void* m_vertex_buffers_mapped{nullptr};
