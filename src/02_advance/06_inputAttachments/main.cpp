...
private:
  ...
  void CreateAttachments() {
    ...
    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      ...
      if (s_index > 0) {
        ...
        barrier.image = m_attachment_image_depth[i];
        ...
        vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_06_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_06_base_frag.spv");
  ...
  void CreateGraphicsPipelineQuad() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_06_quad_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_06_quad_frag.spv");
  ...
  void CreateRenderPass() {
    ...
    color_reference.attachment = 1;  // 帧缓冲第0个附件是交换链，第1个是颜色附件，第2个是深度附件
    ...
    subpass_descriptions[0].pColorAttachments = &color_reference;
    ...
    std::array<VkAttachmentDescription, 3> attachments{swap_chain_attachment, color_attachment, depth_attachment};
  ...
  void CreateFramebuffers() {
    ...
    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      ...
      std::array<VkImageView, 3> attachments{m_swap_chain_image_views[i], m_attachment_view_color[i],
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    clear_values.at(1).color = {{.1f, .2f, .3f, 1.F}};  // 清除色，相当于背景色
    clear_values.at(2).depthStencil = {
  ...
  void RecreateSwapChain() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      depth_image_info.sampler = nullptr;  // 输入附件不需要采样器，仅仅只是像素本地加载不需要采样
      ...
      descriptor_writes[1].pImageInfo = &depth_image_info;
  ...
  void CleanupSwapChain() noexcept {
    ...
    for (auto&& image : m_attachment_image_color) {
      vkDestroyImage(m_device, image, nullptr);
    }
    for (auto&& memory : m_attachment_memory_color) {
      vkFreeMemory(m_device, memory, nullptr);
    }
    for (auto&& view : m_attachment_view_color) {
      vkDestroyImageView(m_device, view, nullptr);
    }

    for (auto&& image : m_attachment_image_depth) {
      vkDestroyImage(m_device, image, nullptr);
    }
    for (auto&& memory : m_attachment_memory_depth) {
      vkFreeMemory(m_device, memory, nullptr);
    }
    for (auto&& view : m_attachment_view_depth) {
      vkDestroyImageView(m_device, view, nullptr);
    }
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes[2].descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2);  // color和depth，所以需要乘以2
