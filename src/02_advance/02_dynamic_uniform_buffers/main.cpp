...
constexpr int k_primitives_count = 2;
...
private:
  ...
  void Cleanup() noexcept {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      vkFreeMemory(m_device, m_uniform_buffer_memory.at(i), nullptr);
    }

    AlignedFree(m_dynamic_uniforms);
...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_02_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_02_base_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    uint32_t uniform_offset = static_cast<uint32_t>(0 * m_dynamic_alignment);
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1,
                            &m_descriptor_sets[m_current_frame], 1, &uniform_offset);
    ...
    uniform_offset = static_cast<uint32_t>(1 * m_dynamic_alignment);
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1,
                            &m_descriptor_sets[m_current_frame], 1, &uniform_offset);
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
  ...
  void DrawFrame() {
    ...
    UpdateUniform(static_cast<uint32_t>(m_current_frame));
  ...
  void CreateDescriptorSetLayout() {
    VkDescriptorSetLayoutBinding ds_layout = {};
    ds_layout.binding = 0;
    ds_layout.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC;
    ds_layout.descriptorCount = 1;
    ds_layout.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    ds_layout.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutCreateInfo layout_info = {};
    layout_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    layout_info.bindingCount = 1;
    layout_info.pBindings = &ds_layout;
  ...
  void CreateUniformBuffer() {
    ...
    m_dynamic_alignment = sizeof(UniformDynamicColor);
    if (min_ubo_alignment > 0) {
      m_dynamic_alignment = (m_dynamic_alignment + min_ubo_alignment - 1) & ~(min_ubo_alignment - 1);
    }
    std::cout << m_dynamic_alignment << '\n';

    auto buffer_size = k_primitives_count * m_dynamic_alignment;

    m_uniform_buffers.resize(k_max_frames_in_flight);
    m_uniform_buffer_memory.resize(k_max_frames_in_flight);
    m_uniform_buffer_mapped.resize(k_max_frames_in_flight);
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      vkMapMemory(m_device, m_uniform_buffer_memory[i], 0, buffer_size, 0, &m_uniform_buffer_mapped[i]);
    }

    m_dynamic_uniforms = static_cast<glm::vec3*>(AlignedAlloc(buffer_size, m_dynamic_alignment));
  }
  ...
  void CreateDescriptorSets() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      buffer_info.range = m_dynamic_alignment;
  ...
  void UpdateUniform(uint32_t current_image) {
    auto* uniform_bytes = static_cast<char*>(static_cast<void*>(m_dynamic_uniforms));
    auto* ubo0 = static_cast<glm::vec3*>(static_cast<void*>(uniform_bytes + (0 * m_dynamic_alignment)));
    *ubo0 = glm::vec3(1.F, 0.F, 0.F);
    auto* ubo1 = static_cast<glm::vec3*>(static_cast<void*>(uniform_bytes + (1 * m_dynamic_alignment)));
    *ubo1 = glm::vec3(0.F, 1.F, 0.F);

    size_t buffer_size = k_primitives_count * m_dynamic_alignment;
    std::memcpy(m_uniform_buffer_mapped[current_image], m_dynamic_uniforms, buffer_size);

    VkMappedMemoryRange mapped_memory_range = {};
    mapped_memory_range.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
    mapped_memory_range.memory = m_uniform_buffer_memory[current_image];
    mapped_memory_range.size = buffer_size;
...
private:
  ...
  std::vector<VkDeviceMemory> m_uniform_buffer_memory{};
  std::vector<void*> m_uniform_buffer_mapped{};
  std::vector<VkDescriptorSet> m_descriptor_sets{};
  VkDescriptorPool m_descriptor_pool{nullptr};

  glm::vec3* m_dynamic_uniforms{nullptr};
  VkDeviceSize m_dynamic_alignment{0};
};
