...
private:
    ...
    void CreateAttachments(bool first = false)
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            CreateAttachmentImpl(
                ...
                m_attachmentImageColor[i],
                m_attachmentMemoryColor[i],
            ...
            CreateAttachmentImpl(
                ...
                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                m_attachmentImageNormal[i],
                m_attachmentMemoryNormal[i],
                m_attachmentViewNormal[i]
            );

            CreateAttachmentImpl(
                VK_FORMAT_R16G16B16A16_SFLOAT,
                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                m_attachmentImagePosition[i],
                m_attachmentMemoryPosition[i],
                m_attachmentViewPosition[i]
            );

            CreateAttachmentImpl(
                FindDepthFormat(),
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                m_attachmentImageDepth[i],
                m_attachmentMemoryDepth[i],
                m_attachmentViewDepth[i]
            );
    ...
    {
        auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_08_base_vert.spv");
        auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_08_base_frag.spv");
    ...
    {
        auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_08_quad_vert.spv");
        auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_08_quad_frag.spv");
    ...
    void CreateRenderPass()
    {
        ...
        std::array<VkAttachmentDescription, 5> attachments {
            swapChainAttachment, colorAttachment, normalAttachment, positionAttachment, depthAttachment
        };
    ...
    void CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            ...
            std::array<VkImageView, 5> attachments {
                m_swapChainImageViews[i], m_attachmentViewColor[i], m_attachmentViewNormal[i], m_attachmentViewPosition[i], m_attachmentViewDepth[i]
            };
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        clearValues.at(1).color = {
            {0.f, 0.f, 0.f, 0.f}
        };
        clearValues.at(2).color = {
            {0.f, 0.f, 0.f, 0.f}
        };
        clearValues.at(3).color = {
            {0.f, 0.f, 0.f, 0.f}
        };
    ...
    void CreateUniformBufferLight()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            CreateBuffer(
                ...
                VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                m_uniformBufferLight.at(i),
                m_uniformBufferLightMemory.at(i)
            );
