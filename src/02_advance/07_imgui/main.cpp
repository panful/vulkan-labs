...
private:
  ...
  int m_combo_index{0};
  float m_slider{50.F};
  bool m_check_box0{false}, m_check_box1{false}, m_check_box2{false};
  const std::array<float, 3> m_input_float3{0.F};
  int m_radio{0};
  int m_radio1{0};
  const std::array<float, 3> m_color{1.F};
  ...
  void PrepareImGui() {
    ...
    if (m_open) {
      ImGui::Begin("Window 0", &m_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
      {
    ...
    ImGui::Begin("Window 1");
    {
      if (ImGui::TreeNode("tree1")) {
        ImGui::RadioButton("Option A", &m_radio, 0);
        ImGui::SameLine();
        ImGui::RadioButton("Option B", &m_radio, 1);
        ImGui::TreePop();
      }
      ...
      //     //-----------------------------------------------------------------------
      //     // 显示文本
      //     ImGui::TextUnformatted("This is a simple GUI");
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_07_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_07_base_frag.spv");
    ...
    vertex_input_info.pVertexBindingDescriptions = nullptr;  // 可选的
    vertex_input_info.vertexAttributeDescriptionCount = 0;
    vertex_input_info.pVertexAttributeDescriptions = nullptr;  // 可选的
