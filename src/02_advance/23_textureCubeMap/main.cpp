...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        destroy_texture(m_texture_cube);
        vkDestroySampler(m_device, m_sampler_cube, nullptr);
...
private:
    void CreateDrawables()
    {
        ...
        CreateTexture(m_texture_cube);
        CreateSampler(m_sampler_cube);
    }
    ...
    {
        std::array<std::string, 6> cube_texture_names {
            PROJECT_ASSETS_DIR "textures/skybox/skybox_right.jpg",
            PROJECT_ASSETS_DIR "textures/skybox/skybox_left.jpg",
            PROJECT_ASSETS_DIR "textures/skybox/skybox_top.jpg",
            PROJECT_ASSETS_DIR "textures/skybox/skybox_bottom.jpg",
            PROJECT_ASSETS_DIR "textures/skybox/skybox_front.jpg",
            PROJECT_ASSETS_DIR "textures/skybox/skybox_back.jpg",
        };

        std::array<std::tuple<VkBuffer, VkDeviceMemory, uint32_t, uint32_t>, 6> ret_val {};
        ...
        for (uint32_t i = 0; i < 6; ++i)
        {
            ...
            int tex_height {0};
            int tex_channels {0};
            auto pixels = stbi_load(cube_texture_names[i].c_str(), &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
            ...
            // 保证每张图片大小一致
            if (!pixels || width != tex_width || height != tex_height)
            {
            ...
            ret_val[i] = std::forward_as_tuple(staging_buffer, staging_buffer_memory, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
        }
    ...
    {
        auto cube_map = LoadCubeMap();
        auto width    = std::get<2>(cube_map.front());
        auto height   = std::get<3>(cube_map.front());
        ...
        for (uint32_t i = 0; i < 6; ++i)
        {
            CopyBufferToImage(std::get<0>(cube_map[i]), texture.image, width, height, i);

            vkDestroyBuffer(m_device, std::get<0>(cube_map[i]), nullptr);
            vkFreeMemory(m_device, std::get<1>(cube_map[i]), nullptr);
        }
    ...
    void TransitionImageLayout(VkImage image, VkFormat /*format*/, VkImageLayout old_layout, VkImageLayout new_layout, bool cube_map = false)
    {
        VkCommandBuffer command_buffer = BeginSingleTimeCommands();
        ...
        barrier.subresourceRange.layerCount     = cube_map ? 6 : 1;
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_23_cube_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_23_cube_frag.spv");
    ...
    VkImageView CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspect_flags, VkImageViewType type = VK_IMAGE_VIEW_TYPE_2D)
    {
        ...
        view_info.subresourceRange.layerCount     = type == VK_IMAGE_VIEW_TYPE_CUBE ? 6 : 1;
    ...
    void CreateDescriptorSets()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            image_info.imageView   = m_texture_cube.imageView;
            image_info.sampler     = m_sampler_cube;
...
private:
    ...
    Texture m_texture_cube {};
    VkSampler m_sampler_cube {};
