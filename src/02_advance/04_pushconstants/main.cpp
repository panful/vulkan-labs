...
private:
  ...
  bool IsDeviceSuitable(VkPhysicalDevice device) const noexcept {
    ...
    auto family_indices = FindQueueFamilies(device);
    ...
    return family_indices.IsComplete() && extensions_supported && swap_chain_adequate;
  }
  ...
  void CreateLogicalDevice() {
    auto family_indices = FindQueueFamilies(m_physical_device);

    std::vector<VkDeviceQueueCreateInfo> queue_create_infos;
    std::set<uint32_t> unique_queue_families{lvk_tidy::GetRequiredValue(family_indices.graphics_family),
                                             lvk_tidy::GetRequiredValue(family_indices.present_family)};
    ...
    // 4.用来存储返回的队列句柄的内存地址
    vkGetDeviceQueue(m_device, lvk_tidy::GetRequiredValue(family_indices.graphics_family), 0, &m_graphics_queue);
    vkGetDeviceQueue(m_device, lvk_tidy::GetRequiredValue(family_indices.present_family), 0, &m_present_queue);
  }
  ...
  void CreateSwapChain() {
    ...
    const std::array<uint32_t, 2> queue_family_indices{
      static_cast<uint32_t>(lvk_tidy::GetRequiredValue(family_indices.graphics_family)),
      static_cast<uint32_t>(lvk_tidy::GetRequiredValue(family_indices.present_family))};
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base_frag.spv");
  ...
  void CreateCommandPool() {
    ...
    pool_info.queueFamilyIndex = lvk_tidy::GetRequiredValue(family_indices.graphics_family);
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &pc);
...
private:
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &pc2);

    const std::array<VkBuffer, 1> vertex_buffers2{m_vertex_buffer2};
    const std::array<VkDeviceSize, 1> offsets2{0};
    vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers2.data(), offsets2.data());
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base2_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base2_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    PushConstant pc = {{1.F, 0.F, 0.F}, {0.F, 1.F, 0.F}};
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base3_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base3_frag.spv");
    ...
    const std::array<VkPushConstantRange, 2> push_constant_ranges{push_constant_range_vert, push_constant_range_frag};
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    // 顶点和片段着色器使用的 PushConstant 一次性提交，着色器中只访问自己需要的数据即可
    PushConstantVert pc_vert = {{1.F, 0.F, 0.F}};
    PushConstantFrag pc_frag = {{0.F, 1.F, 0.F}};
    vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstantVert),
                       &pc_vert);
    vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(PushConstantVert),
                       sizeof(PushConstantFrag), &pc_frag);
