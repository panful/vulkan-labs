...
private:
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        // 顶点和片段着色器使用的 PushConstant 一次性提交，着色器中只访问自己需要的数据即可
        PushConstantVert pc_vert = {
            {1.F, 0.F, 0.F}
        };
        PushConstantFrag pc_frag = {
            {0.F, 1.F, 0.F}
        };
        vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstantVert), &pc_vert);
        vkCmdPushConstants(
            command_buffer, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(PushConstantVert), sizeof(PushConstantFrag), &pc_frag
        );
