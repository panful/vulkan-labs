...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base_frag.spv");
...
private:
  ...
  void RecordCommandBuffer(VkCommandBuffer commandBuffer, VkFramebuffer framebuffer) const {
    ...
    const std::array<VkBuffer, 1> vertexBuffers2{m_vertexBuffer2};
    const std::array<VkDeviceSize, 1> offsets2{0};
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers2.data(), offsets2.data());
    vkCmdBindIndexBuffer(commandBuffer, m_indexBuffer, 0, VK_INDEX_TYPE_UINT16);
  ...
  void CreateVertexBuffer2() {
    ...
    CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_vertexBuffer2, m_vertexBufferMemory2);
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base2_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_04_base2_frag.spv");
