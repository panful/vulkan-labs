...
private:
    ...
    {
        auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_21_base_vert.spv");
        auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_21_base_frag.spv");
...
public:
    ...
    {
        m_output_colors = colors;
    }

    void SetDepthOutput(const ImageData& depth)
    {
        m_output_depth = depth;
    }
    ...
    void Execute(VkCommandBuffer command_buffer, const UniformBufferObject& ubo, const size_t frame_index, const uint32_t image_index) const
    {
    ...
    void CreateGraphicsPipeline()
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_21_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_21_base_frag.spv");
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, const size_t frame_index, const uint32_t image_index) const
    {
        ...
        vkCmdBindDescriptorSets(
            command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1, &m_descriptor_sets[frame_index], 0, nullptr
        );
...
public:
    ...
    void Execute(VkCommandBuffer cmd, const UniformBufferObject& ubo, const size_t frame_index, const uint32_t image_index)
    {
...
private:
    ...
    )
    {
        ...
        alloc_info.sType           = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize  = mem_requirements.size;
        alloc_info.memoryTypeIndex = FindMemoryType(m_physical_device, mem_requirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
