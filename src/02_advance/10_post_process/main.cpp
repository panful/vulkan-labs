...
// 离屏帧缓存的大小
constexpr uint32_t k_off_width = 2048;
constexpr uint32_t k_off_height = 2048;
...
private:
  ...
  void PrepareImGui() {
    ...
    ImGui::Begin("Post process", nullptr, ImGuiWindowFlags_NoMove);
    {
      std::vector<const char*> filters{"Inversion", "Grayscale", "Sharpen", "Blur", "EdgeDetection", "FXAA", "Default"};
      ImGui::Combo("Filter", &m_post_index, filters.data(), static_cast<int>(filters.size()));
    }
    ImGui::End();
  ...
  void CreateImageViewsOffscreen() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(k_off_width, k_off_height, m_swap_chain_image_format, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0, m_offscreen_images[i],
                  m_offscreen_image_memorys[i]);
  ...
  void CreateGraphicsPipelineOffscreen() {
    ...
    viewport.x = 0.F;
    viewport.y = 0.F;
    viewport.width = static_cast<float>(k_off_width);
    viewport.height = static_cast<float>(k_off_height);
    viewport.minDepth = 0.F;  // 深度值范围，必须在 [0.f, 1.f]之间
    viewport.maxDepth = 1.F;
    ...
    VkRect2D scissor = {};
    scissor.offset = {0, 0};
    scissor.extent = VkExtent2D{k_off_width, k_off_height};
  ...
  void CreateFramebuffersOffscreen() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      framebuffer_info.attachmentCount = static_cast<uint32_t>(attachments.size());
      framebuffer_info.pAttachments = attachments.data();
      framebuffer_info.width = k_off_width;
      framebuffer_info.height = k_off_height;
      framebuffer_info.layers = 1;
  ...
  void InitImGui() {
    ...
    ImGui_ImplVulkan_Init(&init_info, m_render_pass_quad);
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer offscreen_fb, VkFramebuffer quad_fb) const {
    ...
    render_pass_info.renderArea.extent = VkExtent2D{k_off_width, k_off_height};
    ...
    PushConstant pc{.resolution = glm::vec2(1.F / static_cast<float>(m_swap_chain_extent.width),
                                            1.F / static_cast<float>(m_swap_chain_extent.height)),
                    .index = m_post_index};

    vkCmdPushConstants(command_buffer, m_pipeline_layout_quad, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstant),
                       &pc);
  ...
  void CreateDepthResources() {
    ...
    CreateImage(k_off_width, k_off_height, depth_format, VK_IMAGE_TILING_OPTIMAL,
