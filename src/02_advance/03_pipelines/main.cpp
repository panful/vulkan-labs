...
const std::vector<uint16_t> k_indices2{
    0, 1,
    1, 2,
    2, 3,
    3, 0,
...
private:
  ...
  void CreateLogicalDevice() {
    ...
    device_features.fillModeNonSolid = VK_TRUE;  // 开启才能使用线框（三角形）
    device_features.wideLines = VK_TRUE;         // 开启才能设置线宽
  ...
  void CreateGraphicsPipelines() {
    // 三角形填充
    CreateGraphicsPipeline(m_graphics_pipelines.at(0), VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_POLYGON_MODE_FILL);
    // 三角形不填充
    CreateGraphicsPipeline(m_graphics_pipelines.at(1), VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_POLYGON_MODE_LINE);
    // 线段
    CreateGraphicsPipeline(m_graphics_pipelines.at(2), VK_PRIMITIVE_TOPOLOGY_LINE_LIST, VK_POLYGON_MODE_LINE);
  }
  ...
  void CreateGraphicsPipeline(VkPipeline& pipeline, VkPrimitiveTopology topo, VkPolygonMode mode) {
    ...
    input_assembly.topology = topo;  // 指定绘制的图元类型：点、线、三角形
    ...
    if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline)) {
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    float width = static_cast<float>(m_swap_chain_extent.width);
    VkViewport viewport = {};
    viewport.x = 0.F;
    viewport.y = 0.F;
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipelines.at(0));
    ...
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipelines.at(1));
    vkCmdSetLineWidth(command_buffer, 5.F);  // 需要将线宽设置为动态状态才能使用该指令动态设置线宽
    vkCmdBindVertexBuffers(command_buffer, 0, 1, &m_vertex_buffer, offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
    ...
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipelines.at(2));
    vkCmdSetLineWidth(command_buffer, 3.F);  // 如果此处不设置线宽，将使用之前设置的线宽
    vkCmdBindVertexBuffers(command_buffer, 0, 1, &m_vertex_buffer2, offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer2, 0, VK_INDEX_TYPE_UINT16);
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices2.size()), 1, 0, 0, 0);
  ...
  void CreateIndexBuffer2() {
    VkDeviceSize buffer_size = sizeof(k_indices2.front()) * k_indices2.size();
    ...
    void* data{nullptr};
    vkMapMemory(m_device, staging_buffer_memory, 0, buffer_size, 0, &data);
    std::memcpy(data, k_indices2.data(), static_cast<size_t>(buffer_size));
    vkUnmapMemory(m_device, staging_buffer_memory);
...
private:
  ...
  std::array<VkPipeline, 3> m_graphics_pipelines{};
