...
private:
  ...
  void CreateImageViewsOffscreen() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(k_width, k_height, m_swap_chain_image_format, VK_IMAGE_TILING_OPTIMAL,
  ...
  void CreateGraphicsPipelineOffscreen() {
    ...
    viewport.x = 0.F;
    viewport.y = 0.F;
    viewport.width = static_cast<float>(k_width);
    viewport.height = static_cast<float>(k_height);
    viewport.minDepth = 0.F;  // 深度值范围，必须在 [0.F, 1.F]之间
    viewport.maxDepth = 1.F;
    ...
    VkRect2D scissor = {};
    scissor.offset = {0, 0};
    scissor.extent = VkExtent2D{k_width, k_height};
  ...
  void CreateFramebuffersOffscreen() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      framebuffer_info.attachmentCount = static_cast<uint32_t>(attachments.size());
      framebuffer_info.pAttachments = attachments.data();
      framebuffer_info.width = k_width;
      framebuffer_info.height = k_height;
      framebuffer_info.layers = 1;
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer offscreen_fb, VkFramebuffer quad_fb) const {
    ...
    render_pass_info.framebuffer = offscreen_fb;            // 指定使用的帧缓冲对象
    render_pass_info.renderArea.offset = {0, 0};            // 指定用于渲染的区域
    render_pass_info.renderArea.extent = VkExtent2D{k_width, k_height};
    render_pass_info.clearValueCount =
      static_cast<uint32_t>(clear_values.size());  // 指定使用 VK_ATTACHMENT_LOAD_OP_CLEAR 标记后使用的清除值
    render_pass_info.pClearValues = clear_values.data();
  ...
  void CreateTextureSampler() {
    ...
    sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_REPEAT;
    sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_REPEAT;
    // sampler_info.anisotropyEnable = VK_TRUE; // 使用各向异性过滤（需要检查是否支持）
  ...
  void CreateDepthResources() {
    ...
    CreateImage(k_width, k_height, depth_format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
