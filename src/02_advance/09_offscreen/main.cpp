...
struct VertexQuad {
  ...
  bool operator==(const VertexQuad& other) const { return pos_tex == other.pos_tex; }
  ...
  static constexpr VkVertexInputAttributeDescription GetAttributeDescription() noexcept {
    ...
    attribute_description.format = VK_FORMAT_R32G32B32A32_SFLOAT;  // vec2 + vec2
    attribute_description.offset = offsetof(VertexQuad, pos_tex);
...
struct hash<VertexQuad> {
  size_t operator()(VertexQuad const& vertex) const { return hash<glm::vec4>()(vertex.pos_tex); }
};
}  // namespace std
...
private:
  ...
  void CleanupOffscreenResouces() {
    ...
    for (int i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyFramebuffer(m_device, m_offscreen_frame_buffers[i], nullptr);
      vkDestroyImageView(m_device, m_offscreen_image_views[i], nullptr);
      vkDestroyImage(m_device, m_offscreen_images[i], nullptr);
      vkFreeMemory(m_device, m_offscreen_image_memorys[i], nullptr);
    }
  }
  ...
  void Cleanup() noexcept {
    ...
    vkDestroySampler(m_device, m_sampler_quad, nullptr);
    ...
    vkDestroyBuffer(m_device, m_offscreen_vertex_buffer, nullptr);
    vkFreeMemory(m_device, m_offscreen_vertex_buffer_memory, nullptr);
    vkDestroyBuffer(m_device, m_offscreen_index_buffer, nullptr);
    vkFreeMemory(m_device, m_offscreen_index_buffer_memory, nullptr);
    ...
    vkDestroyRenderPass(m_device, m_render_pass_quad, nullptr);

    vkDestroyPipeline(m_device, m_offscreen_graphics_pipeline, nullptr);
    vkDestroyPipelineLayout(m_device, m_offscreen_pipeline_layout, nullptr);
    vkDestroyRenderPass(m_device, m_offscreen_render_pass, nullptr);
    ...
    vkDestroyDescriptorSetLayout(m_device, m_offscreen_descriptor_set_layout, nullptr);
...
private:
  ...
  void CreateImageViews() {
    ...
    for (size_t i = 0; i < m_swap_chain_images.size(); ++i) {
      m_swap_chain_image_views.at(i) =
        CreateImageView(m_swap_chain_images.at(i), m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
    }
  }

  /// @brief 创建离屏帧缓冲的Image Memory View
  void CreateImageViewsOffscreen() {
    m_offscreen_images.resize(k_max_frames_in_flight);
    m_offscreen_image_memorys.resize(k_max_frames_in_flight);
    m_offscreen_image_views.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, m_swap_chain_image_format,
                  VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0,
                  m_offscreen_images[i], m_offscreen_image_memorys[i]);
      m_offscreen_image_views[i] =
        CreateImageView(m_offscreen_images[i], m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
    }
  }
  ...
  void CreateSamplerOffscreen() {
    ...
    if (VK_SUCCESS != vkCreateSampler(m_device, &sampler_info, nullptr, &m_sampler_quad)) {
  ...
  void CreateGraphicsPipelineQuad() {
    ...
    pipeline_info.renderPass = m_render_pass_quad;
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipeline_quad)) {
  ...
  void CreateGraphicsPipelineOffscreen() {
    ...
    pipeline_layout_info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipeline_layout_info.setLayoutCount = 1;
    pipeline_layout_info.pSetLayouts = &m_offscreen_descriptor_set_layout;
    pipeline_layout_info.pushConstantRangeCount = 0;

    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_offscreen_pipeline_layout)) {
      throw std::runtime_error("failed to create pipeline layout");
    }
    ...
    pipeline_info.pColorBlendState = &color_blending;
    pipeline_info.pDynamicState = &dynamic_state;
    pipeline_info.layout = m_offscreen_pipeline_layout;
  ...
  void CreateRenderPassOffscreen() {
    ...
    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &m_offscreen_render_pass)) {
  ...
  void CreateRenderPassQuad() {
    ...
    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &m_render_pass_quad)) {
      throw std::runtime_error("failed to create render pass");
    }
  }
  ...
  void CreateFramebuffersQuad() {
    ...
    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      ...
      framebuffer_info.renderPass = m_render_pass_quad;
  ...
  void CreateFramebuffersOffscreen() {
    m_offscreen_frame_buffers.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      // 颜色附着和深度（模板）附着
      // 和每个交换链图像对应不同的颜色附着不同，因为信号量的原因，只有一个subpass同时执行，所以一个深度附着即可
      std::array<VkImageView, 2> attachments{m_offscreen_image_views[i], m_depth_image_view};

      VkFramebufferCreateInfo framebuffer_info = {};
      framebuffer_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
      framebuffer_info.renderPass = m_offscreen_render_pass;
      framebuffer_info.attachmentCount = static_cast<uint32_t>(attachments.size());
      framebuffer_info.pAttachments = attachments.data();
      framebuffer_info.width = m_swap_chain_extent.width;
      framebuffer_info.height = m_swap_chain_extent.height;
      ...
      if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &m_offscreen_frame_buffers[i])) {
        throw std::runtime_error("failed to create framebuffer");
      }
    }
  }
  ...
  /// @brief 记录指令到指令缓冲
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer offscreen_fb, VkFramebuffer quad_fb) const {
    ...
    clear_values.at(0).color = {{1.F, 1.F, 1.F, 1.F}};  // 清除色，相当于背景色
    clear_values.at(1).depthStencil = {
      1.F, 0};  // Vulkan 的深度范围是 [0.0, 1.0] 1.0对应视锥体的远平面，初始化时应该设置为远平面的值

    VkRenderPassBeginInfo render_pass_info = {};
    render_pass_info.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    render_pass_info.renderPass = m_offscreen_render_pass;  // 指定使用的渲染流程对象
    render_pass_info.framebuffer = offscreen_fb;            // 指定使用的帧缓冲对象
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen_graphics_pipeline);
    ...
    const std::array<VkBuffer, 1> vertex_buffers{m_offscreen_vertex_buffer};
    const std::array<VkDeviceSize, 1> offsets{0};

    vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers.data(), offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_offscreen_index_buffer, 0, VK_INDEX_TYPE_UINT32);

    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen_pipeline_layout, 0, 1,
                            &m_offscreen_descriptor_sets.at(m_current_frame), 0, nullptr);
    ...
    //-----------------------------------------------------------------------
    // 渲染到屏幕
    std::array<VkClearValue, 1> clear_values_quad{};
    clear_values_quad[0].color = {{.1f, .2f, .3f, 1.F}};

    VkRenderPassBeginInfo render_pass_info_quad = {};
    render_pass_info_quad.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    render_pass_info_quad.renderPass = m_render_pass_quad;  // 指定使用的渲染流程对象
    render_pass_info_quad.framebuffer = quad_fb;            // 指定使用的帧缓冲对象
    render_pass_info_quad.renderArea.offset = {0, 0};       // 指定用于渲染的区域
    render_pass_info_quad.renderArea.extent = m_swap_chain_extent;
    render_pass_info_quad.clearValueCount =
      static_cast<uint32_t>(clear_values_quad.size());  // 指定使用 VK_ATTACHMENT_LOAD_OP_CLEAR 标记后使用的清除值
    render_pass_info_quad.pClearValues = clear_values_quad.data();

    vkCmdBeginRenderPass(command_buffer, &render_pass_info_quad, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipeline_quad);

    VkViewport viewport_quad = {};
    viewport_quad.x = 0.F;
    viewport_quad.y = 0.F;
    viewport_quad.width = static_cast<float>(m_swap_chain_extent.width);
    viewport_quad.height = static_cast<float>(m_swap_chain_extent.height);
    viewport_quad.minDepth = 0.F;
    viewport_quad.maxDepth = 1.F;

    vkCmdSetViewport(command_buffer, 0, 1, &viewport_quad);
    VkRect2D scissor_quad = {};
    scissor_quad.offset = {0, 0};
    scissor_quad.extent = m_swap_chain_extent;
    vkCmdSetScissor(command_buffer, 0, 1, &scissor_quad);
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout_quad, 0, 1,
                            &m_descriptor_sets_quad.at(m_current_frame), 0, nullptr);
  ...
  void DrawFrame() {
    ...
    RecordCommandBuffer(m_command_buffers.at(image_index), m_offscreen_frame_buffers[m_current_frame],
                        m_swap_chain_framebuffers.at(image_index));
  ...
  void CreateVertexBufferOffscreen() {
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_offscreen_vertex_buffer, m_offscreen_vertex_buffer_memory);

    // m_vertexBuffer 现在关联的内存是设备所有的（显卡），不能使用 vkMapMemory 函数对它关联的内存进行映射
    // 从临时（暂存）缓冲复制数据到显卡读取较快的缓冲中
    CopyBuffer(staging_buffer, m_offscreen_vertex_buffer, buffer_size);
  ...
  void CreateIndexBufferOffscreen() {
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_offscreen_index_buffer, m_offscreen_index_buffer_memory);

    CopyBuffer(staging_buffer, m_offscreen_index_buffer, buffer_size);
  ...
  void CreateVertexBufferQuad() {
    ...
    // 具有 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT 标记的内存最适合显卡读取，CPU通常不能访问这种类型的内存
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_vertex_buffer_quad, m_vertex_buffer_memory_quad);
  ...
  void CreateIndexBufferQuad() {
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_index_buffer_quad, m_index_buffer_memory_quad);
  ...
  void CreateDescriptorSetLayoutOffscreen() {
    ...
    if (VK_SUCCESS !=
        vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_offscreen_descriptor_set_layout)) {
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes.at(1).descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2);  // offscreen texture

    VkDescriptorPoolCreateInfo pool_info{};
    pool_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    pool_info.poolSizeCount = static_cast<uint32_t>(pool_sizes.size());
    pool_info.pPoolSizes = pool_sizes.data();
    pool_info.maxSets = static_cast<uint32_t>(k_max_frames_in_flight * 2);  // 指定可以分配的最大描述符集个数
  ...
  void CreateDescriptorSetsOffscreen() {
    ...
    std::vector<VkDescriptorSetLayout> layouts(k_max_frames_in_flight, m_offscreen_descriptor_set_layout);
    ...
    m_offscreen_descriptor_sets.resize(k_max_frames_in_flight);
    if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_offscreen_descriptor_sets.data())) {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      descriptor_writes.at(0).dstSet = m_offscreen_descriptor_sets.at(i);  // 指定要更新的描述符集对象
      ...
      descriptor_writes.at(1).dstSet = m_offscreen_descriptor_sets.at(i);
  ...
  void UpdateDescriptorSetsQuad() {
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      VkDescriptorImageInfo image_info{};
      image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
      image_info.imageView = m_offscreen_image_views[i];
      image_info.sampler = m_sampler_quad;
      ...
      descriptor_writes[0].dstBinding = 3;  // 绑定点
  ...
  void CreateTextureImage() {
    ...
    TransitionImageLayout(m_texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
private:
  ...
  VkRenderPass m_render_pass_quad{nullptr};
  ...
  std::vector<VkImage> m_offscreen_images{};
  std::vector<VkDeviceMemory> m_offscreen_image_memorys{};
  std::vector<VkImageView> m_offscreen_image_views{};
  std::vector<VkFramebuffer> m_offscreen_frame_buffers{};
  VkRenderPass m_offscreen_render_pass{nullptr};
  VkPipelineLayout m_offscreen_pipeline_layout{nullptr};
  VkPipeline m_offscreen_graphics_pipeline{nullptr};
  VkSampler m_sampler_quad{nullptr};
  VkDescriptorSetLayout m_offscreen_descriptor_set_layout{nullptr};
  std::vector<VkDescriptorSet> m_offscreen_descriptor_sets{};
  VkBuffer m_offscreen_vertex_buffer{nullptr};
  VkDeviceMemory m_offscreen_vertex_buffer_memory{nullptr};
  VkBuffer m_offscreen_index_buffer{nullptr};
  VkDeviceMemory m_offscreen_index_buffer_memory{nullptr};
};
...
private:
  ...
  void CreateImageViewsOffscreen() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
      CreateImage(WIDTH, HEIGHT, m_swapChainImageFormat, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0, m_offscreenImages[i],
                  m_offscreenImageMemorys[i]);
      m_offscreenImageViews[i] =
        CreateImageView(m_offscreenImages[i], m_swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
    }
  ...
  /// @brief 创建屏幕上显示的图形管线
  void CreateGraphicsPipelineQuad() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_09_quad_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_09_quad_frag.spv");
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_graphicsPipelineQuad)) {
      throw std::runtime_error("failed to create graphics pipeline");
    }
  ...
  void CreateGraphicsPipelineOffscreen() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_09_base_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_09_base_frag.spv");
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_offscreenGraphicsPipeline)) {
      throw std::runtime_error("failed to create graphics pipeline");
    }
  ...
  void CreateRenderPassOffscreen() {
    ...
    subpass.pDepthStencilAttachment =
      &depthAttachmentRef;  // 指定深度模板附着，深度模板附着只能是一个，所以不用设置数量
  ...
  /// @brief 记录指令到指令缓冲
  void RecordCommandBuffer(VkCommandBuffer commandBuffer, VkFramebuffer offscreenFB, VkFramebuffer quadFB) const {
    ...
    renderPassInfo.clearValueCount =
      static_cast<uint32_t>(clearValues.size());  // 指定使用 VK_ATTACHMENT_LOAD_OP_CLEAR 标记后使用的清除值
    ...
    const std::array<VkBuffer, 1> vertexBuffers{m_offscreenVertexBuffer};
    const std::array<VkDeviceSize, 1> offsets{0};
    ...
    renderPassInfoQuad.clearValueCount =
      static_cast<uint32_t>(clearValuesQuad.size());  // 指定使用 VK_ATTACHMENT_LOAD_OP_CLEAR 标记后使用的清除值
    ...
    const std::array<VkBuffer, 1> vertexBuffersQuad{m_vertexBufferQuad};

    vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffersQuad.data(), offsets.data());
    vkCmdBindIndexBuffer(commandBuffer, m_indexBufferQuad, 0, VK_INDEX_TYPE_UINT32);
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayoutQuad, 0, 1,
                            &m_descriptorSetsQuad.at(m_currentFrame), 0, nullptr);
  ...
  void DrawFrame() {
    ...
    RecordCommandBuffer(m_commandBuffers.at(imageIndex), m_offscreenFrameBuffers[m_currentFrame],
                        m_swapChainFramebuffers.at(imageIndex));
  ...
  void CreateVertexBufferOffscreen() {
    ...
    CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_offscreenVertexBuffer, m_offscreenVertexBufferMemory);
  ...
  void CreateIndexBufferOffscreen() {
    ...
    CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_offscreenIndexBuffer, m_offscreenIndexBufferMemory);
  ...
  void CreateVertexBufferQuad() {
    ...
    // 具有 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT 标记的内存最适合显卡读取，CPU通常不能访问这种类型的内存
    CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_vertexBufferQuad, m_vertexBufferMemoryQuad);
  ...
  void CreateIndexBufferQuad() {
    ...
    CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_indexBufferQuad, m_indexBufferMemoryQuad);
  ...
  void CreateTextureImage() {
    ...
    auto pixels = stbi_load(PROJECT_ASSETS_DIR "models/viking_room/viking_room.png", &texWidth, &texHeight,
