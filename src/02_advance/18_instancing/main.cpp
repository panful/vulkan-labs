...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_18_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_18_base_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 5, 0, 0, 0);
...
struct InstanceData {
  ...
  static constexpr VkVertexInputBindingDescription GetBindingDescription() noexcept {
    ...
    binding_description.inputRate = VK_VERTEX_INPUT_RATE_INSTANCE;  // 每个渲染实例读取一次顶点属性
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_18_base2_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_18_base2_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    // 3.实例的个数，没有使用实例渲染则设置为1
    // 6.第一个实例的ID，着色器中 gl_InstanceIndex 起始的值
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()),
                     static_cast<uint32_t>(k_instance_data.size()), 0, 0, 0);
