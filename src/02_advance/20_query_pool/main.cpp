...
private:
  ...
  void PrepareImGui() {
    ...
    ImGui::Text("Cube passed samples: %llu", m_passed_samples[0]);
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
    ImGui::Text("Tetra passed samples: %llu", m_passed_samples[1]);
    ImGui::End();
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_SHADER_DIR "02_20_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_SHADER_DIR "02_20_base_frag.spv");
  ...
  void InitImGui() {
    ...
    io.DisplaySize = ImVec2(static_cast<float>(k_width), static_cast<float>(k_height));
    io.Fonts->Build();
  ...
  void GetQueryResult() {
    ...
    // 7. 每个查询结果之间的字节跨度
    // 8. 结果的返回方式和时机
    vkGetQueryPoolResults(m_device, m_query_pool, 0, 2, sizeof(m_passed_samples), m_passed_samples.data(),
  ...
  void CreateQueryPool() {
    ...
    query_pool_info.queryType = VK_QUERY_TYPE_OCCLUSION;  // 查询类型，包括：遮挡、管线统计、时间戳等
    query_pool_info.queryCount = 2;                       // 查询池中查询的数量
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdBeginQuery(command_buffer, m_query_pool, 1, 0);
    {
      // 绘制四面体
      const std::array<VkBuffer, 1> vertex_buffers_tetra{m_drawable_tetra.vertex_buffer};
      vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers_tetra.data(), offsets.data());
      vkCmdBindIndexBuffer(command_buffer, m_drawable_tetra.index_buffer, 0, VK_INDEX_TYPE_UINT16);
      vkCmdDrawIndexed(command_buffer, m_drawable_tetra.index_count, 1, 0, 0, 0);
    }
    vkCmdEndQuery(command_buffer, m_query_pool, 1);
...
private:
  ...
  std::array<uint64_t, 2> m_passed_samples{};
...
private:
  ...
  void PrepareImGui() {
    ...
    ImGui::TextUnformatted(
      ("INPUT_ASSEMBLY_VERTICES: " + std::to_string(m_pipeline_stats[0])).c_str());  // 输入装配阶段处理的顶点个数
    ImGui::TextUnformatted(
      ("INPUT_ASSEMBLY_PRIMITIVES: " + std::to_string(m_pipeline_stats[1])).c_str());  // 输入装配阶段处理的原始图形数量
    ImGui::TextUnformatted(
      ("VERTEX_SHADER_INVOCATIONS: " + std::to_string(m_pipeline_stats[2])).c_str());  // 顶点着色器的调用次数
    ImGui::TextUnformatted(
      ("CLIPPING_INVOCATIONS: " + std::to_string(m_pipeline_stats[3])).c_str());  // 裁剪阶段处理的原始图形数量
    ImGui::TextUnformatted(
      ("CLIPPING_PRIMITIVES: " + std::to_string(m_pipeline_stats[4])).c_str());  // 裁剪阶段输出的原始图形数量
    ImGui::TextUnformatted(
      ("FRAGMENT_SHADER_INVOCATIONS: " + std::to_string(m_pipeline_stats[5])).c_str());  // 片段着色器的调用次数
    ImGui::End();
  ...
  void GetQueryResult() {
    ...
    // 7. 每个查询结果之间的字节跨度
    // 8. 结果的返回方式和时机
    vkGetQueryPoolResults(m_device, m_query_pool, 0, 1, sizeof(m_pipeline_stats), m_pipeline_stats.data(),
