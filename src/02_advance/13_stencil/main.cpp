...
const std::vector<Vertex> k_vertices2 {
...
private:
  ...
  void Cleanup() noexcept {
    ...
    vkDestroyPipeline(m_device, m_graphics_pipeline_small, nullptr);
    vkDestroyPipeline(m_device, m_graphics_pipeline_large, nullptr);
    vkDestroyPipelineLayout(m_device, m_pipeline_layout_small, nullptr);
    vkDestroyPipelineLayout(m_device, m_pipeline_layout_large, nullptr);
...
private:
  ...
  void CreateGraphicsPipelineSmall() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_13_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_13_base_frag.spv");
    ...
    stencil_op.reference = 1;                     // 参考值，用于和模板缓冲中的值比较
    stencil_op.compareMask = 0xFF;                // 比较掩码值，掩码中那些位的值为1则比较那些值，0xFF表示所有位都比较
    stencil_op.writeMask = 0xFF;                  // 是否写入模板缓冲，模板值8位，0xFF表示所有的位
    stencil_op.compareOp = VK_COMPARE_OP_NEVER;   // 模板测试使用的比较方法
    stencil_op.failOp = VK_STENCIL_OP_REPLACE;    // 模板测试不通过执行的操作
    stencil_op.depthFailOp = VK_STENCIL_OP_KEEP;  // 模板测试通过，深度测试不通过执行的操作
    stencil_op.passOp = VK_STENCIL_OP_KEEP;       // 深度测试和模板测试都通过执行的操作
    ...
    depth_stencil.stencilTestEnable = VK_TRUE;  // 开启模板测试
    depth_stencil.front = stencil_op;           // 正面模板测试相应参数
    depth_stencil.back = stencil_op;            // 背面模板测试相应参数
    ...
    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_pipeline_layout_small)) {
    ...
    pipeline_info.layout = m_pipeline_layout_small;
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipeline_small)) {
  ...
  void CreateGraphicsPipelineLarge() {
    ...
    VkStencilOpState stencil_op = {};
    stencil_op.reference = 1;                        // 参考值，用于和模板缓冲中的值比较
    stencil_op.compareMask = 0xFF;                   // 模板的8位值都进行比较
    stencil_op.writeMask = 0x00;                     // 只用参考值和模板缓冲中的值比较，不写入到模板缓冲，即模板缓冲只读
    stencil_op.compareOp = VK_COMPARE_OP_NOT_EQUAL;  // 模板测试使用的比较方法
    stencil_op.failOp = VK_STENCIL_OP_REPLACE;       // 因为writeMask设置为0xFF，所以以下三个值都没用
    ...
    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_pipeline_layout_large)) {
      throw std::runtime_error("failed to create pipeline layout");
    }
    ...
    pipeline_info.pColorBlendState = &color_blending;
    pipeline_info.pDynamicState = &dynamic_state;
    pipeline_info.layout = m_pipeline_layout_large;
    pipeline_info.renderPass = m_render_pass;
    pipeline_info.subpass = 0;                   // 子流程在子流程数组中的索引
    pipeline_info.basePipelineHandle = nullptr;  // 以一个创建好的图形管线为基础创建一个新的图形管线
    pipeline_info.basePipelineIndex = -1;  // 只有该结构体的成员 flags 被设置为 VK_PIPELINE_CREATE_DERIVATIVE_BIT 才有效
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipeline_large)) {
      throw std::runtime_error("failed to create graphics pipeline");
    }
  ...
  void CreateRenderPass() {
    ...
    subpass.pDepthStencilAttachment = &depth_attachment_ref;  // 指定深度模板附着
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipeline_small);
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipeline_large);
    vkCmdBindVertexBuffers(command_buffer, 0, 1, &m_vertex_buffer2, offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
  ...
  void CreateVertexBuffer2() {
    VkDeviceSize buffer_size = sizeof(Vertex) * k_vertices2.size();
    ...
    vkMapMemory(m_device, staging_buffer_memory, 0, buffer_size, 0, &data);
    // 将顶点数据复制到映射后的内存
    std::memcpy(data, k_vertices2.data(), static_cast<size_t>(buffer_size));
    // 结束内存映射
    vkUnmapMemory(m_device, staging_buffer_memory);
    ...
    // 具有 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT 标记的内存最适合显卡读取，CPU通常不能访问这种类型的内存
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_vertex_buffer2, m_vertex_buffer_memory2);

    // m_vertexBuffer 现在关联的内存是设备所有的（显卡），不能使用 vkMapMemory 函数对它关联的内存进行映射
    // 从临时（暂存）缓冲复制数据到显卡读取较快的缓冲中
    CopyBuffer(staging_buffer, m_vertex_buffer2, buffer_size);
  ...
  void CreateIndexBuffer2() {
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_index_buffer2, m_index_buffer_memory2);

    CopyBuffer(staging_buffer, m_index_buffer2, buffer_size);
  ...
  void CreateDepthResources() {
    ...
    m_depth_image_view =
      CreateImageView(m_depth_image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT);
  ...
  VkFormat FindDepthFormat() const {
    ...
    return FindSupportedFormat({VK_FORMAT_D32_SFLOAT_S8_UINT}, VK_IMAGE_TILING_OPTIMAL,
...
private:
  ...
  VkPipelineLayout m_pipeline_layout_small{nullptr};
  VkPipelineLayout m_pipeline_layout_large{nullptr};

  VkPipeline m_graphics_pipeline_small{nullptr};
  VkPipeline m_graphics_pipeline_large{nullptr};
  ...
  VkBuffer m_vertex_buffer2{nullptr};
  VkDeviceMemory m_vertex_buffer_memory2{nullptr};
  VkBuffer m_index_buffer2{nullptr};
  VkDeviceMemory m_index_buffer_memory2{nullptr};
