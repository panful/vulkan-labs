...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base_frag.spv");
  ...
  void ThreadRender(size_t threadId, size_t primCmdBufferId, size_t drawableId,
    ...
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, &m_drawables[drawableId].vertexBuffer, offsets.data());
  ...
  void RecordCommandBuffer(size_t swapChainId) const {
    ...
    std::vector<VkCommandBuffer> commandBuffers{m_threadDatas[swapChainId][0].commandBuffer,
                                                m_threadDatas[swapChainId][1].commandBuffer};
    vkCmdExecuteCommands(m_commandBuffers[swapChainId], static_cast<uint32_t>(commandBuffers.size()),
                         commandBuffers.data());
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base2_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base2_frag.spv");
