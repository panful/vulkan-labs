...
struct VertexQuad {
  ...
  bool operator==(const VertexQuad& other) const { return pos == other.pos; }
...
struct hash<VertexQuad> {
  size_t operator()(VertexQuad const& vertex) const { return hash<glm::vec2>()(vertex.pos); }
};
}  // namespace std
...
private:
  ...
  void CreateAttachmentImpl(VkFormat format, VkImageUsageFlags usage, VkImage& image, VkDeviceMemory& memory,
                            VkImageView& view) {
    ...
    CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, format, VK_IMAGE_TILING_OPTIMAL, usage,
                /*not use*/ 0, image, memory, true);
  ...
  void CreateAttachments(bool first = false) {
    ...
    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      CreateAttachmentImpl(m_swap_chain_image_format, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, m_attachment_image_color[i],
                           m_attachment_memory_color[i], m_attachment_view_color[i]);

      CreateAttachmentImpl(VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                           m_attachment_image_normal[i], m_attachment_memory_normal[i], m_attachment_view_normal[i]);

      CreateAttachmentImpl(VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                           m_attachment_image_position[i], m_attachment_memory_position[i],
                           m_attachment_view_position[i]);

      CreateAttachmentImpl(FindDepthFormat(), VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, m_attachment_image_depth[i],
                           m_attachment_memory_depth[i], m_attachment_view_depth[i]);

      // 转换图像布局，不然缩放窗口的时候会报错：布局不匹配
      // 程序第一次启动的时候并不会报错，具体原因不清楚
      if (!first) {
        TransitionImageLayout(m_attachment_image_color[i], m_swap_chain_image_format, VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
        TransitionImageLayout(m_attachment_image_normal[i], VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
        TransitionImageLayout(m_attachment_image_position[i], VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
        TransitionImageLayout(m_attachment_image_depth[i], FindDepthFormat(), VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  void CreateRenderPass() {
    ...
    std::array<VkAttachmentDescription, 5> attachments{swap_chain_attachment, color_attachment, normal_attachment,
                                                       position_attachment, depth_attachment};
  ...
  void CreateFramebuffers() {
    ...
    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      ...
      std::array<VkImageView, 5> attachments{m_swap_chain_image_views[i], m_attachment_view_color[i],
                                             m_attachment_view_normal[i], m_attachment_view_position[i],
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    clear_values.at(1).color = {{0.F, 0.F, 0.F, 0.F}};
    clear_values.at(2).color = {{0.F, 0.F, 0.F, 0.F}};
    clear_values.at(3).color = {{0.F, 0.F, 0.F, 0.F}};
    clear_values.at(4).depthStencil = {
    ...
    vkCmdPushConstants(command_buffer, m_pipeline_layout_quad, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(int),
                       &m_attachment_index);
  ...
  void CreateUniformBufferLight() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                   ...
                   m_uniform_buffer_light.at(i), m_uniform_buffer_light_memory.at(i));
      vkMapMemory(m_device, m_uniform_buffer_light_memory.at(i), 0, buffer_size, 0,
                  &m_uniform_buffer_light_mapped.at(i));
    }
  }
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes[2].descriptorCount =
      static_cast<uint32_t>(k_max_frames_in_flight * 4);  // color、normal、position、depth
