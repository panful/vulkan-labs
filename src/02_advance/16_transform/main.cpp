...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_base_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_base_frag.spv");
...
private:
  ...
  void RecordCommandBuffer(VkCommandBuffer commandBuffer, VkFramebuffer framebuffer) const {
    ...
    auto drawDrawable = [commandBuffer](const Drawable& drawable) {
      const std::array<VkBuffer, 1> vertexBuffers{drawable.vertexBuffer};
      const std::array<VkDeviceSize, 1> offsets{0};
...
private:
  void CreateDrawables() {
    ...
    CreateTexture(m_texturePlane, PROJECT_ASSETS_DIR "textures/nightsky.png");
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_texture_vert.spv");
    auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_texture_frag.spv");
