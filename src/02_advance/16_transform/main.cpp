...
private:
    ...
    void CreateImageDatasOffscreen(Offscreen& offscreen)
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            offscreen.colors[i].imageView = CreateImageView(offscreen.colors[i].image, m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
        }
    ...
    void CreateGraphicsPipelineOffscreen(Offscreen& offscreen)
    {
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &offscreen.pipeline))
        {
            throw std::runtime_error("failed to create graphics pipeline");
        }
    ...
    void CreateUniformBuffersOffscreen(Offscreen& offscreen)
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            vkMapMemory(m_device, offscreen.uniformBuffersMemory.at(i), 0, buffer_size, 0, &offscreen.uniformBuffersMapped.at(i));
        }
    }
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        vkCmdBindDescriptorSets(
            ...
            0,
            1,
            &m_offscreen.descriptorSets[m_current_frame],
            0,
            nullptr
        );
