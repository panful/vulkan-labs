...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_base_frag.spv");
...
private:
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    auto draw_drawable = [command_buffer](const Drawable& drawable) {
      const std::array<VkBuffer, 1> vertex_buffers{drawable.vertexBuffer};
      const std::array<VkDeviceSize, 1> offsets{0};
...
private:
  ...
  void Cleanup() noexcept {
    ...
    auto destroy_texture = [this](Texture& texture) {
    ...
    destroy_drawable(m_drawable_plane);
    destroy_texture(m_texture_plane);
    vkDestroySampler(m_device, m_sampler_plane, nullptr);
...
private:
  void CreateDrawables() {
    ...
    CreateTexture(m_texture_plane, PROJECT_ASSETS_DIR "textures/nightsky.png");
    CreateSampler(m_sampler_plane);
  }
  ...
  void CreateTexture(Texture& texture, const std::string& file_name) {
    // STBI_rgb_alpha 强制使用alpha通道，如果没有会被添加一个默认的alpha值，texChannels返回图像实际的通道数
    int tex_width{0};
    int tex_height{0};
    int tex_channels{0};
    auto pixels = stbi_load(file_name.c_str(), &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
    if (!pixels) {
    ...
    texture.image_view = CreateImageView(texture.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
  }
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_texture_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_texture_frag.spv");
  ...
  void CreateDescriptorSets() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      image_info.imageView = m_texture_plane.image_view;
      image_info.sampler = m_sampler_plane;
      ...
      descriptor_writes[1].dstSet = m_descriptor_sets.at(i);
...
private:
  ...
  Drawable m_drawable_plane{};
  Texture m_texture_plane{};
  VkSampler m_sampler_plane{};
...
private:
  ...
  void CreateImageDatasOffscreen(Offscreen& offscreen) {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, m_swap_chain_image_format,
                  ...
                  offscreen.colors[i].image, offscreen.colors[i].memory);

      offscreen.colors[i].imageView =
        CreateImageView(offscreen.colors[i].image, m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
    }
    ...
    offscreen.depth.imageView = CreateImageView(offscreen.depth.image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT);
  }
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_offscreen_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_16_offscreen_frag.spv");
  ...
  void UpdateDescriptorSets() {
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      VkDescriptorImageInfo image_info{};
      image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
      image_info.imageView = m_offscreen.colors[i].imageView;
