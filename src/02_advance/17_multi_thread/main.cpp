...
// clang-format off
const std::vector<Vertex> k_vertices_0 {
    {{-0.8F,  0.8F}, {1.F, 0.F, 0.F}},
    {{-0.2F,  0.8F}, {1.F, 0.F, 0.F}},
    {{-0.5F, -0.5F}, {1.F, 0.F, 0.F}},
};

const std::vector<Vertex> k_vertices_1 {
...
private:
  ...
  void Cleanup() noexcept {
    ...
    for (auto& thread_data : m_thread_datas) {
      for (auto& per_thread : thread_data) {
        vkDestroyCommandPool(m_device, per_thread.command_pool, nullptr);
      }
    }
...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base_frag.spv");
    ...
    rasterizer_state.rasterizerDiscardEnable = VK_FALSE;  // 设置为true会禁止一切片段输出到帧缓冲
    rasterizer_state.polygonMode = VK_POLYGON_MODE_FILL;
    rasterizer_state.lineWidth = 1.F;
    rasterizer_state.cullMode = VK_CULL_MODE_BACK_BIT;             // 表面剔除类型，正面、背面、双面剔除
    rasterizer_state.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;  // 指定顺时针的顶点序是正面还是反面
    rasterizer_state.depthBiasEnable = VK_FALSE;
    rasterizer_state.depthBiasConstantFactor = 1.F;
    rasterizer_state.depthBiasClamp = 0.F;
    rasterizer_state.depthBiasSlopeFactor = 0.F;
    ...
    multisampling_state.minSampleShading = 1.F;
  ...
  void CreateSecondaryCommandBuffer() {
    ...
    m_thread_datas.resize(m_command_buffers.size());  // 每个交换链都有一个commandbuffer（主要commandbuffer）
    for (auto& thread_data : m_thread_datas) {
      thread_data.resize(2);  // 两个线程，每个线程绘制一个三角形
      for (auto& per_thread : thread_data) {
        VkCommandPoolCreateInfo pool_info = {};
        pool_info.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
        pool_info.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
        ...
        if (VK_SUCCESS != vkCreateCommandPool(m_device, &pool_info, nullptr, &per_thread.command_pool)) {
          throw std::runtime_error("failed to create command pool");
        }

        VkCommandBufferAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        alloc_info.commandPool = per_thread.command_pool;
        alloc_info.level = VK_COMMAND_BUFFER_LEVEL_SECONDARY;  // 指定是主要还是辅助指令缓冲对象
        alloc_info.commandBufferCount = 1;

        if (VK_SUCCESS != vkAllocateCommandBuffers(m_device, &alloc_info, &per_thread.command_buffer)) {
          throw std::runtime_error("failed to allocate command buffers");
        }
      }
    }
  ...
  /// @brief 线程执行的任务
  /// @param thread_id
  /// @param prim_cmd_buffer_id 主要command_buffer的id
  /// @param drawable_id  绘制对象的id
  /// @param inheritance_info 指定辅助cmdBuf从主要cmdBuf继承的状态
  void ThreadRender(size_t thread_id, size_t prim_cmd_buffer_id, size_t drawable_id,
                    VkCommandBufferInheritanceInfo inheritance_info) const {
    VkCommandBuffer command_buffer = m_thread_datas[prim_cmd_buffer_id][thread_id].command_buffer;
    ...
    vkCmdBindVertexBuffers(command_buffer, 0, 1, &m_drawables[drawable_id].vertex_buffer, offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_drawables[drawable_id].index_buffer, 0, VK_INDEX_TYPE_UINT16);
    vkCmdDrawIndexed(command_buffer, m_drawables[drawable_id].index_count, 1, 0, 0, 0);
  ...
  /// @brief 记录指令到指令缓冲
  void RecordCommandBuffer(size_t swap_chain_id) const {
    // 此处不需要指定主要command_buffer的用途
    VkCommandBufferBeginInfo begin_info = {};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

    if (VK_SUCCESS != vkBeginCommandBuffer(m_command_buffers[swap_chain_id], &begin_info)) {
      throw std::runtime_error("failed to begin recording command buffer");
    }
    ...
    render_pass_info.framebuffer = m_swap_chain_framebuffers[swap_chain_id];
    render_pass_info.renderArea.offset = {0, 0};
    render_pass_info.renderArea.extent = m_swap_chain_extent;
    render_pass_info.clearValueCount = 1;
    render_pass_info.pClearValues = &clear_color;

    vkCmdBeginRenderPass(m_command_buffers[swap_chain_id], &render_pass_info,
    ...
    inheritance_info.framebuffer = m_swap_chain_framebuffers[swap_chain_id];

    // 多线程创建多个辅助command_buffer，每个辅助cmdBuf负责绘制一个三角形
    // 这里可以将两个任务放到线程池去执行，避免每帧都创建销毁线程
    std::thread t1(&HelloTriangleApplication::ThreadRender, this, 0, swap_chain_id, 0, inheritance_info);
    std::thread t2(&HelloTriangleApplication::ThreadRender, this, 1, swap_chain_id, 1, inheritance_info);
    t1.join();
    t2.join();

    // 在主要cmdBuf中提交辅助cmdBuf
    std::vector<VkCommandBuffer> command_buffers{m_thread_datas[swap_chain_id][0].command_buffer,
                                                 m_thread_datas[swap_chain_id][1].command_buffer};
    vkCmdExecuteCommands(m_command_buffers[swap_chain_id], static_cast<uint32_t>(command_buffers.size()),
                         command_buffers.data());

    vkCmdEndRenderPass(m_command_buffers[swap_chain_id]);
    if (VK_SUCCESS != vkEndCommandBuffer(m_command_buffers[swap_chain_id])) {
      throw std::runtime_error("failed to record command buffer");
    }
  }
  ...
  void CreateDrawables() {
    ...
    m_drawables[0].index_count = static_cast<uint32_t>(k_indices.size());
    CreateVertexBuffer(k_vertices_0, m_drawables[0].vertex_buffer, m_drawables[0].vertex_buffer_memory);
    CreateIndexBuffer(k_indices, m_drawables[0].index_buffer, m_drawables[0].index_buffer_memory);

    m_drawables[1].index_count = static_cast<uint32_t>(k_indices.size());
    CreateVertexBuffer(k_vertices_1, m_drawables[1].vertex_buffer, m_drawables[1].vertex_buffer_memory);
    CreateIndexBuffer(k_indices, m_drawables[1].index_buffer, m_drawables[1].index_buffer_memory);
  }

  /// @brief 创建顶点缓冲
  void CreateVertexBuffer(const std::vector<Vertex>& vert, VkBuffer& vertex_buffer,
                          VkDeviceMemory& vertex_buffer_memory) {
    VkDeviceSize buffer_size = sizeof(Vertex) * vert.size();
    ...
    // 将顶点数据复制到映射后的内存
    std::memcpy(data, vert.data(), static_cast<size_t>(buffer_size));
    // 结束内存映射
  ...
  /// @brief 创建索引缓冲
  void CreateIndexBuffer(const std::vector<uint16_t>& ind, VkBuffer& index_buffer,
                         VkDeviceMemory& index_buffer_memory) {
    VkDeviceSize buffer_size = sizeof(ind.front()) * ind.size();
    ...
    void* data{nullptr};
    vkMapMemory(m_device, staging_buffer_memory, 0, buffer_size, 0, &data);
    std::memcpy(data, ind.data(), static_cast<size_t>(buffer_size));
    vkUnmapMemory(m_device, staging_buffer_memory);
...
private:
  ...
  std::vector<std::vector<ThreadData>> m_thread_datas{};
...
struct Drawable {
  VkBuffer vertex_buffer{nullptr};
  VkDeviceMemory vertex_bufferMemory{nullptr};
  VkBuffer index_buffer{nullptr};
  VkDeviceMemory index_bufferMemory{nullptr};
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base2_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_17_base2_frag.spv");
  ...
  void DestroyDrawable(Drawable& drawable) noexcept {
    vkDestroyBuffer(m_device, drawable.vertex_buffer, nullptr);
    vkFreeMemory(m_device, drawable.vertex_bufferMemory, nullptr);
    vkDestroyBuffer(m_device, drawable.index_buffer, nullptr);
    vkFreeMemory(m_device, drawable.index_bufferMemory, nullptr);
  }
  ...
  void CreateVertexBuffer(const std::vector<Vertex>& vertices, Drawable& drawable) {
    ...
    // 具有 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT 标记的内存最适合显卡读取，CPU通常不能访问这种类型的内存
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, drawable.vertex_buffer, drawable.vertex_bufferMemory);
  ...
  void CreateIndexBuffer(const std::vector<uint16_t>& indices, Drawable& drawable) {
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, drawable.index_buffer, drawable.index_bufferMemory);
...
private:
  ...
  void DrawFrame() {
    ...
    std::thread([this, image_index]() {
      std::cout << "record thread id: " << std::this_thread::get_id() << '\n';
      RecordCommandBuffer(m_command_buffers.at(image_index), m_swap_chain_framebuffers.at(image_index));
    }).join();
...
private:
  ...
  void DrawFrame() {
    ...
    std::thread([this, &submit_info]() {
      std::cout << "submit thread id: " << std::this_thread::get_id() << '\n';
      if (VK_SUCCESS != vkQueueSubmit(m_graphics_queue, 1, &submit_info, m_in_flight_fences.at(m_current_frame))) {
        throw std::runtime_error("failed to submit draw command buffer");
      }
    }).join();
