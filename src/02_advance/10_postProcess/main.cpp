...
private:
    ...
    void CreateImageViewsOffscreen()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            CreateImage(
                ...
                0,
                m_offscreen_images[i],
                m_offscreen_image_memorys[i]
            );
            m_offscreen_image_views[i] = CreateImageView(m_offscreen_images[i], m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
        }
    ...
    void CreateGraphicsPipelineOffscreen()
    {
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_offscreen_graphics_pipeline))
        {
            throw std::runtime_error("failed to create graphics pipeline");
        }
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer offscreen_fb, VkFramebuffer quad_fb) const
    {
        ...
        vkCmdBindDescriptorSets(
            ...
            0,
            1,
            &m_offscreen_descriptor_sets.at(m_current_frame),
            0,
            nullptr
        );
        ...
        PushConstant pc {
            .resolution = glm::vec2(1.F / static_cast<float>(m_swap_chain_extent.width), 1.F / static_cast<float>(m_swap_chain_extent.height)),
            .index      = m_post_index
        };

        vkCmdPushConstants(command_buffer, m_pipeline_layout_quad, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstant), &pc);
    ...
    void DrawFrame()
    {
        ...
        RecordCommandBuffer(m_command_buffers.at(image_index), m_offscreen_frame_buffers[m_current_frame], m_swap_chain_framebuffers.at(image_index));
    ...
    void CreateVertexBufferOffscreen()
    {
        ...
        CreateBuffer(
            ...
            m_offscreen_vertex_buffer_memory
        );
    ...
    void CreateIndexBufferOffscreen()
    {
        ...
        CreateBuffer(
            ...
            m_offscreen_index_buffer_memory
        );
    ...
    void CreateDescriptorSetLayoutOffscreen()
    {
        ...
        if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_offscreen_descriptor_set_layout))
        {
    ...
    void CreateTextureImage()
    {
        ...
        TransitionImageLayout(
            m_texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
        );
