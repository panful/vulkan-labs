...
constexpr uint32_t k_vertex_buffer_bind_id{0};
constexpr uint32_t k_instance_buffer_bind_id{1};
...
struct Vertex {
  ...
  static constexpr VkVertexInputBindingDescription GetBindingDescription() noexcept {
    ...
    binding_description.binding = k_vertex_buffer_bind_id;
  ...
  static constexpr std::array<VkVertexInputAttributeDescription, 2> GetAttributeDescriptions() noexcept {
    ...
    attribute_descriptions[0].binding = k_vertex_buffer_bind_id;
    attribute_descriptions[0].location = 0;
    attribute_descriptions[0].format = VK_FORMAT_R32G32_SFLOAT;
    attribute_descriptions[0].offset = offsetof(Vertex, pos);

    attribute_descriptions[1].binding = k_vertex_buffer_bind_id;
...
struct InstanceData {
  ...
  static constexpr VkVertexInputBindingDescription GetBindingDescription() noexcept {
    ...
    binding_description.binding = k_instance_buffer_bind_id;
  ...
  static constexpr std::array<VkVertexInputAttributeDescription, 2> GetAttributeDescriptions() noexcept {
    ...
    attribute_descriptions[0].binding = k_instance_buffer_bind_id;
    attribute_descriptions[0].location = 2;
    attribute_descriptions[0].format = VK_FORMAT_R32G32B32_SFLOAT;
    attribute_descriptions[0].offset = offsetof(InstanceData, color);

    attribute_descriptions[1].binding = k_instance_buffer_bind_id;
    attribute_descriptions[1].location = 3;
    attribute_descriptions[1].format = VK_FORMAT_R32G32_SFLOAT;
    attribute_descriptions[1].offset = offsetof(InstanceData, offset);
...
const std::vector<InstanceData> k_instance_data {
...
private:
  ...
  void Cleanup() noexcept {
    ...
    vkDestroyBuffer(m_device, m_indirect_buffer, nullptr);
    vkFreeMemory(m_device, m_indirect_buffer_memory, nullptr);
...
private:
  ...
  bool IsDeviceSuitable(VkPhysicalDevice device) noexcept {
    ...
    m_support_multi_draw_indirect = device_features.multiDrawIndirect;  // GPU是否支持多次间接绘制
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_19_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_19_base_frag.spv");
    ...
    std::vector binding_descriptions{Vertex::GetBindingDescription(), InstanceData::GetBindingDescription()};
    std::vector attribute_descriptions{Vertex::GetAttributeDescriptions()[0], Vertex::GetAttributeDescriptions()[1],
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    // 4.需要绑定的顶点缓冲数组
    // 5.顶点数据在顶点缓冲中的偏移值数组
    vkCmdBindVertexBuffers(command_buffer, k_vertex_buffer_bind_id, 1, vertex_buffers.data(), offsets.data());
    vkCmdBindVertexBuffers(command_buffer, k_instance_buffer_bind_id, 1, instance_buffers.data(), offsets.data());
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);

    if (m_support_multi_draw_indirect) {
      // 2. 包含绘制参数的缓冲区对象
      // 3. 指向缓冲区中的一个位置，从这里开始读取第一组绘制参数
      // 4. 绘制次数（多次间接绘制需要GPU支持）
      // 5. 连续的绘制参数集之间的字节间隔
      vkCmdDrawIndexedIndirect(command_buffer, m_indirect_buffer, 0, static_cast<uint32_t>(m_indirect_cmd.size()),
                               sizeof(VkDrawIndexedIndirectCommand));
    } else {
      for (size_t i = 0; i < m_indirect_cmd.size(); ++i) {
        vkCmdDrawIndexedIndirect(command_buffer, m_indirect_buffer, i * sizeof(VkDrawIndexedIndirectCommand), 1,
                                 sizeof(VkDrawIndexedIndirectCommand));
      }
    }
  ...
  void CreateIndirectBuffer() {
    ...
    indirect_cmd.indexCount = static_cast<uint32_t>(k_indices.size());           // 要绘制的顶点个数
    indirect_cmd.firstIndex = 0;                                                 // 开始的索引
    indirect_cmd.instanceCount = static_cast<uint32_t>(k_instance_data.size());  // 要绘制的实例个数
    indirect_cmd.firstInstance = 0;                                              // 要绘制的第一个实例的ID
    indirect_cmd.vertexOffset = 0;                                               // 顶点偏移

    m_indirect_cmd.emplace_back(indirect_cmd);

    VkDeviceSize buffer_size = sizeof(VkDrawIndexedIndirectCommand) * m_indirect_cmd.size();
    ...
    void* data{nullptr};
    vkMapMemory(m_device, staging_buffer_memory, 0, buffer_size, 0, &data);
    std::memcpy(data, m_indirect_cmd.data(), static_cast<size_t>(buffer_size));
    vkUnmapMemory(m_device, staging_buffer_memory);

    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_indirect_buffer, m_indirect_buffer_memory);

    CopyBuffer(staging_buffer, m_indirect_buffer, buffer_size);
  ...
  void CreateInstancingBuffer() {
    VkDeviceSize buffer_size = sizeof(InstanceData) * k_instance_data.size();
    ...
    void* data{nullptr};
    vkMapMemory(m_device, staging_buffer_memory, 0, buffer_size, 0, &data);
    std::memcpy(data, k_instance_data.data(), static_cast<size_t>(buffer_size));
    vkUnmapMemory(m_device, staging_buffer_memory);
...
private:
  ...
  VkBuffer m_indirect_buffer{nullptr};
  VkDeviceMemory m_indirect_buffer_memory{nullptr};
  std::vector<VkDrawIndexedIndirectCommand> m_indirect_cmd{};
  bool m_support_multi_draw_indirect{false};
};
