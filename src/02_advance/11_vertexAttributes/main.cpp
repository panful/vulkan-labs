...
struct VertexSeparate
{
    ...
    static constexpr std::array<VkVertexInputBindingDescription, 2> GetBindingDescriptions() noexcept
    {
        ...
        binding_descriptions[0].stride    = sizeof(PosType);
        binding_descriptions[0].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

        binding_descriptions[1].binding   = 1;
        binding_descriptions[1].stride    = sizeof(ColorType);
...
const std::vector<VertexSeparate::PosType> k_vertices_pos{
    { -0.5F, -0.5F },
    { -0.5F,  0.5F },
    {  0.5F,  0.5F },
    {  0.5F, -0.5F },
...
// 注意两种方式的颜色数据不一样
const std::vector<VertexSeparate::ColorType> k_vertices_color{
    { 1.F, 1.F, 0.F },
    { 1.F, 1.F, 0.F },
    { 0.F, 1.F, 1.F },
    { 0.F, 1.F, 1.F },
...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        vkDestroyBuffer(m_device, m_vertex_pos_buffer, nullptr);
        vkFreeMemory(m_device, m_vertex_pos_buffer_memory, nullptr);
        vkDestroyBuffer(m_device, m_vertex_color_buffer, nullptr);
        vkFreeMemory(m_device, m_vertex_color_buffer_memory, nullptr);
        vkDestroyBuffer(m_device, m_index_buffer, nullptr);
        vkFreeMemory(m_device, m_index_buffer_memory, nullptr);

        vkDestroyPipeline(m_device, m_graphics_pipeline, nullptr);
        vkDestroyPipeline(m_device, m_graphics_pipeline_separate, nullptr);
...
private:
    ...
    void PrepareImGui()
    {
        ...
        {
            ImGui::RadioButton("Interleaved", &m_attribute_type, 0);
            ImGui::SameLine();
            ImGui::RadioButton("Separate", &m_attribute_type, 1);
        }
        ImGui::End();
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_11_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_11_base_frag.spv");
        ...
        auto binding_descriptions_separate   = VertexSeparate::GetBindingDescriptions();
        auto attribute_descriptions_separate = VertexSeparate::GetAttributeDescriptions();

        VkPipelineVertexInputStateCreateInfo vertex_input_info_separate = {};
        vertex_input_info_separate.sType                                = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        vertex_input_info_separate.vertexBindingDescriptionCount        = static_cast<uint32_t>(binding_descriptions_separate.size());
        vertex_input_info_separate.pVertexBindingDescriptions           = binding_descriptions_separate.data();
        vertex_input_info_separate.vertexAttributeDescriptionCount      = static_cast<uint32_t>(attribute_descriptions_separate.size());
        vertex_input_info_separate.pVertexAttributeDescriptions         = attribute_descriptions_separate.data();
        ...
        // 创建顶点属性分离的渲染管线
        pipeline_info.pVertexInputState = &vertex_input_info_separate;
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipeline_separate))
        {
            throw std::runtime_error("failed to create graphics pipeline");
        }
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        else if (1 == m_attribute_type)
        {
            vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipeline_separate);

            std::array<VkBuffer, 2> vertex_buffers = {m_vertex_pos_buffer, m_vertex_color_buffer};
    ...
    {
        auto tmp_create_vertex_buffer = [this](VkBuffer& buffer, VkDeviceMemory& memory, VkDeviceSize buffer_size, const void* data_pointer) {
            VkBuffer staging_buffer {};
            VkDeviceMemory staging_buffer_memory {};
            ...
            CreateBuffer(
                buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffer, memory
            );
        ...
        VkDeviceSize pos_size   = sizeof(VertexSeparate::PosType) * k_vertices_pos.size();
        VkDeviceSize color_size = sizeof(VertexSeparate::ColorType) * k_vertices_color.size();

        tmp_create_vertex_buffer(m_vertex_pos_buffer, m_vertex_pos_buffer_memory, pos_size, k_vertices_pos.data());
        tmp_create_vertex_buffer(m_vertex_color_buffer, m_vertex_color_buffer_memory, color_size, k_vertices_color.data());
    }
...
private:
    ...
    VkPipeline m_graphics_pipeline_separate {nullptr};
    VkBuffer m_vertex_pos_buffer {nullptr};
    VkDeviceMemory m_vertex_pos_buffer_memory {nullptr};
    VkBuffer m_vertex_color_buffer {nullptr};
    VkDeviceMemory m_vertex_color_buffer_memory {nullptr};

    uint32_t m_min_image_count {0};
    VkDescriptorPool m_imgui_descriptor_pool {nullptr};
    int m_attribute_type {0};
};
