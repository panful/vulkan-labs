...
private:
    ...
    {
        auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_11_base_vert.spv");
        auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/02_11_base_frag.spv");
    ...
    void CreateVertexBufferAttributes()
    {
        auto tmpCreateVertexBuffer = [this](VkBuffer& buffer, VkDeviceMemory& memory, VkDeviceSize bufferSize, const void* dataPointer) {
            ...
            CreateBuffer(
                bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffer, memory
            );
