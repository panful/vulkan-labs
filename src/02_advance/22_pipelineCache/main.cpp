...
private:
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_22_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/02_22_base_frag.spv");
        ...
        pipeline_cache_ci.pInitialData    = nullptr; // 从文件读取 pipelineCache 并赋值给 VkPipelineCacheCreateInfo

        if (VK_SUCCESS != vkCreatePipelineCache(m_device, &pipeline_cache_ci, nullptr, &m_pipeline_cache))
        {
            throw std::runtime_error("failed to create pipeline cache");
        }

        size_t pipeline_cache_data_size {};
        vkGetPipelineCacheData(m_device, m_pipeline_cache, &pipeline_cache_data_size, nullptr);
        std::cout << "pipeline data size: " << pipeline_cache_data_size << '\n';

        std::vector<uint8_t> pipeline_cache_data {};
        pipeline_cache_data.resize(pipeline_cache_data_size);
        vkGetPipelineCacheData(m_device, m_pipeline_cache, &pipeline_cache_data_size, reinterpret_cast<void*>(pipeline_cache_data.data()));

        // 将生成的 VkPipelineCache 写入文件，可以通过读取文件赋值给 VkPipelineCacheCreateInfo
        std::string cache_file_name {"pipeline_cache.bin"};
        std::ofstream write_pipeline_cache(cache_file_name, std::ios_base::out | std::ios_base::binary);
        if (write_pipeline_cache.good())
        {
            write_pipeline_cache.write(
                reinterpret_cast<const char*>(pipeline_cache_data.data()), static_cast<std::streamsize>(pipeline_cache_data_size)
            );
            write_pipeline_cache.close();
        }
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, m_pipeline_cache, 1, &pipeline_info, nullptr, &m_graphics_pipeline))
        {
