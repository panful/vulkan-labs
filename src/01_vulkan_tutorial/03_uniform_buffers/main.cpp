...
private:
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base_frag.spv");
...
private:
  ...
  void Cleanup() noexcept {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyBuffer(m_device, m_color_uniform_buffers.at(i), nullptr);
      vkFreeMemory(m_device, m_color_uniform_buffers_memory.at(i), nullptr);
      vkDestroyBuffer(m_device, m_mvp_uniform_buffers.at(i), nullptr);
      vkFreeMemory(m_device, m_mvp_uniform_buffers_memory.at(i), nullptr);
    }

    vkDestroyDescriptorPool(m_device, m_descriptor_pool, nullptr);
    vkDestroyDescriptorSetLayout(m_device, m_color_descriptor_set_layout, nullptr);
    vkDestroyDescriptorSetLayout(m_device, m_mvp_descriptor_set_layout, nullptr);
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base2_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base2_frag.spv");
    ...
    std::array<VkDescriptorSetLayout, 2> descriptor_set_layouts{m_color_descriptor_set_layout,
                                                                m_mvp_descriptor_set_layout};
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    bool set_type{true};
    if (set_type) {
      const std::array<VkDescriptorSet, 2> descriptor_sets{m_descriptor_sets[m_current_frame],
                                                           m_descriptor_sets[m_current_frame + 2]};
      vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 2,
                              descriptor_sets.data(), 0, nullptr);
    } else {
      vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1,
                              &m_descriptor_sets[m_current_frame], 0, nullptr);
      vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 1, 1,
                              &m_descriptor_sets[m_current_frame + 2], 0, nullptr);
    }
  ...
  void CreateDescriptorSetLayout() {
    {
      ...
      if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_color_descriptor_set_layout)) {
        throw std::runtime_error("failed to create descriptor set layout");
      }
    }

    {
      ...
      if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_mvp_descriptor_set_layout)) {
        throw std::runtime_error("failed to create descriptor set layout");
      }
    }
  }
  ...
  void CreateUniformBuffers() {
    {
      VkDeviceSize buffer_size = sizeof(UniformBufferObjectColor);

      m_color_uniform_buffers.resize(k_max_frames_in_flight);
      m_color_uniform_buffers_memory.resize(k_max_frames_in_flight);
      m_color_uniform_buffers_mapped.resize(k_max_frames_in_flight);

      for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
        CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     m_color_uniform_buffers.at(i), m_color_uniform_buffers_memory.at(i));
        vkMapMemory(m_device, m_color_uniform_buffers_memory.at(i), 0, buffer_size, 0,
                    &m_color_uniform_buffers_mapped.at(i));
      }
    }

    {
      VkDeviceSize buffer_size = sizeof(UniformBufferObjectMVP);

      m_mvp_uniform_buffers.resize(k_max_frames_in_flight);
      m_mvp_uniform_buffers_memory.resize(k_max_frames_in_flight);
      m_mvp_uniform_buffers_mapped.resize(k_max_frames_in_flight);

      for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
        CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     m_mvp_uniform_buffers.at(i), m_mvp_uniform_buffers_memory.at(i));
        vkMapMemory(m_device, m_mvp_uniform_buffers_memory.at(i), 0, buffer_size, 0,
                    &m_mvp_uniform_buffers_mapped.at(i));
      }
    }
  }
  ...
  void UpdateUniformBuffer(uint32_t current_image) {
    ...
    // 将变换矩阵的数据复制到uniform缓冲
    std::memcpy(m_mvp_uniform_buffers_mapped[current_image], &mvp, sizeof(mvp));

    UniformBufferObjectColor color{.color = {1.F, 0.F, 1.F}};

    std::memcpy(m_color_uniform_buffers_mapped[current_image], &color, sizeof(color));
  }
  ...
  void CreateDescriptorPool() {
    VkDescriptorPoolSize pool_size{};
    pool_size.type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    pool_size.descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight) * 2;
  ...
  void CreateDescriptorSets() {
    ...
    layouts.insert(layouts.cend(), k_max_frames_in_flight, m_color_descriptor_set_layout);
    layouts.insert(layouts.cend(), k_max_frames_in_flight, m_mvp_descriptor_set_layout);
    ...
    // 描述符集对象会在描述符池对象清除时自动被清除
    m_descriptor_sets.resize(k_max_frames_in_flight * 2);  // 并行两帧 * (color + mvp)
    if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_descriptor_sets.data())) {
      throw std::runtime_error("failed to allocate descriptor sets");
    }
...
private:
  ...
  VkDescriptorSetLayout m_color_descriptor_set_layout{nullptr};
  VkDescriptorSetLayout m_mvp_descriptor_set_layout{nullptr};

  std::vector<VkBuffer> m_color_uniform_buffers{};
  std::vector<VkDeviceMemory> m_color_uniform_buffers_memory{};
  std::vector<void*> m_color_uniform_buffers_mapped{};

  std::vector<VkBuffer> m_mvp_uniform_buffers{};
  std::vector<VkDeviceMemory> m_mvp_uniform_buffers_memory{};
  std::vector<void*> m_mvp_uniform_buffers_mapped{};
