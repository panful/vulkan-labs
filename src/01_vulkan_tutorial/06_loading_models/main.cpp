...
private:
  ...
  void CreateVertexBuffer() {
    ...
    buffer_size = sizeof(Vertex) * m_vertices.size();
    ...
    // m_vertex_buffer 现在关联的内存是设备所有的（显卡），不能使用 vkMapMemory 函数对它关联的内存进行映射
  ...
  void CreateIndexBuffer() {
    VkDeviceSize buffer_size{0};
    buffer_size = sizeof(m_indices.front()) * m_indices.size();
  ...
  void CreateTextureImage() {
    // STBI_rgb_alpha 强制使用alpha通道，如果没有会被添加一个默认的alpha值，tex_channels返回图像实际的通道数
    int tex_width{0};
    int tex_height{0};
    int tex_channels{0};
...
enum class LightingMode : int { None, Phong, BlinnPhong, PBR, RayTracing };
...
struct Primitive {
  bool VertexColoring() { return false; }

  bool CellColoring() { return false; }
...
struct Vertex {
  ...
  static std::vector<VkVertexInputBindingDescription> GetBindingDescriptions(
    const PipelineType pipeline_type) noexcept {
  ...
  void DestroyModel() noexcept {
    for (const auto& [_, model] : m_models) {
      ...
      for (const auto& [_, buffer] : model->buffers) {
        ...
        vkFreeMemory(m_device, buffer->buffer_memory, nullptr);
      }
...
private:
  ...
  std::vector<float> ParseAnimationBuffer(const std::unique_ptr<Model>& model, const tinygltf::Accessor& accessor) {
    ...
    const auto data_pointer =
      &model->gltfModel.buffers[buffer_view.buffer].data[accessor.byteOffset + buffer_view.byteOffset];

    return std::vector<float>(reinterpret_cast<float*>(data_pointer),
                              reinterpret_cast<float*>(data_pointer) + accessor.count * size);
  }
  ...
  void ParseNode(const std::unique_ptr<Model>& model, std::vector<std::unique_ptr<Node>>& nodes, int node_index,
                 Node* parent = nullptr) {
    ...
    if (node.matrix.size() == 16) {
      ...
      if (node.translation.size() == 3) {
        temp_node->matrices["translation"] =
          glm::translate(glm::mat4(1.F), glm::vec3(glm::make_vec3(node.translation.data())));
      }
    ...
    if (node.skin >= 0) {
      ...
      if (skin.inverseBindMatrices >= 0) {
        ...
        temp_skin->matrix =
          CreateStorageBuffer(std::get<0>(buffer), std::get<1>(buffer),
                              m_context->descriptorSetLayouts.at("uniform_animation_skin")->descriptorSetLayout);
      }
    ...
    if (node.mesh >= 0) {
      temp_node->modelUniform = CreateUniforms(
        GetNodeMatrix(temp_node.get()), m_context->descriptorSetLayouts.at("uniform_model")->descriptorSetLayout);
  ...
  std::tuple<const void*, size_t, uint32_t, std::string, std::vector<float>> ParseBuffer(
    const std::unique_ptr<Model>& model, const tinygltf::Accessor& accessor) {
    const auto& buffer_view = model->gltfModel.bufferViews[accessor.bufferView];
    const auto data_pointer =
      &model->gltfModel.buffers[buffer_view.buffer].data[accessor.byteOffset + buffer_view.byteOffset];
    ...
    auto buffer_info =
      "ptr: " + std::to_string(reinterpret_cast<std::uintptr_t>(data_pointer)) + "\tsize: " + std::to_string(data_size);

    return std::make_tuple(data_pointer, count * sizeof(float), static_cast<uint32_t>(element_count), buffer_info,
                           f_data);
  }
  ...
  std::string ParseImage(const std::unique_ptr<Model>& model, const tinygltf::Image& image) {
    ...
    if (!image.as_is) {
      ...
      image_info += ("ptr: " + std::to_string(reinterpret_cast<std::uintptr_t>(image.image.data())) +
  ...
  void ParseMesh(const std::unique_ptr<Model>& model, const std::unique_ptr<Mesh>& mesh,
    ...
    for (const auto& primitive : gltf_mesh.primitives) {
      ...
      if (!gltf_mesh.weights.empty()) {
        std::array<float, 2> weights{static_cast<float>(gltf_mesh.weights[0]),
                                     static_cast<float>(gltf_mesh.weights[1])};
        temp_primitive->animationWeight = CreateUniforms(
          weights.data(), m_context->descriptorSetLayouts.at("uniform_animation_morph")->descriptorSetLayout);
      }
      ...
      for (const auto& target : primitive.targets) {
        if (target.contains("POSITION")) {
          ...
          if (!model->buffers.contains(std::get<3>(buffer))) {
            model->buffers.try_emplace(std::get<3>(buffer),
                                       CreateVertexBuffer(std::get<1>(buffer), std::get<0>(buffer)));
          }
      ...
      if (primitive.attributes.contains("JOINTS_0")) {
        ...
        if (!model->buffers.contains(std::get<3>(buffer))) {
          if (!std::get<4>(buffer).empty()) {
            model->buffers.try_emplace(std::get<3>(buffer),
                                       CreateVertexBuffer(std::get<1>(buffer), std::get<4>(buffer).data()));
          }
        }
      ...
      if (primitive.indices >= 0) {
        ...
        auto buffer_info = "ptr: " + std::to_string(reinterpret_cast<std::uintptr_t>(data_pointer)) +
      ...
      if (primitive.material >= 0) {
        ...
        if (material.pbrMetallicRoughness.metallicRoughnessTexture.index >= 0) {
          ...
          CreateTextureUniforms(model, temp_texture,
                                m_context->descriptorSetLayouts.at("uniform_sampler")->descriptorSetLayout);
  ...
  void DrawNode(const std::unique_ptr<Model>& model, VkCommandBuffer command_buffer,
    ...
    if (node->mesh) {
      for (const auto& primitive : node->mesh->primitives) {
        ...
        std::vector<VkDescriptorSet> descriptor_sets{
          node->modelUniform->descriptorSets->descriptorSets[m_current_frame]};
            ...
            switch (primitive->coloringMode) {
              case ColoringMode::DirectRGB: {
                if (AnimationMode::Morph == primitive->animationMode && model->attributes.animation) {
                  ...
                  descriptor_sets.emplace_back(lvk_tidy::GetRequiredValue(primitive->animationWeight)
                                                 ->descriptorSets->descriptorSets[m_current_frame]);

                  auto& base_color =
                    lvk_tidy::GetRequiredValue(primitive->material->pbrMetallicRoughness->baseColorFactor);
                  descriptor_sets.emplace_back(base_color->descriptorSets->descriptorSets[m_current_frame]);

                  vertex_buffers.emplace_back(
                    model->buffers.at(lvk_tidy::GetRequiredValue(primitive->morphPosition)[0])->buffer);
                  vertex_buffers.emplace_back(
                    model->buffers.at(lvk_tidy::GetRequiredValue(primitive->morphPosition)[1])->buffer);
                  ...
                  vertex_buffers.emplace_back(
                    model->buffers.at(lvk_tidy::GetRequiredValue(primitive->weights))->buffer);
              ...
              case ColoringMode::TextureMapping: {
                ...
                auto& base_texture =
                  lvk_tidy::GetRequiredValue(primitive->material->pbrMetallicRoughness->baseColorTexture);
          ...
          case LightingMode::PBR:
            switch (primitive->coloringMode) {
              case ColoringMode::DirectRGB: {
                ...
                auto& rm_factor =
                  lvk_tidy::GetRequiredValue(primitive->material->pbrMetallicRoughness->roughnessMetallicFactor);
        ...
        if (LightingMode::None != primitive->lightingMode) {
          ...
          vkCmdPushConstants(command_buffer, pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(PushConstantVP),
                             sizeof(PushConstantCamPos), &pc_cam_pos);
        }
  ...
  void UpdateNodeJoint(const std::unique_ptr<Scene>& /*scene*/, const std::unique_ptr<Node>& node) const {
    if (node->skin) {
      ...
      node->skin->matrix->UpdateUniform(m_current_frame, joint_matrices.data(),
                                        sizeof(glm::mat4) * joint_matrices.size());
    }
  }
    ...
    for (const auto& animation : model->animations) {
      for (const auto& channel : animation->channels) {
        ...
        if (!node->skin && node->mesh) {
          ...
          for (size_t i = 1; i < input.size(); ++i) {
            if (input[i] > t) {
              weights[0] = static_cast<float>(output[(i - 1) * 2] + (t - input[i - 1]) / (input[i] - input[i - 1]) *
                                                                      (output[i * 2] - output[(i - 1) * 2]));
              weights[1] =
                static_cast<float>(output[(i - 1) * 2 + 1] + (t - input[i - 1]) / (input[i] - input[i - 1]) *
                                                               (output[i * 2 + 1] - output[(i - 1) * 2 + 1]));
              break;
            }
          }

          for (const auto& primitive : node->mesh->primitives) {
            lvk_tidy::GetRequiredValue(primitive->animationWeight)
              ->UpdateUniform(m_current_frame, weights.data(), sizeof(float) * 2);
          }
        }
      }
    }
  ...
  std::unique_ptr<Image> CreateTextureImage(const void* data_pointer, size_t data_size, uint32_t width, uint32_t height,
                                            VkFormat format) {
    ...
    TransitionImageLayout(image->image, format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  ...
  std::unique_ptr<Buffer> CreateDrawableBuffer(const VkDeviceSize buffer_size, const void* data_pointer,
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                 buffer->buffer, buffer->buffer_memory);
  ...
  /// @brief 创建图形管线
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  std::unique_ptr<Pipeline> CreateGraphicsPipeline(PipelineType pipeline_type, const std::string& vert_path,
    ...
    switch (pipeline_type) {
      ...
      case PipelineType::DcNlAW2: {
        descriptor_set_layouts.emplace_back(
          m_context->descriptorSetLayouts.at("uniform_animation_morph")->descriptorSetLayout);
        descriptor_set_layouts.emplace_back(m_context->descriptorSetLayouts.at("uniform_color")->descriptorSetLayout);
      } break;
      case PipelineType::DcNlAS2: {
        descriptor_set_layouts.emplace_back(
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline->pipeline)) {
      throw std::runtime_error("failed to create graphics pipeline");
    }
  ...
  void CreateDescriptorSetLayouts() {
    // 只有 stageFlags 不同，其他都相同，是否可以考虑只保留 vertex fragment 两种描述符绑定信息
    {
      ...
      if (VK_SUCCESS !=
          vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &descriptor_set_layout->descriptorSetLayout)) {
        throw std::runtime_error("failed to create descriptor set layout");
      }
...

    {
      ...
      m_context->descriptorSetLayouts.try_emplace("uniform_roughnessFactor_metallicFactor",
  ...
  void CreateTextureUniforms(const std::unique_ptr<Model>& model, const std::unique_ptr<Texture>& texture,
  ...
  std::unique_ptr<Uniform> CreateStorageBuffer(const void* data, VkDeviceSize buffer_size,
                                               VkDescriptorSetLayout descriptor_set_layout) {
...
private:
  ...
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_SHADER_DIR "01_06_color_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_SHADER_DIR "01_06_color_frag.spv");
  ...
  void CreateDescriptorSets() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      descriptor_writes[0].dstSet = m_descriptor_sets.at(i);  // 指定要更新的描述符集对象
      descriptor_writes[0].dstBinding = 9;                    // 指定缓冲绑定
      descriptor_writes[0].dstArrayElement = 0;               // 描述符数组的第一个元素的索引（没有数组就使用0）
      descriptor_writes[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
      descriptor_writes[0].descriptorCount = 1;
      descriptor_writes[0].pBufferInfo = &buffer_info;  // 指定描述符引用的缓冲数据
      descriptor_writes[0].pImageInfo = nullptr;        // 指定描述符引用的图像数据
  ...
  void LoadModel() {
    const std::string filename{PROJECT_ASSETS_DIR "models/colored_cube.ply"};
    ...
    try {
      ...
      try {
        ...
        throw std::runtime_error(std::string("No vertex x/y/z data: ") + e.what());
      }

      // 普通 PLY 顶点颜色：red green blue
      try {
        colors = ply_file.request_properties_from_element("vertex", {"red", "green", "blue"});
      } catch (...) {
        std::cerr << "No red/green/blue color found, use white.\n";
      }

      // mesh 索引：face.vertex_indices
      // 注意：这里假设都是三角形
      try {
        faces = ply_file.request_properties_from_element("face", {"vertex_indices"}, 3);
      } catch (...) {
        throw std::runtime_error("No triangle face indices found.");
      }
      ...
      if (positions) {
        if (positions->t != tinyply::Type::FLOAT32) {
          throw std::runtime_error("Only float32 x/y/z is supported in this demo.");
        }
        ...
        for (size_t i = 0; i < vertex_count; ++i) {
          m_vertices[i].pos = glm::vec3(pos_data[i * 3 + 0], pos_data[i * 3 + 1], pos_data[i * 3 + 2]);
        }
      ...
      // 填充颜色
      if (colors) {
        if (colors->count != m_vertices.size()) {
          std::cerr << "Color count does not match vertex count.\n";
        } else if (colors->t == tinyply::Type::UINT8) {
          const uint8_t* color_data = reinterpret_cast<const uint8_t*>(colors->buffer.get());

          for (size_t i = 0; i < m_vertices.size(); ++i) {
            m_vertices[i].color = glm::vec3(static_cast<float>(color_data[i * 3 + 0]) / 255.0F,
                                            static_cast<float>(color_data[i * 3 + 1]) / 255.0F,
                                            static_cast<float>(color_data[i * 3 + 2]) / 255.0F);
          }
        } else if (colors->t == tinyply::Type::FLOAT32) {
          const float* color_data = reinterpret_cast<const float*>(colors->buffer.get());

          for (size_t i = 0; i < m_vertices.size(); ++i) {
            m_vertices[i].color = glm::vec3(color_data[i * 3 + 0], color_data[i * 3 + 1], color_data[i * 3 + 2]);
          }
        } else {
          std::cerr << "Unsupported color type, use white.\n";
        }
      }

      // 填充 indices
      if (faces) {
        const size_t triangle_count = faces->count;
        const size_t index_count = triangle_count * 3;

        m_indices.resize(index_count);

        if (faces->t == tinyply::Type::UINT32) {
          const uint32_t* index_data = reinterpret_cast<const uint32_t*>(faces->buffer.get());

          for (size_t i = 0; i < index_count; ++i) {
            m_indices[i] = index_data[i];
          }
        } else if (faces->t == tinyply::Type::INT32) {
          const int32_t* index_data = reinterpret_cast<const int32_t*>(faces->buffer.get());

          for (size_t i = 0; i < index_count; ++i) {
            m_indices[i] = static_cast<uint32_t>(index_data[i]);
          }
        } else {
          std::cerr << "Unsupported face index type.\n";
        }
      ...
      throw std::runtime_error(std::string("tinyply error: ") + e.what());
    }
