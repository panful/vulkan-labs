...
void DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debug_messenger,
                                   const VkAllocationCallbacks* p_allocator) {
  auto func =
    lvk_tidy::LoadInstanceProcAddress<PFN_vkDestroyDebugUtilsMessengerEXT>(instance, "vkDestroyDebugUtilsMessengerEXT");
  if (func != nullptr) {
...
private:
  ...
  void CreateLogicalDevice() {
    ...
    std::set<uint32_t> unique_queue_families = {lvk_tidy::GetRequiredValue(indices.graphicsAndComputeFamily),
  ...
  void CreateSwapChain() {
    ...
    QueueFamilyIndices indices = FindQueueFamilies(m_physical_device);
    const std::array<uint32_t, 2> queue_family_indices{lvk_tidy::GetRequiredValue(indices.graphicsAndComputeFamily),
                                                       lvk_tidy::GetRequiredValue(indices.presentFamily)};
  ...
  void CreateShaderStorageBuffers() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; i++) {
      CreateBuffer(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_shader_storage_buffers[i], m_shader_storage_buffers_memory[i]);
  ...
  void RecordComputeCommandBuffer(VkCommandBuffer command_buffer) {
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_compute_pipeline_layout, 0, 1,
                            &m_compute_descriptor_sets[m_current_frame], 0, nullptr);
  ...
  void DrawFrame() {
    ...
    VkResult result = vkAcquireNextImageKHR(
      m_device, m_swap_chain, UINT64_MAX, m_image_available_semaphores[m_current_frame], VK_NULL_HANDLE, &image_index);
    ...
    const std::array<VkSemaphore, 2> wait_semaphores{m_compute_finished_semaphores[m_current_frame],
