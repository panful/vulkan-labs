...
struct QueueFamilyIndices {
  ...
  constexpr bool IsComplete() const noexcept { return graphicsFamily.has_value(); }
};
...
private:
  ...
  void CreateRenderTargetImages() {
    ...
    for (size_t i = 0; i < m_render_target_images.size(); ++i) {
      CreateImage(m_render_target_extent.width, m_render_target_extent.height, m_render_target_image_format,
                  VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_render_target_images.at(i),
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_SHADER_DIR "04_01_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_SHADER_DIR "04_01_base_frag.spv");
  ...
  void CreateFramebuffers() {
    ...
    for (size_t i = 0; i < m_render_target_image_views.size(); ++i) {
      ...
      if (VK_SUCCESS !=
          vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &m_render_target_framebuffers.at(i))) {
  ...
  void SaveImage(size_t current_frame) {
    ...
    // 将目标图像转换为：传输目标布局
    InsertImageMemoryBarrier(m_save_image_command_buffer, dst_image, VK_IMAGE_LAYOUT_UNDEFINED,
                             VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
                             VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

    if (m_supports_blit) {
      BlitImage(m_save_image_command_buffer, m_render_target_extent.width, m_render_target_extent.height, src_image,
                dst_image);
    } else {
      CopyImage(m_save_image_command_buffer, m_render_target_extent.width, m_render_target_extent.height, src_image,
                dst_image);
    }

    // 将目标图像转换为：通用读取布局
    InsertImageMemoryBarrier(m_save_image_command_buffer, dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  void CreateDepthResources() {
    ...
    CreateImage(m_render_target_extent.width, m_render_target_extent.height, depth_format, VK_IMAGE_TILING_OPTIMAL,
