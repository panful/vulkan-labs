...
private:
  ...
  bool IsDeviceSuitable(VkPhysicalDevice device) const {
    ...
    if (!device_features.geometryShader || !device_features.multiViewport) {
  ...
  void CreateLogicalDevice() {
    ...
    VkPhysicalDeviceFeatures device_features = {};
    device_features.geometryShader = VK_TRUE;  // 开启几何着色器
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/05_02_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/05_02_base_frag.spv");
    auto geom_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/05_02_base_geom.spv");

    VkShaderModule vert_shader_module = CreateShaderModule(vert_shader_code);
    VkShaderModule frag_shader_module = CreateShaderModule(frag_shader_code);
    VkShaderModule geom_shader_module = CreateShaderModule(geom_shader_code);
    ...
    geom_shader_stage_info.pSpecializationInfo = nullptr;
    ...
    viewport_state.viewportCount = static_cast<uint32_t>(m_viewports.size());
    viewport_state.scissorCount = static_cast<uint32_t>(m_scissors.size());
    ...
    std::vector shader_stages = {vert_shader_stage_info, frag_shader_stage_info, geom_shader_stage_info};
  ...
  void SetViewportAndScissor() {
    auto w = m_swap_chain_extent.width;
    auto h = m_swap_chain_extent.height;
    ...
    m_scissors.at(0) = {{0, 0}, {w / 2, h}};
    m_scissors.at(1) = {{static_cast<int32_t>(w / 2), 0}, {w / 2, h}};
  }
