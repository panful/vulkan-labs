...
constexpr uint32_t k_shadow_map_width = 4096;
constexpr uint32_t k_shadow_map_height = 4096;
...
private:
  ...
  void Cleanup() noexcept {
    ...
    vkDestroyImageView(m_device, m_shadow_map_image_view, nullptr);
    vkDestroyImage(m_device, m_shadow_map_image, nullptr);
    vkFreeMemory(m_device, m_shadow_map_image_memory, nullptr);

    for (auto framebuffer : m_shadow_map_framebuffers) {
      vkDestroyFramebuffer(m_device, framebuffer, nullptr);
    }

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyImageView(m_device, m_shadow_map_color_image_views[i], nullptr);
      vkDestroyImage(m_device, m_shadow_map_color_image[i], nullptr);
      vkFreeMemory(m_device, m_shadow_map_color_image_memorys[i], nullptr);
    }
    ...
    destroy_drawable(m_drawable_sphere);
    destroy_drawable(m_drawable_plane);

    vkDestroyPipeline(m_device, m_light_graphics_pipeline, nullptr);
    vkDestroyPipelineLayout(m_device, m_light_pipeline_layout, nullptr);
    vkDestroyRenderPass(m_device, m_light_render_pass, nullptr);

    vkDestroyPipeline(m_device, m_shadow_map_graphics_pipeline, nullptr);
    vkDestroyPipelineLayout(m_device, m_shadow_map_pipeline_layout, nullptr);
    vkDestroyRenderPass(m_device, m_shadow_map_render_pass, nullptr);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyBuffer(m_device, m_shadow_map_uniform_buffers.at(i), nullptr);
      vkFreeMemory(m_device, m_shadow_map_uniform_buffers_memory.at(i), nullptr);
      vkDestroyBuffer(m_device, m_light_uniform_buffers.at(i), nullptr);
      vkFreeMemory(m_device, m_light_uniform_buffers_memory.at(i), nullptr);
    }

    vkDestroyDescriptorPool(m_device, m_descriptor_pool, nullptr);
    vkDestroyDescriptorPool(m_device, m_imgui_descriptor_pool, nullptr);

    vkDestroyDescriptorSetLayout(m_device, m_shadow_map_descriptor_set_layout, nullptr);
...
private:
  std::tuple<std::vector<Vertex>, std::vector<uint16_t>> CreateSphere(uint16_t longitude = 32, uint16_t latitude = 32,
                                                                      float radius = 1.F) {
    ...
    auto delta_latitude = (2 * std::numbers::pi_v<float> / static_cast<float>(latitude));
    auto delta_longitude = (std::numbers::pi_v<float> / static_cast<float>(longitude - 1));

    // 最上面单独的一个点
    vertices.emplace_back(Vertex{{0.F, radius, 0.F}, {0.F, radius, 0.F}});

    // 每一层，即纬线所在的圈
    for (size_t i = 1; i < longitude - 1; ++i) {
      auto r = radius * std::sin(static_cast<float>(i) * delta_longitude);
      auto y = radius * std::cos(static_cast<float>(i) * delta_longitude);

      // 每一层上的每一个点（纬线上的每一个点）
      for (size_t j = 0; j < latitude; ++j) {
        auto x = r * std::sin(static_cast<float>(j) * delta_latitude);
        auto z = r * std::cos(static_cast<float>(j) * delta_latitude);
  ...
  void CreateDrawables() {
    ...
    CreateVertexBuffer(std::get<0>(sphere), m_drawable_sphere);
    CreateIndexBuffer(std::get<1>(sphere), m_drawable_sphere);

    const std::vector<Vertex> vertices_plane{
    ...
    const std::vector<uint16_t> indices_plane{0, 1, 2, 0, 2, 3};

    CreateVertexBuffer(vertices_plane, m_drawable_plane);
    CreateIndexBuffer(indices_plane, m_drawable_plane);
  }
  ...
  void InitImGui() {
    ...
    ImGui_ImplVulkan_Init(&init_info, m_shadow_map_render_pass);
  ...
  bool CheckValidationLayerSupport() const noexcept {
    ...
    for (const char* layer_name : k_validation_layers) {
      ...
      for (const auto& layer_properties : available_layers) {
        if (0 == std::strcmp(layer_name, &layer_properties.layerName[0])) {
          layer_found = true;
          break;
        }
      }
  ...
  bool CheckDeviceExtensionSupported(VkPhysicalDevice device) const noexcept {
    ...
    for (const auto& extension : available_extensions) {
      required_extensions.erase(&extension.extensionName[0]);
    }
  ...
  void CreateSwapChain() {
    ...
    auto indices = FindQueueFamilies(m_physical_device);
    std::array<uint32_t, 2> queue_family_indices{lvk_tidy::GetRequiredValue(indices.graphics_family),
  ...
  void CreateImageViews() {
    m_shadow_map_color_image.resize(k_max_frames_in_flight);
    m_shadow_map_color_image_memorys.resize(k_max_frames_in_flight);
    m_shadow_map_color_image_views.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(k_shadow_map_width, k_shadow_map_height, m_swap_chain_image_format, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0, m_shadow_map_color_image[i],
                  m_shadow_map_color_image_memorys[i]);
      m_shadow_map_color_image_views[i] =
        CreateImageView(m_shadow_map_color_image[i], m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT);
    }
  }
  ...
  void CreateRenderPassLight() {
    ...
    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &m_light_render_pass)) {
  ...
  void CreateDescriptorSetLayout() {
    ...
    if (VK_SUCCESS !=
        vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_shadow_map_descriptor_set_layout)) {
      throw std::runtime_error("failed to create descriptor set layout");
    }
  }

  void CreateGraphicsPipelineLight() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_01_light_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_01_light_frag.spv");
    ...
    pipeline_layout_info.pSetLayouts = &m_light_descriptor_set_layout;
    pipeline_layout_info.pushConstantRangeCount = 1;
    pipeline_layout_info.pPushConstantRanges = &push_constant_range;

    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_light_pipeline_layout)) {
    ...
    pipeline_info.layout = m_light_pipeline_layout;
    pipeline_info.renderPass = m_light_render_pass;
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_light_graphics_pipeline)) {
  ...
  void CreateFramebuffersLight() {
    m_light_framebuffers.resize(m_swap_chain_image_views.size());

    for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i) {
      std::array<VkImageView, 2> attachments{m_swap_chain_image_views.at(i), m_light_depth_image_view};

      VkFramebufferCreateInfo framebuffer_info = {};
      framebuffer_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
      framebuffer_info.renderPass = m_light_render_pass;
      ...
      if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &m_light_framebuffers.at(i))) {
  ...
  void CreateSamplerLight() {
    ...
    if (VK_SUCCESS != vkCreateSampler(m_device, &sampler_info, nullptr, &m_sampler)) {
      throw std::runtime_error("failed to create sampler");
    }
  }

  void CreateDescriptorSetsLight() {
    std::vector<VkDescriptorSetLayout> layouts(k_max_frames_in_flight, m_light_descriptor_set_layout);
    ...
    m_light_descriptor_sets.resize(k_max_frames_in_flight);
    if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_light_descriptor_sets.data())) {
      throw std::runtime_error("failed to allocate descriptor sets");
    }

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      VkDescriptorBufferInfo buffer_info{};
      buffer_info.buffer = m_light_uniform_buffers.at(i);
      buffer_info.offset = 0;
      buffer_info.range = sizeof(UniformBufferObjectLight);

      VkDescriptorImageInfo image_info{};
      image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
      image_info.imageView = m_shadow_map_image_view;
      ...
      descriptor_writes[0].dstSet = m_light_descriptor_sets.at(i);
      ...
      descriptor_writes[1].dstSet = m_light_descriptor_sets.at(i);
  ...
  void CreateUniformBuffersLight() {
    VkDeviceSize buffer_size = sizeof(UniformBufferObjectLight);

    m_light_uniform_buffers.resize(k_max_frames_in_flight);
    m_light_uniform_buffers_memory.resize(k_max_frames_in_flight);
    m_light_uniform_buffers_mapped.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                   m_light_uniform_buffers.at(i), m_light_uniform_buffers_memory.at(i));
      vkMapMemory(m_device, m_light_uniform_buffers_memory.at(i), 0, buffer_size, 0,
                  &m_light_uniform_buffers_mapped.at(i));
    }
  }
  ...
  void CreateDepthResourcesLight() {
    ...
    CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, depth_format, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_light_depth_image,
                m_light_depth_image_memory);

    m_light_depth_image_view = CreateImageView(m_light_depth_image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT);
  }

  /// @brief 创建图形管线
  /// @details 在 Vulkan 中几乎不允许对图形管线进行动态设置，也就意味着每一种状态都需要提前创建一个图形管线
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_01_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_01_base_frag.spv");
    ...
    pipeline_layout_info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipeline_layout_info.setLayoutCount = 1;
    pipeline_layout_info.pSetLayouts = &m_shadow_map_descriptor_set_layout;
    pipeline_layout_info.pushConstantRangeCount = 0;

    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_shadow_map_pipeline_layout)) {
      throw std::runtime_error("failed to create pipeline layout");
    }
    ...
    pipeline_info.pColorBlendState = &color_blending;
    pipeline_info.pDynamicState = &dynamic_state;
    pipeline_info.layout = m_shadow_map_pipeline_layout;
    pipeline_info.renderPass = m_shadow_map_render_pass;
    pipeline_info.subpass = 0;                   // 子流程在子流程数组中的索引
    pipeline_info.basePipelineHandle = nullptr;  // 以一个创建好的图形管线为基础创建一个新的图形管线
    pipeline_info.basePipelineIndex = -1;  // 只有该结构体的成员 flags 被设置为 VK_PIPELINE_CREATE_DERIVATIVE_BIT 才有效
    pipeline_info.pDepthStencilState = &depth_stencil;

    if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
                                                &m_shadow_map_graphics_pipeline)) {
      throw std::runtime_error("failed to create graphics pipeline");
    }
  ...
  void CreateRenderPass() {
    ...
    color_attachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;  // 渲染之后对模板缓冲的操作
    color_attachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;          // 渲染流程开始前的图像布局方式
    color_attachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;  // 渲染流程结束后的图像布局方式

    VkAttachmentDescription depth_attachment = {};
    depth_attachment.format = FindDepthFormat();
    depth_attachment.samples = VK_SAMPLE_COUNT_1_BIT;
    depth_attachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    depth_attachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;  // 绘制结束后不需要从深度缓冲复制深度数据
    depth_attachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
    depth_attachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    depth_attachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;  // 不需要读取之前深度图像数据
    ...
    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &m_shadow_map_render_pass)) {
      throw std::runtime_error("failed to create render pass");
    }
  }
  ...
  void CreateFramebuffers() {
    m_shadow_map_framebuffers.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      // 颜色附着和深度（模板）附着
      // 和每个交换链图像对应不同的颜色附着不同，因为信号量的原因，只有一个subpass同时执行，所以一个深度附着即可
      std::array<VkImageView, 2> attachments{m_shadow_map_color_image_views[i], m_shadow_map_image_view};
      ...
      framebuffer_info.attachmentCount = static_cast<uint32_t>(attachments.size());
      framebuffer_info.pAttachments = attachments.data();
      framebuffer_info.width = k_shadow_map_width;
      framebuffer_info.height = k_shadow_map_height;
      framebuffer_info.layers = 1;

      if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &m_shadow_map_framebuffers[i])) {
        throw std::runtime_error("failed to create framebuffer");
      }
    }
  }
  ...
  void CreateCommandBuffers() {
    m_command_buffers.resize(m_min_image_count);
  ...
  /// @brief 记录指令到指令缓冲
  void RecordCommandBuffer(VkCommandBuffer command_buffer, uint32_t light_framebuffer_index) const {
    VkCommandBufferBeginInfo begin_info = {};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    ...
    VkViewport viewport_shadow_map = {};
    viewport_shadow_map.x = 0.F;
    viewport_shadow_map.y = 0.F;
    viewport_shadow_map.width = static_cast<float>(k_shadow_map_width);
    viewport_shadow_map.height = static_cast<float>(k_shadow_map_height);
    viewport_shadow_map.minDepth = 0.F;
    viewport_shadow_map.maxDepth = 1.F;

    VkRect2D scissor_shadow_map = {};
    scissor_shadow_map.offset = {0, 0};
    scissor_shadow_map.extent = VkExtent2D{k_shadow_map_width, k_shadow_map_height};
    ...
    auto draw_drawable = [command_buffer](const Drawable& drawable) {
      std::array<VkBuffer, 1> vertex_buffers{drawable.vertexBuffer};
      std::array<VkDeviceSize, 1> offsets{0};

      vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers.data(), offsets.data());
      vkCmdBindIndexBuffer(command_buffer, drawable.indexBuffer, 0, VK_INDEX_TYPE_UINT16);
      vkCmdDrawIndexed(command_buffer, drawable.indexCount, 1, 0, 0, 0);
    };

    VkRenderPassBeginInfo render_pass_info = {};
    render_pass_info.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    render_pass_info.renderPass = m_shadow_map_render_pass;
    render_pass_info.framebuffer = m_shadow_map_framebuffers[m_current_frame];
    render_pass_info.renderArea.offset = {0, 0};
    render_pass_info.renderArea.extent = VkExtent2D{k_shadow_map_width, k_shadow_map_height};
    render_pass_info.clearValueCount = static_cast<uint32_t>(clear_values.size());
    render_pass_info.pClearValues = clear_values.data();

    vkCmdBeginRenderPass(command_buffer, &render_pass_info, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_shadow_map_graphics_pipeline);
    vkCmdSetViewport(command_buffer, 0, 1, &viewport_shadow_map);
    vkCmdSetScissor(command_buffer, 0, 1, &scissor_shadow_map);
    vkCmdSetDepthBias(command_buffer, 1.25F, 0.0F, 1.75F);
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_shadow_map_pipeline_layout, 0, 1,
                            &m_shadow_map_descriptor_sets.at(m_current_frame), 0, nullptr);

    draw_drawable(m_drawable_sphere);
    draw_drawable(m_drawable_plane);

    vkCmdEndRenderPass(command_buffer);

    //----------------------------------------
    // light pass
    VkRenderPassBeginInfo light_render_pass_info = {};
    light_render_pass_info.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    light_render_pass_info.renderPass = m_light_render_pass;
    light_render_pass_info.framebuffer = m_light_framebuffers[light_framebuffer_index];
    light_render_pass_info.renderArea.offset = {0, 0};
    light_render_pass_info.renderArea.extent = m_swap_chain_extent;
    light_render_pass_info.clearValueCount = static_cast<uint32_t>(clear_values.size());
    light_render_pass_info.pClearValues = clear_values.data();

    vkCmdBeginRenderPass(command_buffer, &light_render_pass_info, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_light_graphics_pipeline);
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
    int pc_pcf = m_enable_pcf ? 1 : 0;
    vkCmdPushConstants(command_buffer, m_light_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(int), &pc_pcf);
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_light_pipeline_layout, 0, 1,
                            &m_light_descriptor_sets.at(m_current_frame), 0, nullptr);
    draw_drawable(m_drawable_sphere);
    draw_drawable(m_drawable_plane);
  ...
  void DrawFrame() {
    ...
    RecordCommandBuffer(m_command_buffers.at(image_index), image_index);
  ...
  void CleanupSwapChain() noexcept {
    vkDestroyImageView(m_device, m_light_depth_image_view, nullptr);
    vkDestroyImage(m_device, m_light_depth_image, nullptr);
    vkFreeMemory(m_device, m_light_depth_image_memory, nullptr);

    for (auto framebuffer : m_light_framebuffers) {
      vkDestroyFramebuffer(m_device, framebuffer, nullptr);
    }
  ...
  void CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
    ...
    // size: 缓冲需要的内存的字节大小，可能和buffer_info.size的值不同
  ...
  void CreateDescriptorSetLayoutLight() {
    ...
    if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &m_light_descriptor_set_layout)) {
      throw std::runtime_error("failed to create descriptor set layout");
    }
  }
  ...
  void CreateUniformBuffers() {
    ...
    m_shadow_map_uniform_buffers.resize(k_max_frames_in_flight);
    m_shadow_map_uniform_buffers_memory.resize(k_max_frames_in_flight);
    m_shadow_map_uniform_buffers_mapped.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                   m_shadow_map_uniform_buffers.at(i), m_shadow_map_uniform_buffers_memory.at(i));
      vkMapMemory(m_device, m_shadow_map_uniform_buffers_memory.at(i), 0, buffer_size, 0,
                  &m_shadow_map_uniform_buffers_mapped.at(i));
    }
  }
  ...
  void UpdateUniformBuffer(uint32_t current_image) {
    ...
    auto swap_chain_aspect =
      static_cast<float>(m_swap_chain_extent.width) / static_cast<float>(m_swap_chain_extent.height);
    auto light_aspect = static_cast<float>(k_shadow_map_width) / static_cast<float>(k_shadow_map_height);
    ...
    m_light_pos = glm::vec3(pos_x, -5.F, pos_z);

    UniformBufferObject ubo{};
    ubo.model = glm::mat4(1.F);
    ubo.view = glm::lookAt(m_light_pos, glm::vec3(0.F), glm::vec3(0.F, 0.F, 1.F));
    ubo.proj = glm::perspective(glm::radians(90.F), light_aspect, 0.1F, 100.F);  // 点光源的视角角度太小会有问题
    ubo.proj[1][1] *= -1.F;
    std::memcpy(m_shadow_map_uniform_buffers_mapped[current_image], &ubo, sizeof(ubo));

    UniformBufferObjectLight ubo_light{};
    ubo_light.model = glm::mat4(1.F);
    ubo_light.view = glm::lookAt(m_eye_pos, m_look_at, m_view_up);
    ubo_light.proj = glm::perspective(glm::radians(45.F), swap_chain_aspect, 0.1F, 100.F);
    ubo_light.lightSpace = ubo.proj * ubo.view;
    ubo_light.proj[1][1] *= -1.F;
    std::memcpy(m_light_uniform_buffers_mapped[current_image], &ubo_light, sizeof(ubo_light));
  }
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes[0].descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight) * 2;
    ...
    pool_info.poolSizeCount = static_cast<uint32_t>(pool_sizes.size());
    pool_info.pPoolSizes = pool_sizes.data();
    pool_info.maxSets = static_cast<uint32_t>(k_max_frames_in_flight) * 2;
    pool_info.flags = 0;
  ...
  void CreateDescriptorSets() {
    ...
    std::vector<VkDescriptorSetLayout> layouts(k_max_frames_in_flight, m_shadow_map_descriptor_set_layout);
    ...
    m_shadow_map_descriptor_sets.resize(k_max_frames_in_flight);
    if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_shadow_map_descriptor_sets.data())) {
      throw std::runtime_error("failed to allocate descriptor sets");
    }

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      VkDescriptorBufferInfo buffer_info{};
      buffer_info.buffer = m_shadow_map_uniform_buffers.at(i);
      buffer_info.offset = 0;
      buffer_info.range = sizeof(UniformBufferObject);

      VkWriteDescriptorSet descriptor_write{};
      descriptor_write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
      descriptor_write.dstSet = m_shadow_map_descriptor_sets.at(i);  // 指定要更新的描述符集对象
  ...
  void CreateDepthResources() {
    ...
    CreateImage(k_shadow_map_width, k_shadow_map_height, depth_format, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_shadow_map_image, m_shadow_map_image_memory);

    m_shadow_map_image_view = CreateImageView(m_shadow_map_image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT);
  }
...
private:
  ...
  VkRenderPass m_shadow_map_render_pass{nullptr};
  VkPipelineLayout m_shadow_map_pipeline_layout{nullptr};
  VkPipeline m_shadow_map_graphics_pipeline{nullptr};
  std::vector<VkFramebuffer> m_shadow_map_framebuffers{};
  ...
  Drawable m_drawable_sphere{};
  Drawable m_drawable_plane{};

  std::vector<VkImage> m_swap_chain_images{};
  std::vector<VkImageView> m_swap_chain_image_views{};
  std::vector<VkFramebuffer> m_light_framebuffers{};
  VkRenderPass m_light_render_pass{nullptr};
  VkPipelineLayout m_light_pipeline_layout{nullptr};
  VkPipeline m_light_graphics_pipeline{nullptr};

  VkDescriptorSetLayout m_light_descriptor_set_layout{nullptr};
  std::vector<VkDescriptorSet> m_light_descriptor_sets{};
  std::vector<VkBuffer> m_light_uniform_buffers{};
  std::vector<VkDeviceMemory> m_light_uniform_buffers_memory{};
  std::vector<void*> m_light_uniform_buffers_mapped{};
  VkSampler m_sampler{nullptr};

  VkDescriptorSetLayout m_shadow_map_descriptor_set_layout{nullptr};
  std::vector<VkDescriptorSet> m_shadow_map_descriptor_sets{};
  std::vector<VkBuffer> m_shadow_map_uniform_buffers{};
  std::vector<VkDeviceMemory> m_shadow_map_uniform_buffers_memory{};
  std::vector<void*> m_shadow_map_uniform_buffers_mapped{};
  std::vector<VkImage> m_shadow_map_color_image{};
  std::vector<VkDeviceMemory> m_shadow_map_color_image_memorys{};
  std::vector<VkImageView> m_shadow_map_color_image_views{};

  VkImage m_shadow_map_image{nullptr};
  VkDeviceMemory m_shadow_map_image_memory{nullptr};
  VkImageView m_shadow_map_image_view{nullptr};

  VkImage m_light_depth_image{nullptr};
  VkDeviceMemory m_light_depth_image_memory{nullptr};
  VkImageView m_light_depth_image_view{nullptr};

  glm::vec3 m_view_up{0.F, -1.F, 0.F};
  glm::vec3 m_eye_pos{0.F, 0.F, -5.F};
