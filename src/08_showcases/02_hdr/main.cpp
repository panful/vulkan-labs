...
constexpr VkFormat k_hdr_format = VK_FORMAT_R32G32B32A32_SFLOAT;
...
private:
  ...
  void Cleanup() noexcept {
    ...
    auto destroy_offscreen = [this](Offscreen& offscreen) {
      for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
        ...
        vkDestroyImageView(m_device, offscreen.colors[i].image_view, nullptr);

        vkDestroyFramebuffer(m_device, offscreen.framebuffers[i], nullptr);
      }

      vkDestroyImageView(m_device, offscreen.depth.image_view, nullptr);
    ...
    destroy_offscreen(m_offscreen);
    vkDestroySampler(m_device, m_offscreen_sampler, nullptr);
...
private:
  void CreateDrawables() {
    ...
    CreateSampler(m_offscreen_sampler);
  }
  ...
  void CreateImageDatasOffscreen(Offscreen& offscreen) {
    offscreen.colors.resize(k_max_frames_in_flight);
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, k_hdr_format, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0, offscreen.colors[i].image,
                  offscreen.colors[i].memory);

      offscreen.colors[i].image_view =
        CreateImageView(offscreen.colors[i].image, k_hdr_format, VK_IMAGE_ASPECT_COLOR_BIT);
    }
    ...
    CreateImage(m_swap_chain_extent.width, m_swap_chain_extent.height, depth_format, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, offscreen.depth.image,
                offscreen.depth.memory);

    offscreen.depth.image_view = CreateImageView(offscreen.depth.image, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT);
  }

  void CreateGraphicsPipelineOffscreen(Offscreen& offscreen) {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_02_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_02_base_frag.spv");
    ...
    pipeline_layout_info.pSetLayouts = &offscreen.descriptorSetLayout;
    pipeline_layout_info.pushConstantRangeCount = 0;

    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &offscreen.pipelineLayout)) {
    ...
    pipeline_info.layout = offscreen.pipelineLayout;
    pipeline_info.renderPass = offscreen.renderPass;
    ...
    if (VK_SUCCESS !=
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &offscreen.pipeline)) {
  ...
  void CreateRenderPassOffscreen(Offscreen& offscreen) {
    ...
    color_attachment.format = k_hdr_format;                              // 颜色缓冲附着的格式
    ...
    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &offscreen.renderPass)) {
      throw std::runtime_error("failed to create render pass");
    }
  }

  void CreateFramebuffersOffscreen(Offscreen& offscreen) {
    offscreen.framebuffers.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      std::array<VkImageView, 2> attachments{offscreen.colors[i].image_view, offscreen.depth.image_view};

      VkFramebufferCreateInfo framebuffer_info = {};
      framebuffer_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
      framebuffer_info.renderPass = offscreen.renderPass;
      ...
      if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &offscreen.framebuffers[i])) {
  ...
  void CreateDescriptorSetLayoutOffscreen(Offscreen& offscreen) {
    ...
    if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layout_info, nullptr, &offscreen.descriptorSetLayout)) {
  ...
  void CreateUniformBuffersOffscreen(Offscreen& offscreen) {
    ...
    offscreen.uniformBuffers.resize(k_max_frames_in_flight);
    offscreen.uniformBuffersMemory.resize(k_max_frames_in_flight);
    offscreen.uniformBuffersMapped.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                   offscreen.uniformBuffers.at(i), offscreen.uniformBuffersMemory.at(i));
      vkMapMemory(m_device, offscreen.uniformBuffersMemory.at(i), 0, buffer_size, 0,
                  &offscreen.uniformBuffersMapped.at(i));
    }
  }

  void CreateDescriptorSetsOffscreen(Offscreen& offscreen) {
    std::vector<VkDescriptorSetLayout> layouts(k_max_frames_in_flight, offscreen.descriptorSetLayout);
    ...
    offscreen.descriptorSets.resize(k_max_frames_in_flight);
    if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, offscreen.descriptorSets.data())) {
      throw std::runtime_error("failed to allocate descriptor sets");
    }

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      VkDescriptorBufferInfo buffer_info{};
      buffer_info.buffer = offscreen.uniformBuffers.at(i);
      ...
      descriptor_writes[0].dstSet = offscreen.descriptorSets.at(i);
  ...
  void PrepareImGui() {
    ...
    static bool s_enable_hdr{true};
    ImGui::Checkbox("Enable HDR", &s_enable_hdr);
    m_push_constant.enable_hdr = s_enable_hdr ? 1 : 0;
    static const std::array k_hdr_types{"Manual Exposure", "Reinhard Tone Mapping"};
    ImGui::Combo("HDR type", &m_push_constant.type, k_hdr_types.data(), static_cast<int>(k_hdr_types.size()));
    ImGui::InputFloat("exposure", &m_push_constant.exposure, 0.01F);
    ImGui::End();
  ...
  bool CheckValidationLayerSupport() const noexcept {
    ...
    for (const char* layer_name : k_validation_layers) {
      ...
      for (const auto& layer_properties : available_layers) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
        if (0 == std::strcmp(layer_name, layer_properties.layerName)) {
          layer_found = true;
          break;
        }
      }
  ...
  bool IsDeviceSuitable(VkPhysicalDevice device) const noexcept {
    ...
    if (extensions_supported) {
      ...
      swap_chain_adequate = !swap_chain_support.foramts.empty() && !swap_chain_support.present_modes.empty();
    }
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_02_hdr2ldr_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/08_02_hdr2ldr_frag.spv");
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    render_pass_info.renderPass = m_offscreen.renderPass;
    render_pass_info.framebuffer = m_offscreen.framebuffers[m_current_frame];
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen.pipeline);
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);

    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen.pipelineLayout, 0, 1,
                            &m_offscreen.descriptorSets[m_current_frame], 0, nullptr);

    auto draw_drawable = [command_buffer](const Drawable& drawable) {
      std::array vertex_buffers{drawable.vertexBuffer};
    ...
    vkCmdPushConstants(command_buffer, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstant),
                       &m_push_constant);
  ...
  void CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
    ...
    // alignment: 缓冲在实际被分配的内存中的开始位置，依赖于buffer_info的usage和flags
  ...
  void UpdateUniformBuffer(uint32_t current_image) {
    ...
    std::memcpy(m_offscreen.uniformBuffersMapped[current_image], &ubo, sizeof(ubo));
  }
  ...
  void UpdateDescriptorSets() {
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      image_info.imageView = m_offscreen.colors[i].image_view;
      image_info.sampler = m_offscreen_sampler;

      std::array<VkWriteDescriptorSet, 1> descriptor_writes{};

      descriptor_writes[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
      descriptor_writes[0].dstSet = m_descriptor_sets.at(i);
      descriptor_writes[0].dstBinding = 0;
      descriptor_writes[0].dstArrayElement = 0;
