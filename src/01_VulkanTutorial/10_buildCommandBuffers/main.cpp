...
const std::vector<uint16_t> k_indices {0, 1, 2, 0, 2, 3};
...
private:
    ...
    {
        for (auto i = 0U; i < k_max_frames_in_flight; ++i)
        {
            vkResetCommandBuffer(m_command_buffers[i], 0);
            RecordCommandBuffer(m_command_buffers[i], m_swap_chain_framebuffers[i], m_descriptor_sets[i]);
        }
    }
...
private:
    ...
    void CreateSwapChain()
    {
        ...
        if (k_max_frames_in_flight != image_count)
        {
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_10_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_10_base_frag.spv");
    ...
    /// @brief 记录指令到指令缓冲
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer, VkDescriptorSet descriptor_set) const
    {
