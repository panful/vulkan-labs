...
private:
    ...
    void CreateImageViews()
    {
        ...
        {
            m_swap_chain_image_views.at(i) = CreateImageView(m_swap_chain_images.at(i), m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT, 1);
        }
    }
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_08_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/01_08_base_frag.spv");
    ...
    void CreateRenderPass()
    {
        ...
        color_attachment.finalLayout             = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // 渲染流程结束后的图像布局方式，多重采样不需要呈现
    ...
    void CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_swap_chain_image_views.size(); ++i)
        {
            ...
            std::array<VkImageView, 3> attachments {m_color_image_view, m_depth_image_view, m_swap_chain_image_views.at(i)}; // 注意顺序
    ...
    void CreateTextureImage()
    {
        ...
        TransitionImageLayout(
            m_texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, m_mip_levels
        );
        // 执行图像数据复制操作
    ...
    {
        m_texture_image_view = CreateImageView(m_texture_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, m_mip_levels);
    }
    ...
    void CreateTextureSampler()
    {
        ...
        sampler_info.minLod                  = 0.F; // static_cast<float>(m_mipLevels / 2); // 不使用较小细化级别的纹理图像（图像看起来会有些模糊）
    ...
    /// @param texHeight
    /// @param mipLevels
    void GenerateMipmaps(VkImage image, VkFormat image_format, int32_t tex_width, int32_t tex_height, uint32_t mip_levels) const
    {
        ...
        for (uint32_t i = 1; i < mip_levels; ++i)
        {
            ...
            vkCmdPipelineBarrier(
                command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier
            );
            ...
            vkCmdPipelineBarrier(
                command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier
            );
