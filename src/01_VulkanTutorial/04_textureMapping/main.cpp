...
private:
    ...
    void CreateTextureImage()
    {
        ...
        auto pixels = stbi_load(PROJECT_ASSETS_DIR "textures/alpha.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
private:
    ...
    void PrepareImGui()
    {
        ...
        if (ImGui::Combo("Texture", &m_current_texture_index, m_texture_file_names.data(), static_cast<int>(m_texture_file_names.size())))
        {
    ...
    std::unique_ptr<Texture> CreateTextureImage(const std::string& file_path)
    {
        ...
        // 执行图像数据复制操作
        CopyBufferToImage(staging_buffer, texture->image, static_cast<uint32_t>(tex_width), static_cast<uint32_t>(tex_height));
        // 将图像转换为能够在着色器中采样的纹理数据图像
        TransitionImageLayout(
