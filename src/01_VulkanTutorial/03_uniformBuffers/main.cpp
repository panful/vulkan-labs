...
private:
    ...
    {
        auto vertShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base_vert.spv");
        auto fragShaderCode = ReadFile(PROJECT_ASSETS_DIR "shaders/01_03_base_frag.spv");
...
private:
    ...
    void CreateGraphicsPipeline()
    {
        ...
        std::array<VkDescriptorSetLayout, 2> descriptor_set_layouts {m_color_descriptor_set_layout, m_mvp_descriptor_set_layout};
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        if (set_type)
        {
            const std::array<VkDescriptorSet, 2> descriptor_sets {m_descriptor_sets[m_current_frame], m_descriptor_sets[m_current_frame + 2]};
            vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 2, descriptor_sets.data(), 0, nullptr);
        }
        else
        {
            vkCmdBindDescriptorSets(
            ...
            vkCmdBindDescriptorSets(
                command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 1, 1, &m_descriptor_sets[m_current_frame + 2], 0, nullptr
            );
        }
    ...
    {
        {
            ...
            for (size_t i = 0; i < k_max_frames_in_flight; ++i)
            {
                CreateBuffer(
                    ...
                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    m_color_uniform_buffers.at(i),
                    m_color_uniform_buffers_memory.at(i)
                );
                vkMapMemory(m_device, m_color_uniform_buffers_memory.at(i), 0, buffer_size, 0, &m_color_uniform_buffers_mapped.at(i));
            }
        }

        {
            ...
            for (size_t i = 0; i < k_max_frames_in_flight; ++i)
            {
                CreateBuffer(
                    ...
                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    m_mvp_uniform_buffers.at(i),
                    m_mvp_uniform_buffers_memory.at(i)
                );
                vkMapMemory(m_device, m_mvp_uniform_buffers_memory.at(i), 0, buffer_size, 0, &m_mvp_uniform_buffers_mapped.at(i));
            }
        }
    }
