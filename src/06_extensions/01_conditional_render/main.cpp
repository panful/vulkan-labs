...
private:
  ...
  void CreateConditionalRenderingBuffer() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(buffer_size, VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT,
                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                   m_conditional_rendering_buffers.at(i), m_conditional_rendering_buffers_memory.at(i));
      vkMapMemory(m_device, m_conditional_rendering_buffers_memory.at(i), 0, buffer_size, 0,
                  &m_conditional_rendering_buffers_mapped.at(i));
    }
  }
  ...
  void UpdateConditionalRenderingBuffers(size_t current_frame) {
    ...
    std::memcpy(m_conditional_rendering_buffers_mapped[current_frame], s_buffer.data(),
                sizeof(int32_t) * k_conditional_rendering_count);
  }
...
private:
  void SetupExtensionFunctions() {
    m_vk_cmd_begin_conditional_rendering_ext = lvk_tidy::LoadDeviceProcAddress<PFN_vkCmdBeginConditionalRenderingEXT>(
      m_device, "vkCmdBeginConditionalRenderingEXT");
