...
constexpr uint32_t k_off_screen_width = 1024;
constexpr uint32_t k_off_screen_height = 1024;
...
// 逻辑设备的扩展
const std::vector<const char*> k_device_extensions = {VK_KHR_SWAPCHAIN_EXTENSION_NAME, VK_KHR_MULTIVIEW_EXTENSION_NAME};
// 实例的扩展
const std::vector<const char*> k_g_instance_extensions = {VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME};
...
private:
  ...
  void Cleanup() noexcept {
    ...
    for (int i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyFramebuffer(m_device, m_multi_view_frame_buffers[i], nullptr);
      vkDestroyImageView(m_device, m_multi_view_color_image_views[i], nullptr);
      vkDestroyImage(m_device, m_multi_view_color_images[i], nullptr);
      vkFreeMemory(m_device, m_multi_view_color_image_memories[i], nullptr);
      vkDestroyImageView(m_device, m_multi_view_depth_image_views[i], nullptr);
      vkDestroyImage(m_device, m_multi_view_depth_images[i], nullptr);
      vkFreeMemory(m_device, m_multi_view_depth_image_memories[i], nullptr);
    }

    vkDestroySampler(m_device, m_swap_chain_texture_sampler, nullptr);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      vkDestroyPipeline(m_device, m_graphics_pipelines[i], nullptr);
    }
    vkDestroyPipeline(m_device, m_multi_view_graphics_pipeline, nullptr);
    vkDestroyPipelineLayout(m_device, m_pipeline_layout, nullptr);
    vkDestroyRenderPass(m_device, m_render_pass, nullptr);
    vkDestroyPipelineLayout(m_device, m_multi_view_pipeline_layout, nullptr);
    vkDestroyRenderPass(m_device, m_multi_view_render_pass, nullptr);
    ...
    if (m_queue_family_indices.graphicsFamily != m_queue_family_indices.computeFamily) {
      vkDestroyCommandPool(m_device, m_compute_command_pool, nullptr);
    }
    if (m_queue_family_indices.graphicsFamily != m_queue_family_indices.transferFamily) {
...
private:
  ...
  void InitImGui() {
    ...
    vkResetCommandPool(m_device, m_graphics_command_pool, 0);
    ...
    if (VK_SUCCESS != vkQueueSubmit(m_graphics_queue, 1, &end_info, VK_NULL_HANDLE)) {
  ...
  std::vector<const char*> GetRequiredExtensions() const noexcept {
    ...
    required_extensions.insert(required_extensions.cend(), k_g_instance_extensions.cbegin(),
                               k_g_instance_extensions.cend());
    if (k_enable_validation_layers) {
      // 根据需要开启调试报告相关的扩展
  ...
  bool IsDeviceSuitable(VkPhysicalDevice device) noexcept {
    ...
    return m_queue_family_indices.IsComplete() && extensions_supported && swap_chain_adequate;
  }
  ...
  QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice device) const noexcept {
    ...
    for (size_t i = 0; i < queue_families.size(); ++i) {
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(device, static_cast<uint32_t>(i), m_surface, &present_support);
      if (present_support) {
  ...
  void CreateLogicalDevice() {
    ...
    std::set<uint32_t> unique_queue_families{lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily),
                                             lvk_tidy::GetRequiredValue(m_queue_family_indices.presentFamily)};
    ...
    physical_device_multiview_features.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTIVIEW_FEATURES_KHR;
    physical_device_multiview_features.multiview = VK_TRUE;

    VkPhysicalDeviceFeatures2 physical_device_features2 = {};
    physical_device_features2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;
    physical_device_features2.features = device_features;
    physical_device_features2.pNext = &physical_device_multiview_features;

    VkDeviceCreateInfo create_info = {};
    create_info.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
    create_info.queueCreateInfoCount = static_cast<uint32_t>(queue_create_infos.size());
    create_info.pQueueCreateInfos = queue_create_infos.data();
    create_info.pEnabledFeatures = nullptr;  // VkPhysicalDeviceFeatures2.features 已经设置，此处无需再次设置
    ...
    // 4.用来存储返回的队列句柄的内存地址
    vkGetDeviceQueue(m_device, lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily), 0, &m_graphics_queue);
    vkGetDeviceQueue(m_device, lvk_tidy::GetRequiredValue(m_queue_family_indices.presentFamily), 0, &m_present_queue);
  ...
  void CreateSwapChain() {
    ...
    const std::array<uint32_t, 2> queue_family_indices{
      lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily),
      lvk_tidy::GetRequiredValue(m_queue_family_indices.presentFamily)};

    // 在多个队列族使用交换链图像的方式
    if (m_queue_family_indices.graphicsFamily != m_queue_family_indices.presentFamily) {
      // 图形队列和呈现队列不是同一个队列
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_02_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_02_base_frag.spv");
    ...
    specialization_map_entry.constantID = 0;  // constant_id = 0
    specialization_map_entry.offset = 0;
    specialization_map_entry.size = sizeof(SpecializationDataType);

    VkSpecializationInfo specialization_info = {};
    specialization_info.dataSize = sizeof(SpecializationDataType);
    specialization_info.mapEntryCount = 1;
    specialization_info.pMapEntries = &specialization_map_entry;
    specialization_info.pData = nullptr;  // 先设置为nullptr，创建管线时会赋值
    ...
    m_graphics_pipelines.resize(k_max_frames_in_flight);
    for (int i = 0; i < k_max_frames_in_flight; ++i) {
      float view_layer = static_cast<float>(i);
      specialization_info.pData = &view_layer;
      if (VK_SUCCESS !=
          vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipelines[i])) {
  ...
  void CreateDescriptorSetLayout() {
    ...
    std::array bindings = {sampler_layout_binding};
  ...
  void CreateMultiViewRenderPass() {
    ...
    const uint32_t view_mask =
      0b00000011;  // 表示该 subpass 的第0和1个视图是开启的，Shader 中 gl_ViewIndex 可能的值为 0和1
    const uint32_t correlation_mask = 0b00000000;  // 表示视图之间相关性的掩码（比如两个视图需要同步、依赖等）

    VkRenderPassMultiviewCreateInfo render_pass_multiview_ci{};
    render_pass_multiview_ci.sType = VK_STRUCTURE_TYPE_RENDER_PASS_MULTIVIEW_CREATE_INFO;
    render_pass_multiview_ci.subpassCount = 1;
    render_pass_multiview_ci.pViewMasks = &view_mask;  // 每个 subpass 的视图掩码（此处只有一个subpass)
    ...
    render_pass_info.pNext = &render_pass_multiview_ci;

    if (VK_SUCCESS != vkCreateRenderPass(m_device, &render_pass_info, nullptr, &m_multi_view_render_pass)) {
  ...
  void CreateMultiViewFramebuffers() {
    m_multi_view_frame_buffers.resize(k_max_frames_in_flight);
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      std::array<VkImageView, 2> attachments{m_multi_view_color_image_views[i], m_multi_view_depth_image_views[i]};

      VkFramebufferCreateInfo framebuffer_info = {};
      framebuffer_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
      framebuffer_info.renderPass = m_multi_view_render_pass;
      framebuffer_info.attachmentCount = static_cast<uint32_t>(attachments.size());
      framebuffer_info.pAttachments = attachments.data();
      framebuffer_info.width = k_off_screen_width;
      framebuffer_info.height = k_off_screen_height;
      framebuffer_info.layers = 1;

      if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebuffer_info, nullptr, &m_multi_view_frame_buffers[i])) {
  ...
  void CreateMultiViewGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_02_multi_view_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_02_multi_view_frag.spv");
    ...
    if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipeline_layout_info, nullptr, &m_multi_view_pipeline_layout)) {
      throw std::runtime_error("failed to create pipeline layout");
    }
    ...
    pipeline_info.pColorBlendState = &color_blending;
    pipeline_info.pDynamicState = &dynamic_state;
    pipeline_info.layout = m_multi_view_pipeline_layout;
    pipeline_info.renderPass = m_multi_view_render_pass;
    pipeline_info.subpass = 0;                   // 子流程在子流程数组中的索引
    pipeline_info.basePipelineHandle = nullptr;  // 以一个创建好的图形管线为基础创建一个新的图形管线
    pipeline_info.basePipelineIndex = -1;  // 只有该结构体的成员 flags 被设置为 VK_PIPELINE_CREATE_DERIVATIVE_BIT 才有效
    ...
    if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
                                                &m_multi_view_graphics_pipeline)) {
  ...
  void CreateMultiViewColorResources() {
    m_multi_view_color_images.resize(k_max_frames_in_flight);
    m_multi_view_color_image_memories.resize(k_max_frames_in_flight);
    m_multi_view_color_image_views.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(k_off_screen_width, k_off_screen_height, m_swap_chain_image_format, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 0, m_multi_view_color_images[i],
                  m_multi_view_color_image_memories[i], 2);
      m_multi_view_color_image_views[i] =
        CreateImageView(m_multi_view_color_images[i], m_swap_chain_image_format, VK_IMAGE_ASPECT_COLOR_BIT, 2);
    }
  }
  ...
  void CreateSwapChainTextureSampler() {
    ...
    if (VK_SUCCESS != vkCreateSampler(m_device, &sampler_info, nullptr, &m_swap_chain_texture_sampler)) {
  ...
  void CreateMultiViewDepthResources() {
    ...
    m_multi_view_depth_images.resize(k_max_frames_in_flight);
    m_multi_view_depth_image_memories.resize(k_max_frames_in_flight);
    m_multi_view_depth_image_views.resize(k_max_frames_in_flight);

    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateImage(k_off_screen_width, k_off_screen_height, depth_format, VK_IMAGE_TILING_OPTIMAL,
                  VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                  m_multi_view_depth_images[i], m_multi_view_depth_image_memories[i], 2);

      m_multi_view_depth_image_views[i] =
        CreateImageView(m_multi_view_depth_images[i], depth_format, VK_IMAGE_ASPECT_DEPTH_BIT, 2);
    }
  }
  ...
  VkImageView CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspect_flags, uint32_t layer_count) {
    VkImageViewCreateInfo view_info{};
    view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    view_info.image = image;
    view_info.viewType = 2 == layer_count ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D;
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes.at(0).type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  ...
  void CreateDescriptorSets() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      image_info.imageView = m_multi_view_color_image_views[i];
      image_info.sampler = m_swap_chain_texture_sampler;
  ...
  void CreateCommandPool() {
    ...
    pool_info.queueFamilyIndex = lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily);

    if (VK_SUCCESS != vkCreateCommandPool(m_device, &pool_info, nullptr, &m_graphics_command_pool)) {
    ...
    if (m_queue_family_indices.graphicsFamily != m_queue_family_indices.computeFamily) {
      ...
      m_compute_command_pool = m_graphics_command_pool;
    }
    ...
    if (m_queue_family_indices.graphicsFamily != m_queue_family_indices.transferFamily) {
      ...
      m_transfer_command_pool = m_graphics_command_pool;
    }
  }
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    viewport.x = 0.F;
    viewport.y = 0.F;
    viewport.width = static_cast<float>(k_off_screen_width);
    viewport.height = static_cast<float>(k_off_screen_height);
    viewport.minDepth = 0.F;
    viewport.maxDepth = 1.F;

    VkRect2D scissor = {};
    scissor.offset = {0, 0};
    scissor.extent = {k_off_screen_width, k_off_screen_height};
    ...
    render_pass_info.renderPass = m_multi_view_render_pass;                      // 指定使用的渲染流程对象
    render_pass_info.framebuffer = m_multi_view_frame_buffers[m_current_frame];  // 指定使用的帧缓冲对象
    render_pass_info.renderArea.offset = {0, 0};                                 // 指定用于渲染的区域
    render_pass_info.renderArea.extent = {k_off_screen_width, k_off_screen_height};
    ...
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_multi_view_graphics_pipeline);
    ...
    viewport.width = static_cast<float>(m_swap_chain_extent.width) / 2.F;
    viewport.height = static_cast<float>(m_swap_chain_extent.height);
    scissor.extent = m_swap_chain_extent;

    //-------------------------------------------------------------------
    // left viewport
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipelines[0]);
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
    ...
    //-------------------------------------------------------------------
    // right viewport
    viewport.x = static_cast<float>(m_swap_chain_extent.width) / 2.F;
    scissor.offset.x = static_cast<int>(m_swap_chain_extent.width) / 2;
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphics_pipelines[1]);
    vkCmdSetViewport(command_buffer, 0, 1, &viewport);
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
    vkCmdDraw(command_buffer, 6, 1, 0, 0);
  ...
  void CopyBuffer(VkBuffer src_buffer, VkBuffer dst_buffer, VkDeviceSize size) const {
    ...
    // 提交到内存传输指令队列执行内存传输
    if (VK_SUCCESS != vkQueueSubmit(m_graphics_queue, 1, &submit_info, nullptr)) {
...
private:
  ...
  std::vector<VkPipeline> m_graphics_pipelines{};  // 两个 pipeline 有不同的 constant_id(VIEW_LAYER)
  ...
  VkSampler m_swap_chain_texture_sampler{nullptr};

  std::vector<VkImage> m_multi_view_color_images{};
  std::vector<VkDeviceMemory> m_multi_view_color_image_memories{};
  std::vector<VkImageView> m_multi_view_color_image_views{};
  std::vector<VkImage> m_multi_view_depth_images{};
  std::vector<VkDeviceMemory> m_multi_view_depth_image_memories{};
  std::vector<VkImageView> m_multi_view_depth_image_views{};

  VkRenderPass m_multi_view_render_pass{nullptr};
  VkPipelineLayout m_multi_view_pipeline_layout{nullptr};
  VkPipeline m_multi_view_graphics_pipeline{nullptr};
  std::vector<VkFramebuffer> m_multi_view_frame_buffers{};
};
