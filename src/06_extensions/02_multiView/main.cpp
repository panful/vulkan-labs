...
private:
    ...
    std::vector<const char*> GetRequiredExtensions() const noexcept
    {
        ...
        required_extensions.insert(required_extensions.cend(), k_g_instance_extensions.cbegin(), k_g_instance_extensions.cend());
        if (k_enable_validation_layers)
        {
            // 根据需要开启调试报告相关的扩展
    ...
    QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice device) const noexcept
    {
        ...
        auto get_support_queue = [&queue_families](VkQueueFlagBits queue_flag_bits, size_t index) -> std::optional<uint32_t> {
    ...
    void CreateLogicalDevice()
    {
        ...
        std::set<uint32_t> unique_queue_families {
            lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily), lvk_tidy::GetRequiredValue(m_queue_family_indices.presentFamily)
        };
    ...
    void CreateGraphicsPipeline()
    {
        ...
        for (int i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipelines[i]))
            {
    ...
    void CreateMultiViewRenderPass()
    {
        ...
        const uint32_t view_mask        = 0b00000011; // 表示该 subpass 的第0和1个视图是开启的，Shader 中 gl_ViewIndex 可能的值为 0和1
    ...
    void CreateMultiViewGraphicsPipeline()
    {
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_multi_view_graphics_pipeline))
        {
    ...
    void CreateMultiViewColorResources()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            CreateImage(
                ...
                0,
                m_multi_view_color_images[i],
                m_multi_view_color_image_memories[i],
                2
            );
    ...
    void CreateMultiViewDepthResources()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            CreateImage(
                ...
                m_multi_view_depth_images[i],
                m_multi_view_depth_image_memories[i],
                2
            );

            m_multi_view_depth_image_views[i] = CreateImageView(m_multi_view_depth_images[i], depth_format, VK_IMAGE_ASPECT_DEPTH_BIT, 2);
        }
    }
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        clear_values.at(1).depthStencil = {1.F, 0}; // Vulkan 的深度范围是 [0.0, 1.0] 1.0对应视锥体的远平面，初始化时应该设置为远平面的值
        ...
        vkCmdBindDescriptorSets(
            command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1, &m_descriptor_sets[m_current_frame], 0, nullptr
        );
