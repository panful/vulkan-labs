/**
 * 1. m_vk_cmd_begin_rendering_khr
...
// 实例的扩展
const std::vector<const char*> k_instance_extensions = {VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME};
...
private:
  ...
  void TransitionImageLayout(VkImage image, VkFormat /*format*/, VkImageLayout old_layout,
    ...
    VkImageAspectFlags aspect_flags = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL == new_layout
    ...
    barrier.image = image;
    barrier.subresourceRange.aspectMask = aspect_flags;  // 设置布局变换影响的范围
    ...
    } else if (VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL == old_layout &&
    ...
    } else if (VK_IMAGE_LAYOUT_UNDEFINED == old_layout && VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL == new_layout) {
      barrier.srcAccessMask = 0;
  ...
  std::vector<const char*> GetRequiredExtensions() const noexcept {
    ...
    required_extensions.insert(required_extensions.cend(), k_instance_extensions.cbegin(),
                               k_instance_extensions.cend());
    if (k_enable_validation_layers) {
      // 根据需要开启调试报告相关的扩展
  ...
  void CreateLogicalDevice() {
    ...
    physical_device_features_2.features = device_features;
    physical_device_features_2.pNext = &dynamic_rendering_features;
    ...
    create_info.pNext = &physical_device_features_2;
  ...
  void CreateGraphicsPipeline() {
    auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_03_base_vert.spv");
    auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_03_base_frag.spv");
    ...
    rendering_info.pColorAttachmentFormats = &m_swap_chain_image_format;
    rendering_info.depthAttachmentFormat = FindDepthFormat();
    rendering_info.stencilAttachmentFormat = FindDepthFormat();
  ...
  /// @brief 记录指令到指令缓冲
  void RecordCommandBuffer(VkCommandBuffer command_buffer, const uint32_t image_index) const {
    VkCommandBufferBeginInfo begin_info = {};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    ...
    TransitionImageLayout(m_swap_chain_images[image_index], m_swap_chain_image_format, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);

    TransitionImageLayout(m_depth_image, FindDepthFormat(), VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);

    VkRenderingAttachmentInfoKHR color_attachment{};
    color_attachment.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR;
    color_attachment.imageView = m_swap_chain_image_views[image_index];
    color_attachment.imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    color_attachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    color_attachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
    color_attachment.clearValue.color = {0.1F, 0.2F, 0.3F, 1.0F};
    ...
    depth_stencil_attachment.imageView = m_depth_image_view;
    ...
    depth_stencil_attachment.clearValue.depthStencil = {1.F, 0};

    VkRenderingInfoKHR rendering_info{};
    rendering_info.sType = VK_STRUCTURE_TYPE_RENDERING_INFO_KHR;
    rendering_info.renderArea = {0, 0, m_swap_chain_extent.width, m_swap_chain_extent.height};
    ...
    m_vk_cmd_begin_rendering_khr(command_buffer, &rendering_info);
    ...
    m_vk_cmd_end_rendering_khr(command_buffer);

    TransitionImageLayout(m_swap_chain_images[image_index], m_swap_chain_image_format,
...
private:
  ...
  void SetupExtensionFunctions() {
    m_vk_cmd_begin_rendering_khr =
      reinterpret_cast<PFN_vkCmdBeginRenderingKHR>(vkGetDeviceProcAddr(m_device, "vkCmdBeginRenderingKHR"));
    if (nullptr == m_vk_cmd_begin_rendering_khr) {
      throw std::runtime_error("Could not get a valid function pointer for vkCmdBeginRenderingKHR");
    }

    m_vk_cmd_end_rendering_khr =
      reinterpret_cast<PFN_vkCmdEndRenderingKHR>(vkGetDeviceProcAddr(m_device, "vkCmdEndRenderingKHR"));
    if (nullptr == m_vk_cmd_end_rendering_khr) {
  ...
  static void CursorPosCallback(GLFWwindow* window, double xpos, double ypos) noexcept {
    if (auto app = reinterpret_cast<HelloTriangleApplication*>(glfwGetWindowUserPointer(window))) {
      ...
      if (app->m_is_rotating) {
        ...
        // app->m_view_up = glm::normalize(glm::cross(xAxis, -app->m_eye_pos));
      }
...
private:
  ...
  PFN_vkCmdBeginRenderingKHR m_vk_cmd_begin_rendering_khr{nullptr};
  PFN_vkCmdEndRenderingKHR m_vk_cmd_end_rendering_khr{nullptr};
};
...
// 实例的扩展
const std::vector<const char*> k_g_instance_extensions = {};
