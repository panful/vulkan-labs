...
constexpr uint64_t k_conditional_rendering_count {5};
...
// 交换链扩展
const std::vector<const char*> k_device_extensions = {VK_KHR_SWAPCHAIN_EXTENSION_NAME, VK_EXT_CONDITIONAL_RENDERING_EXTENSION_NAME};
...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        {
            vkDestroyBuffer(m_device, m_conditional_rendering_buffers.at(i), nullptr);
            vkFreeMemory(m_device, m_conditional_rendering_buffers_memory.at(i), nullptr);
        }
...
private:
    bool m_check_box0 {true}, m_check_box1 {true}, m_check_box2 {true}, m_check_box3 {true}, m_check_box4 {true};
    ...
    void PrepareImGui()
    {
        ...
        {
            for (uint64_t i = 0; i < k_conditional_rendering_count; ++i)
            {
                ImGui::Checkbox((std::string("CheckBox ") + std::to_string(i)).c_str(), &m_check_boxes.at(i));
            }
        }
        ImGui::End();
        ImGui::Render();
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_01_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/06_01_base_frag.spv");
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        vkCmdBindVertexBuffers(command_buffer, 0, 1, &m_vertex_buffer, offsets.data());
        ...
        for (uint64_t i = 0; i < k_conditional_rendering_count; ++i)
        {
            ...
            conditional_rendering_begin_info.sType  = VK_STRUCTURE_TYPE_CONDITIONAL_RENDERING_BEGIN_INFO_EXT;
            conditional_rendering_begin_info.buffer = m_conditional_rendering_buffers.at(m_current_frame);
            conditional_rendering_begin_info.offset = sizeof(int32_t) * i;

            m_vk_cmd_begin_conditional_rendering_ext(command_buffer, &conditional_rendering_begin_info);
            vkCmdDrawIndexed(command_buffer, 3, 1, static_cast<uint32_t>(3 * i), 0, 0); // 3表示一个三角形3个顶点
            m_vk_cmd_end_conditional_rendering_ext(command_buffer);
        }
    ...
    void DrawFrame()
    {
        ...
        UpdateConditionalRenderingBuffers(m_current_frame);
    ...
    {
        VkDeviceSize buffer_size = sizeof(int32_t) * k_conditional_rendering_count;

        m_conditional_rendering_buffers.resize(k_max_frames_in_flight);
        m_conditional_rendering_buffers_memory.resize(k_max_frames_in_flight);
        m_conditional_rendering_buffers_mapped.resize(k_max_frames_in_flight);
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            CreateBuffer(
                ...
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                m_conditional_rendering_buffers.at(i),
                m_conditional_rendering_buffers_memory.at(i)
            );
            vkMapMemory(m_device, m_conditional_rendering_buffers_memory.at(i), 0, buffer_size, 0, &m_conditional_rendering_buffers_mapped.at(i));
        }
    }

    void UpdateConditionalRenderingBuffers(size_t current_frame)
    {
        static std::vector<int32_t> s_buffer(k_conditional_rendering_count);
        for (uint64_t i = 0; i < k_conditional_rendering_count; ++i)
        {
            s_buffer.at(i) = m_check_boxes.at(i) ? 1 : 0;
        }
        std::memcpy(m_conditional_rendering_buffers_mapped[current_frame], s_buffer.data(), sizeof(int32_t) * k_conditional_rendering_count);
    }
...
private:
    ...
    {
        m_vk_cmd_begin_conditional_rendering_ext =
            lvk_tidy::LoadDeviceProcAddress<PFN_vkCmdBeginConditionalRenderingEXT>(m_device, "vkCmdBeginConditionalRenderingEXT");
        if (!m_vk_cmd_begin_conditional_rendering_ext)
        {
            throw std::runtime_error("Could not get a valid function pointer for vkCmdBeginConditionalRenderingEXT");
        }

        m_vk_cmd_end_conditional_rendering_ext =
            lvk_tidy::LoadDeviceProcAddress<PFN_vkCmdEndConditionalRenderingEXT>(m_device, "vkCmdEndConditionalRenderingEXT");
        if (!m_vk_cmd_end_conditional_rendering_ext)
        {
...
private:
    ...
    PFN_vkCmdBeginConditionalRenderingEXT m_vk_cmd_begin_conditional_rendering_ext {};
    PFN_vkCmdEndConditionalRenderingEXT m_vk_cmd_end_conditional_rendering_ext {};

    std::array<bool, k_conditional_rendering_count> m_check_boxes {true, true, true, true, true};

    std::vector<VkBuffer> m_conditional_rendering_buffers {};
    std::vector<VkDeviceMemory> m_conditional_rendering_buffers_memory {};
    std::vector<void*> m_conditional_rendering_buffers_mapped {};
};
