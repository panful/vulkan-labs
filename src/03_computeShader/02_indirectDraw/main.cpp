...
constexpr uint32_t k_vertex_binding_id {0};
constexpr uint32_t k_instance_binding_id {1};
...
struct Vertex
{
    ...
    static constexpr VkVertexInputBindingDescription GetBindingDescription() noexcept
    {
        ...
        binding_description.binding   = k_vertex_binding_id;
    ...
    static constexpr std::array<VkVertexInputAttributeDescription, 2> GetAttributeDescriptions() noexcept
    {
        ...
        attribute_descriptions[0].binding  = k_vertex_binding_id;
        attribute_descriptions[0].location = 0;
        attribute_descriptions[0].format   = VK_FORMAT_R32G32B32_SFLOAT;
        attribute_descriptions[0].offset   = offsetof(Vertex, pos);

        attribute_descriptions[1].binding  = k_vertex_binding_id;
...
struct InstanceData
{
    ...
    {
        VkVertexInputBindingDescription binding_description {};
        binding_description.binding   = k_instance_binding_id;
    ...
    static constexpr VkVertexInputAttributeDescription GetAttributeDescription() noexcept
    {
        ...
        attribute_description.binding  = k_instance_binding_id;
        attribute_description.format   = VK_FORMAT_R32G32B32_SFLOAT;
        attribute_description.location = 2;
        attribute_description.offset   = offsetof(InstanceData, offset);
...
private:
    ...
    void MainLoop()
    {
        static auto s_last_time {0.};
        static auto s_now_time {0.};
        static auto s_last_second {0.};
        while (!glfwWindowShouldClose(m_window))
        {
            s_now_time = glfwGetTime();
            if (s_now_time - s_last_second > 1.)
            {
                m_fps         = static_cast<uint32_t>(1. / (s_now_time - s_last_time));
                s_last_second = s_now_time;
            }
    ...
    void Cleanup() noexcept
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            vkDestroyBuffer(m_device, m_indirect_draw_buffers.at(i), nullptr);
            vkFreeMemory(m_device, m_indirect_draw_buffers_memory.at(i), nullptr);
        }
...
private:
    ...
    void CreateComputeUniformBuffers()
    {
        ...
        {
            CreateBuffer(
                compute_ubo_buffer_size,
                VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                m_compute_ubo_buffers.at(i),
                m_compute_ubo_buffers_memory.at(i)
            );
            vkMapMemory(m_device, m_compute_ubo_buffers_memory.at(i), 0, compute_ubo_buffer_size, 0, &m_compute_ubo_buffers_mapped.at(i));
        }

        VkDeviceSize indirect_draw_buffer_size = sizeof(VkDrawIndexedIndirectCommand);

        m_indirect_draw_buffers.resize(k_max_frames_in_flight);
        m_indirect_draw_buffers_memory.resize(k_max_frames_in_flight);
        m_indirect_draw_buffers_mapped.resize(k_max_frames_in_flight);
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            CreateBuffer(
                ...
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                m_indirect_draw_buffers.at(i),
                m_indirect_draw_buffers_memory.at(i)
            );
            vkMapMemory(m_device, m_indirect_draw_buffers_memory.at(i), 0, indirect_draw_buffer_size, 0, &m_indirect_draw_buffers_mapped.at(i));
        }
    }
    ...
    void UpdateComputeUniformBuffer(size_t current_image)
    {
        ...
        ubo.firstIndex    = 0;
        ubo.firstInstance = 0;
        ubo.indexCount    = static_cast<uint32_t>(k_indices.size());
    ...
    void CreateComputeDescriptorSets()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            indirect_draw_buffer_info.buffer = m_indirect_draw_buffers.at(i);
            indirect_draw_buffer_info.offset = 0;
            indirect_draw_buffer_info.range  = sizeof(VkDrawIndexedIndirectCommand);
    ...
    {
        auto compute_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_02_base_comp.spv");
    ...
    void PrepareImGui()
    {
        ...
        {
            ImGui::TextUnformatted(("FPS: " + std::to_string(m_fps)).c_str());
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_02_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_02_base_frag.spv");
        ...
        std::array binding_descriptions {Vertex::GetBindingDescription(), InstanceData::GetBindingDescription()};
        std::array attribute_descriptions {
            Vertex::GetAttributeDescriptions()[0], Vertex::GetAttributeDescriptions()[1], InstanceData::GetAttributeDescription()
        };
    ...
    /// @brief 记录指令到指令缓冲
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer, VkBuffer indirect_draw_buffer) const
    {
        VkCommandBufferBeginInfo begin_info = {};
        begin_info.sType                    = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        ...
        const std::array<VkBuffer, 1> instance_buffers {m_instance_buffer};
        const std::array<VkDeviceSize, 1> offsets {0};

        vkCmdBindVertexBuffers(command_buffer, k_vertex_binding_id, 1, vertex_buffers.data(), offsets.data());
        vkCmdBindVertexBuffers(command_buffer, k_instance_binding_id, 1, instance_buffers.data(), offsets.data());
        ...
        vkCmdDrawIndexedIndirect(command_buffer, indirect_draw_buffer, 0, 1, sizeof(VkDrawIndexedIndirectCommand));
    ...
    void DrawFrame()
    {
        ...
        RecordCommandBuffer(
            m_command_buffers.at(image_index), m_swap_chain_framebuffers.at(image_index), m_indirect_draw_buffers.at(m_current_frame)
        );
        ...
        std::array wait_semaphores = {m_compute_finished_semaphores[m_current_frame], m_image_available_semaphores[m_current_frame]};
    ...
    void CreateInstanceBuffer()
    {
        ...
        m_instance_count = static_cast<uint32_t>(num_x * num_y * num_z);

        std::vector<InstanceData> instance_data;
        instance_data.reserve(num_x * num_y * num_z);
        ...
        VkDeviceSize buffer_size = sizeof(InstanceData) * instance_data.size();
        ...
        std::memcpy(data, instance_data.data(), static_cast<size_t>(buffer_size));
        ...
        CreateBuffer(
            ...
            m_instance_buffer,
            m_instance_buffer_memory
        );

        CopyBuffer(staging_buffer, m_instance_buffer, buffer_size);
    ...
    {
        VkDeviceSize buffer_size = sizeof(UBOTransform);
    ...
    /// @param currentImage
    void UpdateUniformBuffer(size_t current_image)
    {
    ...
    void CreateDescriptorPool()
    {
        ...
        pool_sizes.at(0).descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2); // 图形着色器、计算着色器各一个 Uniform
        ...
        pool_info.poolSizeCount = static_cast<uint32_t>(pool_sizes.size());
        pool_info.pPoolSizes    = pool_sizes.data();
        pool_info.maxSets       = static_cast<uint32_t>(k_max_frames_in_flight * 2); //  图形、计算各一个描述符集
    ...
    void CreateDescriptorSets()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            buffer_info.range  = sizeof(UBOTransform);
...
private:
    ...
    VkBuffer m_instance_buffer {nullptr};
    VkDeviceMemory m_instance_buffer_memory {nullptr};
    ...
    glm::vec3 m_eye_pos {0.F, 0.F, -50.F};
    ...
    std::vector<VkBuffer> m_indirect_draw_buffers {};
    std::vector<VkDeviceMemory> m_indirect_draw_buffers_memory {};
    std::vector<void*> m_indirect_draw_buffers_mapped {};
};
