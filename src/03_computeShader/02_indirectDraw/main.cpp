...
private:
  ...
  void CreateComputeUniformBuffers() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      CreateBuffer(indirect_draw_buffer_size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                   m_indirect_draw_buffers.at(i), m_indirect_draw_buffers_memory.at(i));
      vkMapMemory(m_device, m_indirect_draw_buffers_memory.at(i), 0, indirect_draw_buffer_size, 0,
                  &m_indirect_draw_buffers_mapped.at(i));
    }
  }
  ...
  void CreateGraphicsPipeline() {
    ...
    std::array attribute_descriptions{Vertex::GetAttributeDescriptions()[0], Vertex::GetAttributeDescriptions()[1],
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer,
                           VkBuffer indirect_draw_buffer) const {
    VkCommandBufferBeginInfo begin_info = {};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
  ...
  void DrawFrame() {
    ...
    RecordCommandBuffer(m_command_buffers.at(image_index), m_swap_chain_framebuffers.at(image_index),
                        m_indirect_draw_buffers.at(m_current_frame));

    const std::array<VkPipelineStageFlags, 2> wait_stages{VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                                          VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
    std::array wait_semaphores = {m_compute_finished_semaphores[m_current_frame],
  ...
  void CreateInstanceBuffer() {
    ...
    for (size_t x = 0; x < num_x; ++x) {
      for (size_t y = 0; y < num_y; ++y) {
        for (size_t z = 0; z < num_z; ++z) {
          ...
          instance_data.emplace_back(InstanceData{{tmp_x, tmp_y, tmp_z}});
        }
      }
    }
    ...
    CreateBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_instance_buffer, m_instance_buffer_memory);
  ...
  void CreateDescriptorPool() {
    ...
    pool_sizes.at(0).descriptorCount =
      static_cast<uint32_t>(k_max_frames_in_flight * 2);  // 图形着色器、计算着色器各一个 Uniform
