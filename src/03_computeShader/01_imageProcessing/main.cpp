...
// 窗口默认大小
constexpr uint32_t k_width  = 1000;
...
struct Vertex
{
    ...
    static constexpr std::array<VkVertexInputAttributeDescription, 2> GetAttributeDescriptions() noexcept
    {
        ...
        attribute_descriptions[1].location = 2; // 和顶点着色器的 layout(location = 2) in 对应
...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        vkDestroyImageView(m_device, m_target_texture_image_view, nullptr);
        vkDestroyImage(m_device, m_target_texture_image, nullptr);
        vkFreeMemory(m_device, m_target_texture_image_memory, nullptr);
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            vkDestroySemaphore(m_device, m_compute_finished_semaphores.at(i), nullptr);
...
private:
    ...
    bool IsDeviceSuitable(VkPhysicalDevice device) noexcept
    {
        ...
        return m_queue_family_indices.IsComplete() && extensions_supported && swap_chain_adequate && supported_features.samplerAnisotropy;
    }
    ...
    void CreateLogicalDevice()
    {
        ...
        std::set<uint32_t> unique_queue_families {
            ...
            lvk_tidy::GetRequiredValue(m_queue_family_indices.computeFamily),
    ...
    {
        auto compute_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_01_base_comp.spv");
    ...
    {
        auto vert_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_01_base_vert.spv");
        auto frag_shader_code = ReadFile(PROJECT_ASSETS_DIR "shaders/03_01_base_frag.spv");
    ...
    {
        m_compute_command_buffers.resize(m_swap_chain_framebuffers.size());
    ...
    void RecordComputeCommandBuffer(VkCommandBuffer command_buffer)
    {
        ...
        vkCmdBindDescriptorSets(
            ...
            0,
            1,
            &m_compute_descriptor_sets.at(m_current_frame),
            0,
            nullptr
        );
        ...
        auto group_count_x = static_cast<uint32_t>(std::ceil(static_cast<double>(m_texture_width) / 16.));
        auto group_count_y = static_cast<uint32_t>(std::ceil(static_cast<double>(m_texture_height) / 16.));
    ...
    void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const
    {
        ...
        viewport.x          = 0.F;
        viewport.y          = 0.F;
        viewport.width      = static_cast<float>(m_swap_chain_extent.width / 2);
        viewport.height     = static_cast<float>(m_swap_chain_extent.height);
        ...
        vkCmdBindDescriptorSets(
            command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1, &m_pre_descriptor_sets.at(m_current_frame), 0, nullptr
        );
        vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);

        //----------------------------------------------------------------
        viewport.x = static_cast<float>(m_swap_chain_extent.width / 2);
        vkCmdSetViewport(command_buffer, 0, 1, &viewport);
        vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers.data(), offsets.data());
        vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);
        vkCmdBindDescriptorSets(
            command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1, &m_post_descriptor_sets.at(m_current_frame), 0, nullptr
        );
        vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
    ...
    void DrawFrame()
    {
        ...
        submit_info.pWaitSemaphores      = wait_semaphores.data();                            // 指定队列开始执行前需要等待的信号量
    ...
    void CreateComputeSyncObjects()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            if (VK_SUCCESS != vkCreateSemaphore(m_device, &semaphore_info, nullptr, &m_compute_finished_semaphores.at(i))
                || VK_SUCCESS != vkCreateFence(m_device, &fence_info, nullptr, &m_compute_in_flight_fences.at(i)))
            {
    ...
    void EndSingleTimeCommands(VkCommandBuffer command_buffer) const
    {
        ...
        // 提交到内存传输指令队列执行内存传输
        if (VK_SUCCESS != vkQueueSubmit(m_transfer_queue, 1, &submit_info, nullptr))
        {
    ...
    {
        VkDescriptorSetLayoutBinding input_image = {};
        input_image.binding                      = 0;
        input_image.descriptorType               = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        input_image.descriptorCount              = 1;
        input_image.stageFlags                   = VK_SHADER_STAGE_COMPUTE_BIT; // 指定在哪一个着色器阶段使用
        input_image.pImmutableSamplers           = nullptr;

        VkDescriptorSetLayoutBinding output_image = {};
        output_image.binding                      = 1;
        output_image.descriptorType               = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        output_image.descriptorCount              = 1;
        output_image.stageFlags                   = VK_SHADER_STAGE_COMPUTE_BIT;
        output_image.pImmutableSamplers           = nullptr;

        std::array<VkDescriptorSetLayoutBinding, 2> bindings = {input_image, output_image};
    ...
    {
        auto aspect = static_cast<float>(m_swap_chain_extent.width / 2) / static_cast<float>(m_swap_chain_extent.height);
    ...
    void CreateDescriptorPool()
    {
        ...
        pool_sizes.at(0).descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2); // pre post 两个 uniform
        pool_sizes.at(1).type            = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        pool_sizes.at(1).descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2); // pre post 两个 sampler
        pool_sizes.at(2).type            = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
        pool_sizes.at(2).descriptorCount = static_cast<uint32_t>(k_max_frames_in_flight * 2); // 两个 STORAGE_IMAGE

        VkDescriptorPoolCreateInfo pool_info {};
        pool_info.sType         = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
        pool_info.poolSizeCount = static_cast<uint32_t>(pool_sizes.size());
        pool_info.pPoolSizes    = pool_sizes.data();
        pool_info.maxSets       = static_cast<uint32_t>(k_max_frames_in_flight * 3); // pre post compute 共3个描述符集
    ...
    void CreateComputeDescriptorSets()
    {
        ...
        for (size_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            input_image_info.imageLayout = VK_IMAGE_LAYOUT_GENERAL; // 图形支持任何类型的操作：读取、写入、图形着色器、计算着色器
            input_image_info.imageView   = m_texture_image_view;
            input_image_info.sampler     = m_texture_sampler;

            VkDescriptorImageInfo output_image_info {};
            output_image_info.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
            output_image_info.imageView   = m_target_texture_image_view;
            output_image_info.sampler     = m_texture_sampler;
            ...
            descriptor_writes.at(0).descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE; // 对应着色器中的 image2D
            descriptor_writes.at(0).descriptorCount  = 1;
            descriptor_writes.at(0).pBufferInfo      = nullptr;                          // 指定描述符引用的缓冲数据
            descriptor_writes.at(0).pImageInfo       = &input_image_info;                // 指定描述符引用的图像数据
            ...
            descriptor_writes.at(1).descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
            descriptor_writes.at(1).descriptorCount  = 1;
            descriptor_writes.at(1).pBufferInfo      = nullptr;
            descriptor_writes.at(1).pImageInfo       = &output_image_info;
    ...
    void CreateDescriptorSets()
    {
        ...
        m_pre_descriptor_sets.resize(k_max_frames_in_flight);
        if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_pre_descriptor_sets.data()))
        {
            throw std::runtime_error("failed to allocate pre descriptor sets");
        }

        m_post_descriptor_sets.resize(k_max_frames_in_flight);
        if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &alloc_info, m_post_descriptor_sets.data()))
        {
            throw std::runtime_error("failed to allocate post descriptor sets");
        }

        auto update_descriptor_sets = [this](const std::vector<VkDescriptorSet>& descriptor_sets, VkImageView image_view) {
            for (size_t i = 0; i < k_max_frames_in_flight; ++i)
            {
                ...
                // mvp变换矩阵
                descriptor_writes.at(0).sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
                descriptor_writes.at(0).dstSet           = descriptor_sets.at(i); // 指定要更新的描述符集对象
                ...
                // 纹理采样器
                descriptor_writes.at(1).sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
                descriptor_writes.at(1).dstSet           = descriptor_sets.at(i);
        ...
        update_descriptor_sets(m_pre_descriptor_sets, m_texture_image_view);
        update_descriptor_sets(m_post_descriptor_sets, m_target_texture_image_view);
    }
    ...
    {
        CreateImage(
            static_cast<uint32_t>(m_texture_width),
            static_cast<uint32_t>(m_texture_height),
            VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            m_target_texture_image,
            m_target_texture_image_memory
        );

        TransitionImageLayout(m_target_texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);

        m_target_texture_image_view = CreateImageView(m_target_texture_image, VK_FORMAT_R8G8B8A8_UNORM);
    }
    ...
    void CreateTextureImage()
    {
        ...
        auto pixels = stbi_load(PROJECT_ASSETS_DIR "textures/nightsky.png", &m_texture_width, &m_texture_height, &tex_channels, STBI_rgb_alpha);
        std::cout << "image extent: " << m_texture_width << '\t' << m_texture_height << '\t' << tex_channels << '\n';
        if (!pixels)
        {
            throw std::runtime_error("failed to load texture image");
        }

        VkDeviceSize image_size = m_texture_width * m_texture_height * 4;
        ...
        TransitionImageLayout(m_texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        // 执行图像数据复制操作
        CopyBufferToImage(staging_buffer, m_texture_image, static_cast<uint32_t>(m_texture_width), static_cast<uint32_t>(m_texture_height));
        // 将图像转换为能够在着色器中采样的纹理数据图像
        TransitionImageLayout(m_texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL);
    ...
    )
    {
        ...
        std::vector<uint32_t> queue_family_indices;           // 不能放在 if else 内部，因为调用 vkCreateImage 之前会释放
        std::set<uint32_t> unique_queue_family_indices {
            lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily),
            lvk_tidy::GetRequiredValue(m_queue_family_indices.computeFamily),
            lvk_tidy::GetRequiredValue(m_queue_family_indices.transferFamily)
        };
        if (1 == unique_queue_family_indices.size())
        {
        ...
        {
            queue_family_indices.insert(queue_family_indices.begin(), unique_queue_family_indices.begin(), unique_queue_family_indices.end());
    ...
    void TransitionImageLayout(VkImage image, VkFormat /*format*/, VkImageLayout old_layout, VkImageLayout new_layout)
    {
        ...
        else if (VK_IMAGE_LAYOUT_GENERAL == new_layout && VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
        {
            barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
...
private:
    ...
    std::vector<VkDescriptorSet> m_pre_descriptor_sets {};
    std::vector<VkDescriptorSet> m_post_descriptor_sets {};
    ...
    VkImage m_target_texture_image {nullptr};
    VkDeviceMemory m_target_texture_image_memory {nullptr};
    VkImageView m_target_texture_image_view {nullptr};
