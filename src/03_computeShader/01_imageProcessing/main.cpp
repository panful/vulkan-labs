...
private:
  ...
  void RecordCommandBuffer(VkCommandBuffer command_buffer, VkFramebuffer framebuffer) const {
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1,
                            &m_pre_descriptor_sets.at(m_current_frame), 0, nullptr);
    ...
    vkCmdBindIndexBuffer(command_buffer, m_index_buffer, 0, VK_INDEX_TYPE_UINT16);
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 0, 1,
                            &m_post_descriptor_sets.at(m_current_frame), 0, nullptr);
    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
  ...
  void DrawFrame() {
    ...
    const std::array<VkPipelineStageFlags, 2> wait_stages{VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                                          VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
    std::array<VkSemaphore, 2> wait_semaphores = {m_compute_finished_semaphores[m_current_frame],
  ...
  void CreateComputeDescriptorSets() {
    ...
    for (size_t i = 0; i < k_max_frames_in_flight; ++i) {
      ...
      input_image_info.imageLayout =
        VK_IMAGE_LAYOUT_GENERAL;  // 图形支持任何类型的操作：读取、写入、图形着色器、计算着色器
  ...
  void CreateTargetTexture() {
    CreateImage(static_cast<uint32_t>(m_texture_width), static_cast<uint32_t>(m_texture_height),
                VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                m_target_texture_image, m_target_texture_image_memory);

    TransitionImageLayout(m_target_texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  void CreateTextureImage() {
    ...
    auto pixels = stbi_load(PROJECT_ASSETS_DIR "textures/nightsky.png", &m_texture_width, &m_texture_height,
    ...
    TransitionImageLayout(m_texture_image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    // 执行图像数据复制操作
    CopyBufferToImage(staging_buffer, m_texture_image, static_cast<uint32_t>(m_texture_width),
  ...
  void CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
    ...
    std::set<uint32_t> unique_queue_family_indices{lvk_tidy::GetRequiredValue(m_queue_family_indices.graphicsFamily),
                                                   lvk_tidy::GetRequiredValue(m_queue_family_indices.computeFamily),
                                                   lvk_tidy::GetRequiredValue(m_queue_family_indices.transferFamily)};
    if (1 == unique_queue_family_indices.size()) {
      ...
      queue_family_indices.insert(queue_family_indices.begin(), unique_queue_family_indices.begin(),
  ...
  void CreateTextureImageView() { m_texture_image_view = CreateImageView(m_texture_image, VK_FORMAT_R8G8B8A8_UNORM); }
