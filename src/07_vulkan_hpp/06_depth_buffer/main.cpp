...
const std::vector<Vertex> k_vertices  {
    ...
    {{ 0.2F,  0.5F, 0.0F}, {0.0F, 1.0F, 0.0F}, {1.0F, 1.0F}},
    {{ 0.2F, -0.5F, 0.0F}, {0.0F, 1.0F, 0.0F}, {1.0F, 0.0F}},

    {{-0.2F, -0.5F, 0.1F}, {1.0F, 0.0F, 0.0F}, {0.0F, 0.0F}},
    {{-0.2F,  0.5F, 0.1F}, {1.0F, 0.0F, 0.0F}, {0.0F, 1.0F}},
    {{ 0.8F,  0.5F, 0.1F}, {0.0F, 1.0F, 0.0F}, {1.0F, 1.0F}},
    {{ 0.8F, -0.5F, 0.1F}, {0.0F, 1.0F, 0.0F}, {1.0F, 0.0F}},
};
...
vk::raii::Pipeline MakeGraphicsPipeline(
  ...
  std::vector<std::pair<vk::Format, uint32_t>> const& vertex_input_attribute_format_offset, vk::FrontFace front_face,
  bool depth_buffered, vk::raii::PipelineLayout const& pipeline_layout, vk::raii::RenderPass const& render_pass) {
  static_cast<void>(vertex_stride);
  static_cast<void>(vertex_input_attribute_format_offset);
  ...
  static_cast<void>(vertex_input_attribute_descriptions);
  ...
  vk::PipelineDepthStencilStateCreateInfo pipeline_depth_stencil_state_create_info(
    {}, depth_buffered, depth_buffered, vk::CompareOp::eLessOrEqual, false, false, {}, {});
...
template <typename T>
void CopyToDevice(vk::raii::DeviceMemory const& device_memory, T const* data, size_t count,
...
struct BufferData {
  ...
  void Upload(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device,
    ...
    OneTimeSubmit(device, command_pool, queue, [&](vk::raii::CommandBuffer const& cmd_buf) {
      cmd_buf.copyBuffer(*staging_buffer.buffer, *this->buffer, vk::BufferCopy(0, 0, data_size));
    });
  }

  vk::raii::DeviceMemory device_memory = nullptr;  // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
  vk::raii::Buffer buffer = nullptr;               // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
...
struct ImageData {
  ImageData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device, vk::Format fmt,
    ...
    image_view = create_image_view
                   ? vk::raii::ImageView(device, vk::ImageViewCreateInfo({}, image, vk::ImageViewType::e2D, fmt, {},
                                                                         {aspect_mask, 0, 1, 0, 1}))
                   : nullptr;
  }
...
struct DepthBufferData : public ImageData {
  DepthBufferData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device, vk::Format format,
                  vk::Extent2D const& extent)
      : ImageData(physical_device, device, format, extent, vk::ImageTiling::eOptimal,
...
struct TextureData {
  TextureData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device,
              vk::Extent2D const& extent_param = {256, 256}, vk::ImageUsageFlags usage_flags = {},
              vk::FormatFeatureFlags format_feature_flags = {}, bool anisotropy_enable = false,
    ...
    if (needs_staging) {
      ...
      staging_buffer_data =
        BufferData(physical_device, device, extent.width * extent.height * 4, vk::BufferUsageFlagBits::eTransferSrc);
    ...
    image_data =
      ImageData(physical_device, device, format, extent, image_tiling, usage_flags | vk::ImageUsageFlagBits::eSampled,
                initial_layout, requirements, vk::ImageAspectFlagBits::eColor);
  }

  template <typename ImageGenerator>
  void SetImage(vk::raii::CommandBuffer const& command_buffer, ImageGenerator const& image_generator) {
    void* data = needs_staging ? staging_buffer_data.device_memory.mapMemory(
    ...
    needs_staging ? staging_buffer_data.device_memory.unmapMemory() : image_data.device_memory.unmapMemory();

    if (needs_staging) {
      // Since we're going to blit to the texture image, set its layout to eTransferDstOptimal
      SetImageLayout(command_buffer, image_data.image, image_data.format, vk::ImageLayout::eUndefined,
                     vk::ImageLayout::eTransferDstOptimal);
      vk::BufferImageCopy copy_region(0, extent.width, extent.height,
                                      vk::ImageSubresourceLayers(vk::ImageAspectFlagBits::eColor, 0, 0, 1),
                                      vk::Offset3D(0, 0, 0), vk::Extent3D(extent, 1));
      command_buffer.copyBufferToImage(staging_buffer_data.buffer, image_data.image,
      ...
      SetImageLayout(command_buffer, image_data.image, image_data.format, vk::ImageLayout::eTransferDstOptimal,
                     vk::ImageLayout::eShaderReadOnlyOptimal);
    } else {
      // If we can use the linear tiled image as a texture, just do it
      SetImageLayout(command_buffer, image_data.image, image_data.format, vk::ImageLayout::ePreinitialized,
                     vk::ImageLayout::eShaderReadOnlyOptimal);
    }
  }
...
int main() {
  try {
    ...
    const auto& physical_device = physical_devices[use_physical_device_index];
    ...
    DepthBufferData depth_image_data(physical_device, device, depth_format, extent);
    ...
    vk::SubpassDescription subpass_description({}, vk::PipelineBindPoint::eGraphics, {}, color_attachments, {},
                                               &depth_attachment, {});
    ...
    int tex_channels{0};
    auto pixels =
      stbi_load(PROJECT_ASSETS_DIR "textures/nightsky.png", &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
    assert(pixels);

    TextureData texture_data(physical_device, device,
                             {static_cast<uint32_t>(tex_width), static_cast<uint32_t>(tex_height)});
    OneTimeSubmit(device, command_pool, graphics_and_transfer_queue,
                  [&texture_data, pixels](vk::raii::CommandBuffer const& cmd_buf) {
                    texture_data.SetImage(cmd_buf, [pixels](void* data, vk::Extent2D extent) {
    ...
    // 三个采样器是一样的，因此使用同一个
    vk::DescriptorImageInfo descriptor_image_info(texture_data.sampler, texture_data.image_data.image_view,
                                                  vk::ImageLayout::eShaderReadOnlyOptimal);

    std::array<std::array<vk::WriteDescriptorSet, 2>, k_max_frames_in_flight> write_descriptor_sets{
      std::array{vk::WriteDescriptorSet{descriptor_sets[0], 9, 0, vk::DescriptorType::eUniformBuffer, nullptr,
                                        descriptor_buffer_info_0},
                 vk::WriteDescriptorSet{descriptor_sets[0], 6, 0, vk::DescriptorType::eCombinedImageSampler,
                                        descriptor_image_info, nullptr}},
      std::array{vk::WriteDescriptorSet{descriptor_sets[1], 9, 0, vk::DescriptorType::eUniformBuffer, nullptr,
                                        descriptor_buffer_info_1},
                 vk::WriteDescriptorSet{descriptor_sets[1], 6, 0, vk::DescriptorType::eCombinedImageSampler,
                                        descriptor_image_info, nullptr}},
      std::array{vk::WriteDescriptorSet{descriptor_sets[2], 9, 0, vk::DescriptorType::eUniformBuffer, nullptr,
                                        descriptor_buffer_info_2},
                 vk::WriteDescriptorSet{descriptor_sets[2], 6, 0, vk::DescriptorType::eCombinedImageSampler,
                                        descriptor_image_info, nullptr}}};
    ...
    //--------------------------------------------------------------------------------------
    std::array<std::array<vk::ImageView, 2>, k_max_frames_in_flight> image_views{
      std::array<vk::ImageView, 2>{color_image_datas[0].image_view, depth_image_data.image_view},
      std::array<vk::ImageView, 2>{color_image_datas[1].image_view, depth_image_data.image_view},
      std::array<vk::ImageView, 2>{color_image_datas[2].image_view, depth_image_data.image_view},
    };
    ...
    while (s_count++ < 9) {
      ...
      auto result = device.waitForFences({draw_fences[g_current_frame_index], blit_fences[g_current_frame_index]},
      ...
      // 更新 Uniform
      UniformBufferObject default_ubo{};
      default_ubo.model = glm::mat4(1.0F);
      default_ubo.view = glm::lookAt(glm::vec3(0.0F, 0.0F, -2.0F), glm::vec3(0.0F), glm::vec3(0.0F, 1.0F, 0.0F));
      default_ubo.proj = glm::perspective(glm::radians(45.0F), 8.F / 6.F, 0.1F, 100.0F);
      default_ubo.proj[1][1] *= -1;

      CopyToDevice(uniform_buffer_objects[g_current_frame_index].device_memory, default_ubo);
      ...
      clear_values[0].color = vk::ClearColorValue(0.2F, 0.2F, 0.2F, 1.0F);
      ...
      std::array<vk::Semaphore, 1> semaphore{render_finished_semaphore[g_current_frame_index]};
      vk::SubmitInfo draw_submit_info({}, {}, draw_command_buffers, semaphore);
      ...
      vk::SubmitInfo blit_submit_info(semaphore, wait_stages, blit_command_buffers, {});
