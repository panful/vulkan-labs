...
template <typename T>
void CopyToDevice(vk::raii::DeviceMemory const& device_memory, T const* data_ptr, size_t count,
                  vk::DeviceSize stride = sizeof(T)) {
  assert(sizeof(T) <= stride);
  uint8_t* device_data = static_cast<uint8_t*>(device_memory.mapMemory(0, count * stride));
  if (stride == sizeof(T)) {
    memcpy(device_data, data_ptr, count * sizeof(T));
  } else {
    for (size_t i = 0; i < count; i++) {
      memcpy(device_data, &data_ptr[i], sizeof(T));
      device_data += stride;
    }
  }
...
struct ImageData {
  ImageData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device, vk::Format format_value,
    ...
    image_view = create_image_view
                   ? vk::raii::ImageView(device, vk::ImageViewCreateInfo({}, image, vk::ImageViewType::e2D,
                                                                         format_value, {}, {aspect_mask, 0, 1, 0, 1}))
                   : nullptr;
  }
...
int main() {
  try {
    ...
    //--------------------------------------------------------------------------------------
    std::vector<uint32_t> vert_spv = ReadFile(PROJECT_ASSETS_DIR "shaders/01_02_base_vert.spv");
    std::vector<uint32_t> frag_spv = ReadFile(PROJECT_ASSETS_DIR "shaders/01_02_base_frag.spv");
    ...
    while (s_count++ < 9) {
      ...
      command_buffers[g_current_frame_index].bindVertexBuffers(0, {vertex_buffer_data.buffer}, {0});
      command_buffers[g_current_frame_index].bindIndexBuffer(index_buffer_data.buffer, 0, vk::IndexType::eUint16);
      command_buffers[g_current_frame_index].drawIndexed(static_cast<uint32_t>(k_indices.size()), 1, 0, 0, 0);
      ...
      vk::SubmitInfo draw_submit_info(0, nullptr, nullptr, 1, draw_command_buffers.data(), 1, semaphores.data());
      ...
      vk::SubmitInfo blit_submit_info(1, semaphores.data(), wait_stages.data(), 1, blit_command_buffers.data());
