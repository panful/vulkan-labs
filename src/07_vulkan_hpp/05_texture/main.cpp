...
struct BufferData {
  BufferData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device, vk::DeviceSize size,
    ...
    deviceMemory = AllocateDeviceMemory(device, physical_device.getMemoryProperties(), buffer.getMemoryRequirements(),
                                        property_flags);
  ...
  void Upload(DataType const& data) const {
    ...
    void* data_ptr = deviceMemory.mapMemory(0, sizeof(DataType));
  ...
  void Upload(std::vector<DataType> const& data, size_t stride = 0) const {
    ...
    CopyToDevice(deviceMemory, data.data(), data.size(), element_size);
  }
  ...
  void Upload(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device,
    ...
    CopyToDevice(staging_buffer.deviceMemory, data.data(), data.size(), element_size);
  ...
  ImageData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device, vk::Format format,
      ...
      : format(format),
        image(device, {vk::ImageCreateFlags(),
                       ...
                       {},
                       initial_layout}) {
    deviceMemory = AllocateDeviceMemory(device, physical_device.getMemoryProperties(), image.getMemoryRequirements(),
                                        memory_properties);
...
struct TextureData {
  TextureData(vk::raii::PhysicalDevice const& physical_device, vk::raii::Device const& device,
              vk::Extent2D const& extent = {256, 256}, vk::ImageUsageFlags usage_flags = {},
  ...
  void SetImage(vk::raii::CommandBuffer const& command_buffer, ImageGenerator const& image_generator) {
    void* data = needs_staging ? staging_buffer_data.deviceMemory.mapMemory(
                                   0, staging_buffer_data.buffer.getMemoryRequirements().size)
                               : image_data.deviceMemory.mapMemory(0, image_data.image.getMemoryRequirements().size);
    image_generator(data, extent);
    needs_staging ? staging_buffer_data.deviceMemory.unmapMemory() : image_data.deviceMemory.unmapMemory();
...
int main() {
  try {
    ...
    //--------------------------------------------------------------------------------------
    std::vector<uint32_t> vert_spv = ReadFile(PROJECT_SHADER_DIR "07_05_base_vert.spv");
    std::vector<uint32_t> frag_spv = ReadFile(PROJECT_SHADER_DIR "07_05_base_frag.spv");
    ...
    CopyToDevice(uniform_buffer_objects[0].deviceMemory, default_ubo);
    CopyToDevice(uniform_buffer_objects[1].deviceMemory, default_ubo);
    CopyToDevice(uniform_buffer_objects[2].deviceMemory, default_ubo);
    ...
    OneTimeSubmit(device, command_pool, graphics_and_transfer_queue,
                  [&texture_data, pixels](vk::raii::CommandBuffer const& command_buffer) {
                    texture_data.SetImage(command_buffer, [pixels](void* data, vk::Extent2D data_extent) {
                      std::memcpy(data, pixels, static_cast<size_t>(data_extent.width * data_extent.height * 4));
                    });
                  });
    ...
    // 三个采样器是一样的，因此使用同一个
    vk::DescriptorImageInfo descriptor_image_info(texture_data.sampler, texture_data.image_data.imageView,
                                                  vk::ImageLayout::eShaderReadOnlyOptimal);
    ...
    std::array<std::array<vk::ImageView, 1>, k_max_frames_in_flight> image_views{
      std::array<vk::ImageView, 1>{color_image_datas[0].imageView},
      std::array<vk::ImageView, 1>{color_image_datas[1].imageView},
      std::array<vk::ImageView, 1>{color_image_datas[2].imageView}};
    ...
    while (s_count++ < 9) {
      ...
      auto save_image_pixels = reinterpret_cast<uint8_t*>(
        save_image_datas[g_current_frame_index].deviceMemory.mapMemory(0, vk::WholeSize, {}));
      ...
      save_image_datas[g_current_frame_index].deviceMemory.unmapMemory();

      // 更新 Uniform
      UniformBufferObject default_ubo{};
      default_ubo.model =
        glm::rotate(glm::mat4(1.F), glm::radians(30.F * static_cast<float>(s_count)), glm::vec3(0.F, 0.F, 1.F));
      CopyToDevice(uniform_buffer_objects[g_current_frame_index].deviceMemory, default_ubo);
      ...
      std::array<vk::Semaphore, 1> semaphores{*render_finished_semaphore[g_current_frame_index]};
      vk::ArrayProxy<const vk::CommandBuffer> draw_command_buffer_proxy(draw_command_buffers);
      vk::ArrayProxy<const vk::Semaphore> draw_semaphore_proxy(semaphores);
      vk::SubmitInfo draw_submit_info({}, {}, draw_command_buffer_proxy, draw_semaphore_proxy);
      ...
      vk::ArrayProxy<const vk::Semaphore> semaphore_proxy(semaphores);
      vk::ArrayProxy<const vk::PipelineStageFlags> wait_stages_proxy(wait_stages);
      vk::ArrayProxy<const vk::CommandBuffer> command_buffer_proxy(blit_command_buffers);
      vk::SubmitInfo blit_submit_info(semaphore_proxy, wait_stages_proxy, command_buffer_proxy);
